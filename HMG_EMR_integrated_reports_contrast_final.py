#!/usr/bin/env python3
"""
HMG Hospital - Complete EMR System v13.2.0
FIXED: Collapsible clerking sections (overlap + rendering bugs resolved)
"""
...
# ---------------------------------------------------------------------------
# High-contrast Spinner styling
# Keeps the existing Spinner-based structure, but makes the closed control
# and its dropdown options clearly visible on Android/light backgrounds.
# ---------------------------------------------------------------------------
class HMGSpinnerOption(SpinnerOption):
...
class HMGSpinner(Spinner):
    def __init__(self, **kwargs):
        # Do not alter how the rest of the application creates Spinners.
        # This subclass only supplies a readable visual theme.
        kwargs.setdefault('background_normal', '')
        kwargs.setdefault('background_down', '')
        kwargs.setdefault('background_color', get_color_from_hex('#34495E'))
        kwargs.setdefault('color', get_color_from_hex('#FFFFFF'))
        kwargs.setdefault('font_size', 14)
        kwargs.setdefault('bold', False)
        super().__init__(**kwargs)
        self.option_cls = HMGSpinnerOption
        self._hmg_style_dropdown()

    def _hmg_style_dropdown(self):
        # The actual DropDown is created by Spinner on first opening.
        # Styling option_cls is the reliable Kivy/Android part; these
        # bindings additionally refresh option appearance after creation.
        try:
            self.bind(is_open=self._hmg_dropdown_opened)
        except Exception:
            pass

    def _hmg_dropdown_opened(self, instance, is_open):
        if not is_open:
            return
        try:
            dropdown = getattr(self, '_dropdown', None)
            if dropdown is not None:
                dropdown.background_color = get_color_from_hex('#1F2937')
                dropdown.bar_width = 8
                for child in list(dropdown.container.children):
                    if isinstance(child, HMGSpinnerOption):
                        child.background_normal = ''
                        child.background_down = ''
                        child.background_color = get_color_from_hex('#2C3E50')
                        child.color = get_color_from_hex('#FFFFFF')
                        child.height = max(42, child.height)
        except Exception:
            pass


# High-contrast button styling. Existing call-sites remain unchanged; explicit
# colors supplied by the application are preserved via setdefault().
class HMGButton(Button):
    """High-contrast button that avoids overriding Kivy's state event."""
    def __init__(self, **kwargs):
        kwargs.setdefault('background_normal', '')
        kwargs.setdefault('background_down', '')
        kwargs.setdefault('background_disabled_normal', '')
        kwargs.setdefault('background_color', get_color_from_hex('#1769AA'))
        kwargs.setdefault('color', get_color_from_hex('#FFFFFF'))
        kwargs.setdefault('font_size', 14)
        super().__init__(**kwargs)

# Existing code continues to use the names Spinner/Button, so no structural
# rewrite or individual call-site changes are required.
Spinner = HMGSpinner
Button = HMGButton
...
class CollapsibleSection(BoxLayout):
    ...
    Fix summary:
      - Collapsed body: height=0 + opacity=0 + disabled=True
        (height=0 alone is not enough — Kivy keeps rendering the children)
      - Header uses background_normal='' so background_color actually shows
      - Toggling notifies parent via a bound callback so parent re-flows
    """

    def __init__(self, title, start_open=False, on_height_change=None, **kwargs):
        ...
        self.header_btn = Button(
            ...
            background_normal='',      # Kivy quirk — needed for color to show
            background_down='',
            color=(1, 1, 1, 1),
        )
        self.header_btn.bind(on_press=lambda x: self.toggle())
        ...
        self.body.bind(minimum_height=self._on_body_min_change)
        self.add_widget(self.body)

        Clock.schedule_once(lambda dt: self._apply_state(), 0)

    def _header_text(self):
        return f"{'▼  ' if self._is_open else '▶  '}{self._title}"

    def _on_body_min_change(self, *args):
        if self._is_open:
            self.body.height = self.body.minimum_height
            self._recalc()
    ...
    def _recalc(self):
        self.height = self.header_btn.height + self.body.height + self.spacing
        if callable(self._on_height_change):
            self._on_height_change()

    def toggle(self):
        self._is_open = not self._is_open
        self.header_btn.text = self._header_text()
        self._apply_state()


# ====================== USER MANAGEMENT ======================
class UserManager:
    ROLES = ["Administrator", "Doctor", "Nurse", "Pharmacist",
    ...
    def _ensure_default_admin(self):
        if not self.users:
            ...
            self.users.append({
                ...
                "last_login": "", "must_change_password": True
            })
            self.save()
            print(f"[UserManager] Default admin created: "
    ...
    def create_user(self, username, full_name, password, role):
        ...
        self.users.append({
            ...
            "role": role, "salt": salt, "password_hash": h,
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "last_login": "", "must_change_password": False
        })
        self.save()
    ...
    def delete_user(self, username):
        ...
        self.users = [u for u in self.users if u.get("username", "").lower() != target]
        self.save()
    ...
    def change_password(self, username, current_password, new_password):
        ...
        if not user:
            return False, "User not found."
        if not self._verify(current_password, user.get("salt", ""), user.get("password_hash", "")):
...
class LoginScreen(BoxLayout):
    def __init__(self, user_manager, on_login, **kwargs):
        ...
        self.add_widget(Label(text="HMG HOSPITAL EMR", font_size=32, bold=True,
                              color=get_color_from_hex('#1a5276'),
                              size_hint_y=None, height=50))
        self.add_widget(Label(text="Electronic Medical Records System", font_size=14,
                              color=get_color_from_hex('#7f8c8d'),
                              size_hint_y=None, height=24))
        self.add_widget(Label(size_hint_y=0.05))

        card = BoxLayout(orientation='vertical', spacing=10, padding=24,
                         size_hint=(0.7, None), size_hint_y=None, height=340)
        card.pos_hint = {"center_x": 0.5}
        card.add_widget(Label(text="Sign In", font_size=22, bold=True,
                              color=get_color_from_hex('#2c3e50'),
                              size_hint_y=None, height=34))
        card.add_widget(Label(text="Username", font_size=13, bold=True,
        ...
        card.add_widget(Label(text="Password", font_size=13, bold=True,
                              size_hint_y=None, height=20))
        self.password_input = TextInput(multiline=False, font_size=16, password=True,
                                        size_hint_y=None, height=42,
                                        hint_text="Enter password",
                                        on_text_validate=self._attempt_login)
        card.add_widget(self.password_input)
        self.error_label = Label(text="", font_size=12,
                                 color=get_color_from_hex('#e74c3c'),
                                 size_hint_y=None, height=24)
        card.add_widget(self.error_label)
        btn_row = BoxLayout(size_hint_y=None, height=46, spacing=8)
        btn_row.add_widget(Button(text="LOGIN", font_size=16, bold=True,
                                  background_color=get_color_from_hex('#27ae60'),
                                  background_normal='',
                                  on_press=self._attempt_login))
        btn_row.add_widget(Button(text="EXIT", font_size=16, bold=True,
                                  background_color=get_color_from_hex('#e74c3c'),
                                  background_normal='',
                                  on_press=self._exit_app))
        card.add_widget(btn_row)
        card.add_widget(Label(text="Default: admin / admin123  (change after first login)",
                              font_size=11, color=get_color_from_hex('#7f8c8d'),
                              size_hint_y=None, height=20))
    ...
    def save(self, provider, api_key, model):
        ...
        try:
            with open("ai_settings.json", "w") as f:
                json.dump({"provider": provider, "api_key": self.api_key,
...
class PaymentTracker:
    ...
    def add(self, pid, bid, amt, method="Cash"):
        p = {"id": f"PAY{datetime.now().strftime('%Y%m%d%H%M%S')}",
             "patient_id": pid, "bill_id": bid, "amount": amt,
             "method": method,
             "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
...
class TaskManager:
    STATUSES = ["Pending", "In Progress", "Completed", "Waiting",
                "On Hold", "Cancelled"]
    ...
    def create_task(self, patient_id, patient_name, assigned_to, task_type,
                    ...
                    department=None):
        task = {"id": f"TASK{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                "patient_id": patient_id, "patient_name": patient_name,
                "assigned_to": assigned_to, "task_type": task_type,
                "description": description, "order_items": order_items or [],
                "status": "Pending", "priority": priority,
                "department": department,
                "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "completed_at": None, "assigned_to_name": "", "notes": ""}
    ...
    def auto_assign_from_emr(self, emr_record, patient):
        ...
        for test in ensure_list(emr_record.get("lab_orders", "")):
            if test and test.strip():
                tasks.append(self.create_task(
                    ...
                    assigned_to="Lab Scientist", task_type="Laboratory",
                    description=f"Lab test: {test}", order_items=[test],
        ...
        for study in ensure_list(emr_record.get("radiology_orders", "")):
            if study and study.strip():
                tasks.append(self.create_task(
                    patient_id=patient.get("id"),
                    patient_name=patient.get("name"),
                    assigned_to="Radiologist", task_type="Radiology",
                    description=f"Radiology: {study}", order_items=[study],
                    department="Radiology"))
        for drug in (ensure_list(emr_record.get("medications")) +
                     ensure_list(emr_record.get("prescribed_drugs"))):
            if drug and drug.strip():
                tasks.append(self.create_task(
                    patient_id=patient.get("id"),
                    patient_name=patient.get("name"),
                    assigned_to="Pharmacy", task_type="Pharmacy",
                    description=f"Dispense: {drug}", order_items=[drug],
                    department="Pharmacy"))
        ...
        if nursing and any(nursing):
            ...
            tasks.append(self.create_task(
                patient_id=patient.get("id"),
                patient_name=patient.get("name"),
                assigned_to="Nurse", task_type="Nursing",
                description=f"Nursing care: {txt}",
                order_items=nursing if isinstance(nursing, list) else [txt],
                department="Nursing"))
        self.save()
        return tasks
...
3. HEART RATE (Ventricular, Atrial, Method, Interpretation)
4. RHYTHM (Regularity, Diagnosis, Supporting features)
...
17. ISCHEMIC CHANGES
18. HYPERTROPHY
19. ELECTROLYTE PATTERN
20. DRUG EFFECTS
21. COMPARISON WITH PREVIOUS ECG
22. FINAL IMPRESSION
23. RECOMMENDATIONS

Output a JSON summary with keys: rate, rhythm, axis, pr_interval, qrs_duration, qt_qtc, findings (list), interpretation, urgency, recommendations.
"""
    ...
    def analyze(self, pt, ecg_data=None, previous_ecg=None):
        ...
        sd = {"rate": "72 bpm", "rhythm": "Sinus rhythm, regular",
              "axis": "Normal (+30°)", "pr_interval": "160 ms",
              "qrs_duration": "90 ms", "qt_qtc": "400/420 ms",
              "findings": ["Normal ECG"], "interpretation": "Normal ECG",
              "urgency": "Routine", "recommendations": ["Continue management"]}
        return {"analysis": analysis, "structured": sd, "ai_used": False,
                "report_id": rid}
...
Hospital No.: {pt.get('id','N/A')}
Age/Sex: {pt.get('age','N/A')} / {pt.get('gender','N/A')}
Date & Time of ECG: {datetime.now().strftime('%Y-%m-%d %H:%M')}
Clinical Indication: {pt.get('ecg_reason','Routine')}

Technical Quality
Calibration: 10 mm/mV | Paper Speed: 25 mm/sec | Quality: Good

ECG ANALYSIS
1. Heart Rate: Ventricular 72 bpm | Atrial 72 bpm | Method: 300 | Normal
2. Rhythm: Regular | Normal Sinus Rhythm
3. Cardiac Axis: Normal | +30°
4. P Wave: Present, normal morphology
5. PR Interval: 160 ms (Normal 120–200 ms)
6. QRS Complex: 90 ms | Narrow | Normal voltage
7. R Wave Progression: Normal
8. Pathological Q Waves: No
9. ST Segment: No elevation | No depression
10. T Wave: Normal upright
11. QT Interval: 400 ms | QTc 420 ms | Bazett | Normal
12. U Waves: No
13. Ectopic Activity: None
14. Arrhythmias: None
15. Ischemic Changes: None
16. Hypertrophy: None
17. Electrolyte Pattern: No changes
18. Drug Effects: None
19. Comparison: Not available

Final Impression
Normal sinus rhythm at 72 bpm.
Normal cardiac axis.
PR 160 ms | QRS 90 ms | QTc 420 ms.
Normal R-wave progression.
No pathological Q waves, no ST changes, normal T waves.
...
class ECGReportGenerator:
    ...
    def generate(result, pt):
        rid = result.get("report_id",
                          f"ECG-{datetime.now().strftime('%Y%m%d%H%M%S')}")
        qr = QRCodeGenerator.generate(f"HMG|ECG|{rid}|{pt.get('name','')}")
        return result.get("analysis", "No report generated."), rid, qr


# ====================== CLINICAL AI ASSISTANT ======================
class ModularAIClinicalAssistant:
    SP = """You are an expert clinical assistant. Conduct a structured interview:
- Chief Complaint → HPI → PMH → Medications → Allergies → Family History → Social History → ROS.
- Ask ONE question at a time.
- Output [DIAGNOSIS_READY] when sufficient information.
- Provide differential diagnosis, investigations, treatment plan, JSON EMR data.
"""
    ...
    def process(self, msg):
        ...
        if self.ai.enabled:
            ...
            ctx += "\nNext: ask question OR provide [DIAGNOSIS_READY]."
...
class AIPathologyExtractor:
    ...
    def extract(self, pd, emr, labs, rx):
        ctx = (f"PATIENT: {pd.get('name','')} | {pd.get('age','')}yr | "
               f"{pd.get('gender','')}\n")
        for e in emr[-5:]:
        ...
        return {"analysis": f"Pathology Analysis\nAbnormal: {ac}\nRisk: {rl}",
                "structured": {"risk_score": risk, "risk_level": rl,
...
class LISManager:
        ...
        self.instrument_configs = {
            "lab": {
                "hematology_analyzer": {
                    ...
                    "manufacturer": "Beckman Coulter", "model": "DxH 900",
                    "port": 5001, "protocol": "ASTM", "status": "Stopped",
                    "auto_process": True,
                    "test_mapping": {"CBC": "Full Blood Count (FBC)"}},
                "chemistry_analyzer": {
                    "name": "Chemistry Analyzer",
                    "manufacturer": "Roche", "model": "Cobas c311",
                    "port": 5002, "protocol": "HL7", "status": "Stopped",
                    "auto_process": True,
                    "test_mapping": {"GLU": "Blood Glucose (Fasting)"}}},
            "radiology": {
                "ct_scanner": {"name": "CT Scanner", "manufacturer": "Siemens",
                               "model": "SOMATOM", "port": 5101,
                               "protocol": "DICOM", "modality": "CT",
                               "status": "Stopped", "auto_process": False},
                "xray_machine": {"name": "X-Ray Machine", "manufacturer": "Philips",
                                 "model": "DigitalDiagnost", "port": 5103,
                                 "protocol": "DICOM", "modality": "X-Ray",
                                 "status": "Stopped", "auto_process": False}},
            "vitals": {
                "bp_machine": {"name": "BP Machine", "manufacturer": "Omron",
                               "model": "HEM-907", "port": 5201,
                               "protocol": "Serial", "status": "Stopped",
                               "auto_process": True,
                               "test_mapping": {"BP": "Blood Pressure"}},
                "ecg_machine": {"name": "ECG Machine",
                                "manufacturer": "GE Healthcare",
                                "model": "MAC 2000", "port": 5202,
                                "protocol": "DICOM", "status": "Stopped",
                                "auto_process": False}}
        }
        self.load_configs()
    ...
    def start_instrument(self, category, instrument_id):
        ...
        if category in ["lab", "vitals"]:
            server["thread"] = threading.Thread(
                ...
                args=(server_id, config["port"]), daemon=True)
        else:
            server["thread"] = threading.Thread(
            ...
            while server_id in self.servers and self.servers[server_id].get("running"):
                try:
                    ...
                    threading.Thread(target=self._handle_lab,
                                     args=(c, server_id), daemon=True).start()
                ...
                while '\n' in buf or '\r' in buf:
                    ...
                    if line and '|' in line:
                        ...
                        if (len(p) >= 4 and
                            ...
                            self._auto_process({
                                "instrument": p[0], "patient_id": p[1],
                                "test_code": p[2], "result": p[3]}, server_id)
                        client.send(b"ACK\r\n")
        except Exception as e:
    ...
    def _auto_process(self, result, server_id):
        ...
        test_name = test_mapping.get(result.get("test_code", "").upper(),
        ...
        self.app.lab_tests.append({
            ...
            "test_name": test_name, "status": "Completed",
            "result": result.get("result", ""), "unit": "", "reference": "",
...
class AIBOS:
    ...
    def calculate_health_score(self):
        areas = ["Leadership", "Strategy", "Finance", "Marketing", "Sales",
                 "Customer Service", "Operations", "HR", "Technology",
                 "AI Readiness", "Innovation", "Brand", "Customer Experience",
                 "Cybersecurity", "Legal Compliance"]
        self.health_scores = {a: (30 if (a == "AI Readiness" and not self.ai.enabled)
                                   else 50) for a in areas}
        return self.health_scores

    def generate_market_intelligence(self):
        self.market_intel["analysis"] = (
            "MARKET INTELLIGENCE REPORT\n\n"
            "Market Size: Growing market with significant potential.\n"
            "Customer Behavior: Digital convenience valued highly.\n"
            "Competitor Analysis:\n- Strengths: Brand\n- Weaknesses: Slow tech\n"
            "Opportunities: Digital transformation, AI engagement\n"
            "Threats: Regulation, Competition\n"
            "Positioning: Technology-forward, customer-centric.\n")
        return self.market_intel

    def generate_revenue_engine(self):
        opps = ["Referral programs", "Affiliate programs", "Loyalty programs",
        ...
        analysis = {o: {
            "revenue_potential": "High" if o in ["Subscription models", "Partnerships"] else "Medium",
            "profit_potential": "High" if o in ["Digital products", "Licensing"] else "Medium",
            "cost": "Low" if o in ["Referral programs", "Affiliate programs"] else "Medium",
            "roi": "High" if o in ["Subscription models", "Digital products"] else "Medium"
        } for o in opps}
        self.revenue_engine = {
            "opportunities": analysis,
            "ranked": sorted(analysis.items(),
                             key=lambda x: (x[1]['roi'] == "High",
                                            x[1]['revenue_potential'] == "High"),
                             reverse=True)}
        return self.revenue_engine

    def generate_sales_engine(self):
        self.sales_engine = {
            "strategy": "Customer-centric sales approach.",
            "sales_scripts": ["Outreach", "Discovery", "Demo", "Closing", "Follow-up"],
            "crm_workflow": "Lead → Qualify → Nurture → Pitch → Close → Onboard",
            "lead_qualification": "BANT",
            "sales_kpis": {"monthly_revenue": "N500,000",
                           "conversion_rate": "25%"}}
        return self.sales_engine

    def generate_marketing_engine(self):
        self.marketing_engine = {
            "strategy": "Integrated digital marketing",
            "brand_positioning": "Leading tech-enabled provider",
            "seo_plan": ["Keyword research", "Local SEO", "Content"],
            "social_media_campaigns": ["Facebook ads", "LinkedIn"],
    ...
    def generate_customer_experience_engine(self):
        self.cx_engine = {
            "onboarding": ["Welcome email", "Intro call"],
            "customer_support": ["24/7 hotline", "Live chat", "Email"],
            "customer_retention": ["Check-ins", "Surveys", "Loyalty"],
            "customer_journey_mapping": ["Awareness", "Consideration", "Decision",
    ...
    def generate_financial_engine(self):
        ...
        self.financial_engine = {
            "profit_loss": {"revenue": rev, "cost_of_goods": rev * 0.4,
                            "gross_profit": rev * 0.6,
                            "operating_expenses": rev * 0.3,
                            "net_profit": rev * 0.3},
            "budget_recommendations": {"marketing": rev * 0.1,
                                        "operations": rev * 0.15,
                                        "technology": rev * 0.05,
                                        "staff": rev * 0.25},
            "cost_reduction_opportunities": ["Negotiate supplier contracts",
                                              "Automate processes"]}
    ...
    def generate_ai_automation_engine(self):
        self.ai_automation = {
            ...
            "appointment_booking": ["Online booking", "Automated reminders"],
            "marketing": ["AI email personalization"],
            "sales": ["Lead scoring automation"],
            "roi_prioritization": {
                "high_impact": ["Chatbot", "Appointment booking"],
    ...
    def generate_document(self, doc_type, custom_data=None):
        name = self.business_profile.get('company_name', 'HMG Hospital')
        docs = {
            "business_plan": f"BUSINESS PLAN: {name}\n\n1. SUMMARY\n{name} delivers innovation.\n",
            "sales_plan": "SALES PLAN\n\nMonthly Revenue: N500,000\n",
            "marketing_plan": "MARKETING PLAN\n\nBrand: Tech-enabled provider\n",
            "referral_program": f"REFERRAL PROGRAM\n\n{name} rewards referrals.\n",
            "sop": f"STANDARD OPERATING PROCEDURE - {name}\n",
            "employee_handbook": f"EMPLOYEE HANDBOOK - {name}\n",
            "financial_report": f"FINANCIAL REPORT - {name}\nPeriod: {datetime.now().strftime('%B %Y')}\n",
            "risk_assessment": "RISK ASSESSMENT\n\nOperational, Financial, Mitigation\n",
            "pitch_deck": f"PITCH DECK - {name}\n",
            "grant_proposal": f"GRANT PROPOSAL - {name}\n",
            "partnership_proposal": f"PARTNERSHIP PROPOSAL - {name}\n",
            "project_plan": f"PROJECT PLAN - {name}\n",
            "kpi_dashboard": "KPI DASHBOARD\n"}
        result = docs.get(doc_type, "Document type not found.")
        self.documents[doc_type] = result
        return result

    def generate_implementation_roadmap(self):
        self.implementation_roadmap = {
            "first_7_days": {"milestones": ["Discovery", "Quick wins"], "costs": "N0"},
            "first_30_days": {"milestones": ["Revenue engine", "Sales plan"], "costs": "N200,000"},
            "first_90_days": {"milestones": ["AI automation", "Financial ops"], "costs": "N500,000"},
            "first_6_months": {"milestones": ["Expansion", "Integration"], "costs": "N1,000,000"},
            "first_year": {"milestones": ["Revenue targets", "Improvement"], "costs": "N2,000,000"}}
        return self.implementation_roadmap

    def decision_support(self, context, options):
        analysis = {"context": context, "options": options,
                    "recommendation": options[0] if options else "Continue current strategy",
                    "success_metrics": ["20% revenue increase",
...
class HMGBillingApp(App):
    ...
    # ---------- Build ----------
    def build(self):
        self.title = "HMG Hospital EMR v13.2.0"
    ...
    def _on_login_success(self, user):
        ...
        self.audit.log(username=user.get("username"), action="LOGIN",
                       details=f"Role: {user.get('role')}")
        self.root_container.clear_widgets()
        self._build_main_app()
        if user.get("must_change_password"):
            Clock.schedule_once(lambda dt: self._popup(
                "Security Warning",
                "You are using a default or reset password.\n\n"
                "Go to Settings → SECURITY → CHANGE MY PASSWORD."), 0.5)

    def _logout(self, instance=None):
        if self.current_user:
            self.audit.log(username=self.current_user.get("username"),
                           action="LOGOUT")
        self.save_all()
        self.current_user = None
        self.root_container.clear_widgets()
    ...
    def _build_main_app(self):
        ...
        self._user_management_tab()
        self._audit_log_tab()

        self.root_container.add_widget(self.main_root)
        Clock.schedule_once(lambda dt: self._refresh_dash(), 0.5)

    # ---------- Config / Delete Passcode ----------
    def _load_config(self):
        if not os.path.exists(self.CONFIG_FILE):
            default = {"delete_passcode_hash":
                       self._hash_passcode(self.DEFAULT_DELETE_PASSCODE)}
            try:
        ...
        try:
            ...
            if "delete_passcode_hash" not in cfg:
                cfg["delete_passcode_hash"] = self._hash_passcode(
                    self.DEFAULT_DELETE_PASSCODE)
                with open(self.CONFIG_FILE, "w", encoding="utf-8") as f:
                    json.dump(cfg, f, indent=2)
            return cfg
        except Exception as e:
            print(f"Config read error: {e}")
            return {"delete_passcode_hash":
                    self._hash_passcode(self.DEFAULT_DELETE_PASSCODE)}
    ...
    def load_all(self):
        files = {
            "patients.json": "patients", "emr.json": "emr_records",
            "lab.json": "lab_tests", "rad.json": "radiology",
            "rx.json": "prescriptions", "bills.json": "bills",
            "drugs.json": "drugs", "invs.json": "investigations",
            "ecg.json": "ecg_reports", "pathology.json": "pathology_data",
            "followups.json": "followups", "summaries.json": "patient_summaries",
            ...
            "lab_results.json": "lab_results", "tasks.json": "tasks",
    ...
    def save_all(self):
        data = {
            "patients.json": self.patients, "emr.json": self.emr_records,
            "lab.json": self.lab_tests, "rad.json": self.radiology,
            "rx.json": self.prescriptions, "bills.json": self.bills,
            "drugs.json": self.drugs, "invs.json": self.investigations,
            ...
            "lab_results.json": self.lab_results, "tasks.json": self.tasks,
    ...
    def _current_username(self):
        return (self.current_user.get("username", "system")
                if self.current_user else "system")
    ...
    def audit_log(self, action, details="", patient_id=""):
        try:
            self.audit.log(username=self._current_username(), action=action,
                           details=details, patient_id=patient_id)
        except Exception as e:
    ...
    def _patient_identity_for_report(self, patient_id=None):
        """Return the canonical patient identity from the EMR for any report."""
        pid = (patient_id or "").strip()
        if not pid and getattr(self, 'cp', None):
            pid = str(self.cp.get('id', '')).strip()
        patient = None
        for pt in getattr(self, 'patients', []):
            if pid and str(pt.get('id', '')).strip() == pid:
                patient = pt
                break
        if patient is None and getattr(self, 'cp', None):
            patient = self.cp
        if patient is None:
            return {"id": pid, "name": "", "age": "", "gender": "", "dob": ""}
        return {
            "id": str(patient.get('id', pid)),
            "name": str(patient.get('name', '')),
            "age": str(patient.get('age', '')),
            "gender": str(patient.get('gender', patient.get('sex', ''))),
            "dob": str(patient.get('dob', '')),
        }

    def _get_patient_lab_results(self, patient_id):
        """Collect reportable laboratory data from the EMR and saved items.

        Completed results are preferred. If a laboratory test has only been
        ordered/billed, it is included as an explicitly marked pending item;
        no result is invented. This connects Enhanced Lab reporting to the
        existing saved billing/lab workflow without changing that workflow.
        """
        ...
        def add_row(row, source):
            ...
            if not name:
                return
            row['source'] = source
            key = (name.lower(), str(row.get('result', '')).strip(),
        ...
        # 1. Actual laboratory orders/results. Keep completed and pending
        # records so an ordered test is visible in the generated report.
        for t in getattr(self, 'lab_tests', []):
            if str(t.get('patient_id', '')).strip() != pid:
                continue
            status = str(t.get('status', '')).strip() or 'Pending'
            is_completed = status.lower() == 'completed'
            add_row({
                'id': t.get('test_id', ''), 'patient_id': pid,
                'patient_name': t.get('patient_name', ''),
                'test_name': t.get('test_name', ''),
                'result': t.get('result', '') if is_completed else '—',
                'unit': t.get('unit', ''),
                'reference': t.get('reference', ''),
                'date': t.get('date_completed') or t.get('date_ordered', ''),
                'status': 'Completed' if is_completed else 'Ordered — Result Pending',
                'category': 'Laboratory'
            }, 'Lab Orders')

        # 2. Enhanced Lab results.
        for r in getattr(self, 'lab_results', []):
            if str(r.get('patient_id', '')).strip() != pid:
                continue
            row = dict(r)
            row.setdefault('status', 'Completed')
            row.setdefault('reference', r.get('reference', r.get('reference_range', '')))
            row.setdefault('category', 'Laboratory')
            add_row(row, 'Enhanced Lab')

        # 3. Saved billing items. These are orders/charges, not laboratory
        # results, so they are added only when they clearly represent a lab
        # investigation and are labelled as pending if no result exists.
        def clean_lab_name(raw):
            nm = str(raw or '').strip()
            # Remove common billing prefixes such as "Lab:", "Lab.",
            # and leading item numbers ("1. FBC").
            nm = re.sub(r'^\s*lab(?:oratory)?\s*[:.\-]?\s*', '', nm, flags=re.I)
            nm = re.sub(r'^\s*\d+\s*[.)\-:]\s*', '', nm)
            return nm.strip()

        def looks_like_lab(item):
            raw = str(item.get('name', '')).strip()
            cat = str(item.get('cat', '')).strip().lower()
            low = raw.lower()
            if low.startswith(('lab:', 'lab.', 'lab ', 'laboratory')):
                return True
            if 'laboratory' in cat or cat == 'investigations':
                # Do not pull radiology/ECG into the laboratory report.
                return not any(x in low for x in ('x-ray', 'xray', 'ultrasound',
                                                   'scan', 'mri', 'ct ', 'ecg'))
            lab_terms = ('fbc', 'full blood count', 'cbc', 'urinalysis',
                         'lipid profile', 'lft', 'renal function', 'urea',
                         'creatinine', 'electrolyte', 'glucose', 'h.pylori',
                         'h. pylori', 'malaria', 'blood film', 'd-dimer',
                         'liver function', 'thyroid', 'albumin', 'bilirubin')
            return any(term in low for term in lab_terms)

        def add_saved_bill_items(items, when):
            for item in items or []:
                if not looks_like_lab(item):
                    continue
                nm = clean_lab_name(item.get('name', ''))
                if not nm:
                    continue
                add_row({
                    'id': '', 'patient_id': pid,
                    'patient_name': self.elab_name.text if hasattr(self, 'elab_name') else '',
                    'test_name': nm, 'result': '—', 'unit': '', 'reference': '',
                    'date': when or '',
                    'status': 'Ordered — Result Pending',
                    'category': 'Laboratory'
                }, 'Saved Items')

        # Current unsaved bill items for the selected patient.
        current_bill_pid = self.bill_pid.text.strip() if hasattr(self, 'bill_pid') else ''
        if current_bill_pid == pid:
            add_saved_bill_items(getattr(self, 'bill_items', []), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

        # Persisted bills.
        for bill in getattr(self, 'bills', []):
            if str(bill.get('patient', {}).get('id', '')).strip() != pid:
                continue
            add_saved_bill_items(bill.get('items', []), bill.get('ts', ''))

        combined.sort(key=lambda x: str(x.get('date', '')), reverse=True)
        return combined

    def _render_professional_report_image(self, text, prefix, title, patient=None, report_id=None):
        """Create the same branded, identity/QR/signature/stamp image format for every report."""
        if not PIL_AVAILABLE:
            return None
        patient = patient or self._patient_identity_for_report()
        now = datetime.now()
        rid = report_id or f"HMG-{prefix.upper()}-{now.strftime('%Y%m%d%H%M%S')}"
        W, H = 1654, 2339
        ...
        try:
            ...
            def F(sz, bold=False):
                path = os.path.join(font_dir, 'Roboto-Bold.ttf' if bold else 'Roboto-Regular.ttf')
                return ImageFont.truetype(path, sz)
            f10, f11, f14, f18, f26 = F(28), F(31), F(39, True), F(52, True), F(72, True)
        except Exception:
            f10=f11=f14=f18=f26=None
        navy, blue, light, dark, muted = '#123B73', '#1976A8', '#EAF4FB', '#1E2B38', '#66727D'
        draw.rectangle((0,0,W,150), fill=navy)
        draw.text((55,28), 'HMG HOSPITAL LTD', fill='white', font=f26)
        draw.text((55,100), '5-Obazi Gate, Abonnema, Akulga, Rivers State', fill='white', font=f10)
        draw.text((W-55,38), '...Quality Care, Healthier Tomorrow', fill='white', font=f11, anchor='ra')
        draw.rectangle((45,180,W-45,275), fill=blue)
        draw.text((W//2,228), title.upper(), fill='white', font=f18, anchor='mm')
        # Patient identity panel
        y=305
        draw.rounded_rectangle((45,y,W-45,y+205), radius=16, fill=light, outline=blue, width=3)
        left=[('Patient Name',patient.get('name','')),('Patient ID',patient.get('id','')),('Age',patient.get('age','')),('Sex',patient.get('gender',''))]
        right=[('Report ID',rid),('Date',now.strftime('%Y-%m-%d %H:%M:%S')),('Reporter','Dr. Precious Belema Ibiabuo')]
        yy=y+25
        for k,v in left:
            draw.text((70,yy), f'{k}:', fill=dark, font=f11); draw.text((285,yy), str(v), fill=dark, font=f11); yy+=40
        yy=y+25
        for k,v in right:
            draw.text((850,yy), f'{k}:', fill=dark, font=f11); draw.text((1050,yy), str(v), fill=dark, font=f10); yy+=48
        # Report body
        yy=y+235
        draw.text((55,yy), 'REPORT DETAILS', fill=navy, font=f14); yy+=50
        lines=[]
        ...
        for line in lines[:max_lines]:
            ...
            for word in words:
                ...
                if len(trial)>82:
                    draw.text((65,yy), cur, fill=dark, font=f10); yy+=34; cur=word
                else: cur=trial
            if cur:
                draw.text((65,yy), cur, fill=dark, font=f10); yy+=34
            if yy>1880: break
        # Signature / QR / stamp
        footer=1930
        draw.line((45,footer,W-45,footer), fill=blue, width=3)
        draw.text((60,footer+30),'REPORTED BY',fill=muted,font=f10)
        draw.text((60,footer+68),'Dr. Precious Belema Ibiabuo',fill=navy,font=f14)
        draw.text((60,footer+110),'B.Sc., MBBS, M.Sc., MLT, FWACS (Primary)',fill=dark,font=f10)
        draw.text((60,footer+145),'Medical Director, HMG Hospital Ltd',fill=dark,font=f10)
        # electronic signature mark
        draw.arc((60,footer+150,300,footer+205),195,345,fill=navy,width=5)
        draw.line((80,footer+203,290,footer+193),fill=navy,width=4)
        # QR
        try:
            qr_b64=QRCodeGenerator.generate(f'HMG|{prefix}|{rid}|{patient.get("id","")}|{patient.get("name","")}')
            if qr_b64:
                q=PILImage.open(BytesIO(base64.b64decode(qr_b64))).convert('RGB').resize((175,175))
                img.paste(q,(W//2-88,footer+25))
                draw.text((W//2,footer+208),'SCAN TO VERIFY REPORT',fill=navy,font=f10,anchor='ma')
        except Exception: pass
        sx,sy,sr=W-170,footer+110,95
        draw.ellipse((sx-sr,sy-sr,sx+sr,sy+sr),outline=navy,width=7)
        draw.ellipse((sx-sr+13,sy-sr+13,sx+sr-13,sy+sr-13),outline=blue,width=3)
        draw.text((sx,sy-38),'HMG HOSPITAL',fill=navy,font=f10,anchor='mm')
        draw.text((sx,sy),'PATHOLOGY',fill=navy,font=f10,anchor='mm')
        draw.text((sx,sy+35),'✓',fill=navy,font=f18,anchor='mm')
        draw.text((sx,sy+62),'ABONNEMA',fill=navy,font=f10,anchor='mm')
        draw.rectangle((0,H-55,W,H),fill=navy)
        draw.text((55,H-28),'HMG Hospital Ltd | Medical Records & Diagnostic Services',fill='white',font=f10,anchor='lm')
        path=os.path.join(self._get_reports_dir(),f'{prefix}_{rid}.png')
    ...
    def _export_to_pdf(self, text, prefix, title):
        try:
            ...
            patient = self._patient_identity_for_report()
            image_path = self._render_professional_report_image(text, prefix, title, patient=patient)
            if PDF_AVAILABLE:
                try:
                    class PDF(FPDF):
                        def header(self):
                            self.set_font('Arial', 'B', 12)
                            self.cell(0, 10, 'HMG Hospital Ltd', 0, 1, 'C')
                            self.ln(5)
                        def footer(self):
                            self.set_y(-15); self.set_font('Arial', 'I', 8)
                            self.cell(0, 10, f'Page {self.page_no()}', 0, 0, 'C')
                    pdf = PDF(); pdf.add_page(); pdf.set_font('Arial','B',16)
                    pdf.cell(0,10,self._sanitize_text(title),ln=True,align='C'); pdf.ln(5)
                    pdf.set_font('Arial','',10)
                    if patient.get('name'):
                        pdf.cell(0,6,self._sanitize_text(f"Patient: {patient.get('name')} | ID: {patient.get('id','')} | Age: {patient.get('age','')} | Sex: {patient.get('gender','')}"),ln=True)
                    pdf.cell(0,6,f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",ln=True)
                    pdf.cell(0,6,"Reported by: Dr. Precious Belema Ibiabuo, Medical Director",ln=True); pdf.ln(5)
                    for line in text.split('\n'):
                        safe_line=self._sanitize_text(line)
                        if not safe_line.strip(): pdf.ln(2); continue
                        pdf.set_font('Arial','',11); pdf.multi_cell(0,6,safe_line.strip()[:100])
                    pdf_fn=f"{fn}.pdf"; pdf.output(pdf_fn)
                    self._open_file(image_path or pdf_fn)
                    msg=f"Professional report saved:\n{os.path.basename(pdf_fn)}"
                    if image_path: msg += f"\n\nImage report:\n{os.path.basename(image_path)}"
                    self._popup("Export Success", msg)
                    self.audit_log("EXPORT_REPORT", f"{title} → {pdf_fn}", patient.get('id',''))
                    return True, pdf_fn
                except Exception as e:
                    print('PDF export error:', e)
            if image_path:
                self._open_file(image_path)
                self._popup("Export Success", f"Image report saved:\n{image_path}")
                return True, image_path
            return False, None
        except Exception as e:
            self._popup("Export Error", str(e)); return False, None
    ...
    def _init_procs(self):
        return [{"name": "Caesarean Section", "price": 500000},
    ...
    def _header(self):
        ...
        h.add_widget(Label(
            text="HMG Hospital v13.2.0",
            font_size=18, bold=True,
            color=get_color_from_hex('#1a5276'),
            size_hint_x=0.17))

        lab_running = sum(1 for s in self.lis_manager.servers.values()
                          if s.get("running"))
        self.lis_lbl = Label(
    ...
    def _dashboard(self):
        ...
        for t, c in [("Patients", "#3498db"), ("Today", "#2ecc71"),
            ...
            lbl = Label(text="0", font_size=26, bold=True,
        ...
        l.add_widget(Label(text="QUICK ACTIONS", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        ...
        for t, c in [("Register", "#27ae60"), ("Clinical AI", "#e67e22"),
                     ...
                     ("Settings", "#7f8c8d")]:
            btn = Button(text=t, font_size=13, bold=True,
        ...
        self.act_log = TextInput(multiline=True, readonly=True, font_size=11,
                                 size_hint_y=None, height=140)
        l.add_widget(Label(text="ACTIVITY", font_size=14, bold=True,
                           size_hint_y=None, height=20))
    ...
    def _register(self):
        ...
        l.add_widget(Label(
            text="PATIENT REGISTRATION", font_size=20, bold=True,
        ...
        fields = [
            ("ID:", TextInput(
                text=f"PT{datetime.now().strftime('%Y%m%d%H%M%S')}",
                font_size=15, size_hint_y=None, height=40)),
            ("Name:", TextInput(font_size=15, size_hint_y=None, height=40)),
            ("DOB:", TextInput(text="1990-01-01", font_size=15,
                                size_hint_y=None, height=40)),
            ("Age:", TextInput(text="0", font_size=15,
                                size_hint_y=None, height=40, readonly=True)),
            ("Gender:", Spinner(text="Select",
                                 values=["Male", "Female", "Other"],
                                 size_hint_y=None, height=40)),
            ("Phone:", TextInput(font_size=15, size_hint_y=None, height=40)),
            ("Height(m):", TextInput(font_size=15, size_hint_y=None, height=40,
                                      hint_text="1.65")),
            ("Blood:", Spinner(
                text="Unknown",
                values=["Unknown", "A+", "A-", "B+", "B-",
                        "AB+", "AB-", "O+", "O-"],
                size_hint_y=None, height=40)),
        ]
        ...
        for lb, w in fields:
            ...
            row.add_widget(Label(
                text=lb, font_size=15, bold=True,
                size_hint_x=0.28, size_hint_y=None, height=44,
                halign='left', valign='middle',
                text_size=(None, None)
            ))
            row.add_widget(w)
            l.add_widget(row)
            self.rw[lb.split(":")[0].lower().replace(" ", "_")
                    .replace("(", "").replace(")", "")] = w
        ...
        def update_age(instance, value):
            try:
                ...
                if (datetime.now().month < dob.month or
                        (datetime.now().month == dob.month and
        ...
        srch = BoxLayout(size_hint_y=None, height=40, spacing=4)
        self.reg_srch = TextInput(
            font_size=14, size_hint_x=0.7, size_hint_y=None, height=40,
            hint_text="Search by name or ID")
        srch.add_widget(self.reg_srch)
        srch.add_widget(Button(
            text="Search", font_size=13,
            size_hint_x=0.15, size_hint_y=None, height=40,
            background_color=get_color_from_hex('#2980b9'),
            background_normal='',
            on_press=lambda x: self._refresh_plist(
                self.reg_srch.text.strip().lower())))
        srch.add_widget(Button(
        ...
        self.plist = BoxLayout(orientation='vertical',
    ...
    def _reg_pt(self, instance):
        ...
        self.audit_log("REGISTER_PATIENT", nm, p["id"])
    ...
    def _refresh_plist(self, filter_text=None):
        ...
        for p in matches:
            ...
            del_btn = Button(
                text="✕", font_size=14, bold=True, size_hint_x=0.15,
                size_hint_y=None, height=38,
                color=(1, 1, 1, 1),
                background_color=get_color_from_hex('#e74c3c'),
                background_normal=''
    ...
    def _confirm_delete_patient(self, pid):
        ...
        if self.delete_attempts[pid] > 3:
            self._popup("Access Denied", "Too many failed attempts.")
            self.audit_log("DELETE_PATIENT_LOCKOUT", f"Patient ID {pid}", pid)
            return

        content = BoxLayout(orientation='vertical', spacing=10, padding=10)
        content.add_widget(Label(text=(f"Enter passcode to delete "
        ...
        content.add_widget(Label(text=f"Patient: {patient.get('name')} ({pid})",
                                 font_size=14, bold=True,
                                 color=get_color_from_hex('#c0392b')))
        content.add_widget(Label(text="This removes all associated records.",
                                 font_size=12))
        ...
        def do_delete(inst):
            if not self._verify_delete_passcode(passcode_input.text):
                ...
                self.audit_log("DELETE_PATIENT_FAILED",
                               patient.get('name'), pid)
                return
            self.delete_attempts[pid] = 0
            self.patients = [p for p in self.patients if p.get("id") != pid]
            self.emr_records = [r for r in self.emr_records
            ...
            self.radiology = [r for r in self.radiology
                              if r.get("patient_id") != pid]
            self.prescriptions = [rx for rx in self.prescriptions
                                  if rx.get("patient_id") != pid]
            self.bills = [b for b in self.bills
                          if b.get("patient", {}).get("id") != pid]
            self.ecg_reports = [e for e in self.ecg_reports
                                if e.get("patient_id") != pid]
            self.followups = [f for f in self.followups
                              if f.get("patient_id") != pid]
            self.daily_notes = [n for n in self.daily_notes
                                if n.get("patient_id") != pid]
            self.consent_forms = [c for c in self.consent_forms
                                  if c.get("patient_id") != pid]
            self.referral_notes = [r for r in self.referral_notes
                                   if r.get("patient_id") != pid]
            self.patient_snapshots = [s for s in self.patient_snapshots
                                       if s.get("patient_id") != pid]
            self.lab_results = [l for l in self.lab_results
                                if l.get("patient_id") != pid]
            ...
            self.admissions = [a for a in self.admissions
            ...
            self._clear_pt()
            pp.dismiss()
            self._popup("Deleted", f"Patient {patient.get('name')} removed.")
            self.log_activity(f"Deleted patient {patient.get('name')} ({pid})")
            self.audit_log("DELETE_PATIENT",
                           f"Deleted {patient.get('name')} with all records", pid)

        btns.add_widget(Button(text="DELETE", on_press=do_delete,
                               background_color=get_color_from_hex('#e74c3c'),
                               background_normal=''))
        btns.add_widget(Button(text="CANCEL", background_normal='',
                               on_press=lambda x: (pp.dismiss(),
                                                    self.delete_attempts.pop(pid, None))))
    ...
    def _sel_by_id(self, pid):
        for p in self.patients:
            if p.get("id") == pid:
                ...
                self.audit_log("SELECT_PATIENT", p.get('name'), pid)
                return
    ...
    def _clear_pt(self):
        ...
        for fn in [self._refresh_timeline, self._refresh_uploaded_results,
                   self._refresh_vitals_timeline, self._refresh_daily_notes,
                   self._refresh_snapshots, self._refresh_saved_items,
                   self._refresh_elab, self._refresh_tasks,
                   self._refresh_pharmacy_dispense,
                   self._refresh_admissions_list,
                   self._refresh_saved_reports_list,
                   self._refresh_appointments,
    ...
    def _upd_tabs(self, p):
        ...
        nm = p.get("name", "")
        mapping = {
            "emr_pid": pid, "emr_name": nm, "bill_pid": pid, "bill_name": nm,
            "pay_pid": pid, "pay_name": nm, "pharm_pid": pid, "pharm_name": nm,
            "lab_pid": pid, "lab_name": nm, "rad_pid": pid, "rad_name": nm,
        ...
        def rf(t=""):
            ...
            for p in self.patients:
                if (not t or t.lower() in p.get("name", "").lower() or
                    ...
                    btn = Button(text=(f"{p.get('name','?')} | "
                                        f"{p.get('id','')} | {p.get('age','')}yr"),
                                 font_size=15, size_hint_y=None, height=44,
                                 bold=True,
    ...
    def _clinical_ai_tab(self):
        ...
        left.add_widget(Label(text="AI CLINICAL ASSISTANT", font_size=20,
                              bold=True, size_hint_y=None, height=32,
        ...
        self.ai_status_label = Label(
            ...
            color=get_color_from_hex('#27ae60') if self.ai.enabled
            else get_color_from_hex('#e74c3c'))
        ai_status.add_widget(self.ai_status_label)
        ai_status.add_widget(Button(text="Test AI", font_size=12, size_hint_x=0.15,
                                     on_press=self._test_ai_connection,
                                     background_color=get_color_from_hex('#2980b9'),
                                     background_normal=''))
        left.add_widget(ai_status)
        self.ca_ctx = Label(text="No patient selected", font_size=13,
                            size_hint_y=None, height=20,
                            color=get_color_from_hex('#7f8c8d'))
        ...
        self.ca_chat = TextInput(multiline=True, readonly=True, font_size=12,
        ...
        self.ca_input = TextInput(size_hint_x=0.6, font_size=15,
                                   size_hint_y=None, height=45,
                                   hint_text="Type response...",
                                   on_text_validate=self._send_ca)
        inp.add_widget(self.ca_input)
        inp.add_widget(Button(text="📷", font_size=16, bold=True, size_hint_x=0.08,
                              on_press=self._ca_camera,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal=''))
        inp.add_widget(Button(text="SEND", font_size=16, bold=True, size_hint_x=0.25,
        ...
        for txt, cb, col in [
            ("START", self._start_ca, '#27ae60'),
            ("RESET", self._reset_ca, '#f39c12'),
            ("RE-ANALYZE", self._re_analyze_ca, '#e67e22'),
            ("ASSESSMENT", self._show_assessment, '#c0392b'),
            ("POPULATE EMR", self._populate_emr, '#8e44ad'),
            ("SCHEDULE", self._schedule_followup, '#16a085'),
            ("AUTO-ASSIGN TASKS", self._auto_assign_tasks_from_ai, '#2c3e50')]:
            ctrl.add_widget(Button(text=txt, font_size=14, bold=True,
                                    on_press=cb, background_normal='',
        ...
        right.add_widget(Label(text="ASSESSMENT", font_size=16, bold=True,
                               size_hint_y=None, height=25,
                               color=get_color_from_hex('#2c3e50')))
        assess_scroll = ScrollView(size_hint_y=0.3, bar_width=10, scroll_type=['bars'])
        self.ca_diag = TextInput(multiline=True, readonly=True, font_size=11,
        ...
        right.add_widget(Label(text="EMR PREVIEW", font_size=16, bold=True,
                               size_hint_y=None, height=25,
                               color=get_color_from_hex('#2c3e50')))
        emr_scroll = ScrollView(size_hint_y=0.5, bar_width=10, scroll_type=['bars'])
        self.ca_emr = TextInput(multiline=True, readonly=True, font_size=11,
        ...
        act.add_widget(Button(text="SAVE TO EMR", font_size=14, bold=True,
                              on_press=self._save_ca_emr,
                              background_color=get_color_from_hex('#27ae60'),
                              background_normal=''))
        act.add_widget(Button(text="ADD TO BILL", font_size=14,
    ...
    def _test_ai_connection(self, instance):
        if not self.ai.enabled:
            self._popup("AI Status", "AI is not enabled. Set API key in Settings.")
            return
        self.ai_status_label.text = "AI Status: Testing..."

        def run_test():
            ok, resp = self.ai.call("You are a helpful assistant.",
                                     "Reply with 'OK' if you can read this.", mt=10)
            if ok:
                Clock.schedule_once(lambda dt: setattr(
                    self.ai_status_label, 'text', "AI Status: ✅ Connected"), 0)
                self._popup("AI Test", "AI is working correctly!")
            else:
                Clock.schedule_once(lambda dt: setattr(
                    self.ai_status_label, 'text',
    ...
    def _ca_camera(self, instance):
        ...
        def capture(inst):
            if cam.texture:
                ...
                if self.cp:
                    self.chat_sys.send(self.cp.get('name', 'User'), "General",
                                        f"[Photo] {fn}", image=fn)
        btns.add_widget(Button(text="📸 CAPTURE", font_size=16, bold=True,
                               on_press=capture,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        btns.add_widget(Button(text="CANCEL", font_size=16,
                               on_press=pp.dismiss, background_normal=''))
        content.add_widget(btns)
        pp.open()
    ...
    def _process_ca(self, msg):
        ...
        pid_at_start = self.ai_patient_id
        Clock.schedule_once(lambda dt: self._update_ca(r, pid_at_start)
                            if r else self._update_ca("Could not process.",
                                                       pid_at_start), 0)

    def _update_ca(self, response, expected_pid=None):
        current_pid = self.cp.get("id") if self.cp else None
        if expected_pid and expected_pid != current_pid:
            self.ca_chat.text += "\n[Warning: Response discarded — patient changed]\n"
            return
        ...
        if self.clinical_ai.done:
            ...
            if emr:
                ...
                if self.cp:
                    pid = self.cp.get("id")
                    self.patient_summaries[pid] = self._format_summary(emr)
                    self.save_all()
                    self._refresh_clinical_summary()

    def _format_summary(self, emr):
        lines = ["AI CLINICAL SUMMARY",
                 f"Chief Complaint: {emr.get('chief_complaint', 'N/A')}",
                 f"History: {emr.get('history_of_presenting_illness', 'N/A')}",
                 f"PMH: {emr.get('past_medical_history', 'N/A')}",
                 f"Medications: {emr.get('medications', 'N/A')}",
                 f"Allergies: {emr.get('allergies', 'N/A')}",
                 "Differentials:"]
        for d in emr.get('differential_diagnosis', []):
            lines.append(f"- {d}")
        lines.append(f"Investigations: "
                     f"{', '.join(emr.get('recommended_investigations', []))}")
        lines.append(f"Treatment: {emr.get('treatment_plan', 'N/A')}")
    ...
    def _show_assessment(self, instance):
        if not self.clinical_ai.done or not self.clinical_ai.emr:
            if self.ai.enabled:
                ...
                self._popup("Assessment", "Complete interview or enable AI.")
            return
        ...
        for i, d in enumerate(emr.get('differential_diagnosis', [])):
            text += f"  {i+1}. {d}\n"
        text += (f"Investigations: "
                 f"{', '.join(emr.get('recommended_investigations', []))}\n")
        text += f"Treatment: {emr.get('treatment_plan','N/A')}\n"
    ...
    def _generate_assessment_ai(self):
        if not self.ai.enabled or not self.cp:
            ...
            return
        conv_text = "\n".join(
            f"{'Doctor' if m['role']=='assistant' else 'Patient'}: {m['content']}"
            for m in self.clinical_ai.conv)
        prompt = (f"Generate a comprehensive SOAP assessment:\n\n{conv_text}\n\n"
                  f"Provide Subjective, Objective, Assessment, Plan.")

        def run():
            ok, resp = self.ai.call(
                "You are a senior clinician providing SOAP assessment.",
                prompt, mt=1500)
            if ok:
                Clock.schedule_once(lambda dt: self._text_popup("AI Assessment", resp), 0)
            else:
        ...
        threading.Thread(
            target=lambda: Clock.schedule_once(
                lambda dt: self._update_ca(
                    self.clinical_ai.re_analyze() or "Re-analysis failed."), 0),
            daemon=True).start()
    ...
    def _add_ca_bill(self, instance):
        ...
        self.bill_items.append({"name": "AI Consultation", "cat": "Charges",
                                 "qty": 1, "price": 7500, "total": 7500})
        self._refresh_bill()
    ...
    def _schedule_followup(self, instance):
        ...
        date_inp = TextInput(text=datetime.now().strftime("%Y-%m-%d"),
                              font_size=15, size_hint_y=None, height=40)
        c.add_widget(date_inp)
        c.add_widget(Label(text="Description:", font_size=15, bold=True))
        desc_inp = TextInput(font_size=15, size_hint_y=None, height=40,
        ...
        def save_fu(inst):
            ...
            try:
                ...
                self._popup("Error", "Invalid date format.")
                return
            self.followups.append({"patient_id": self.cp.get("id"),
            ...
            if self.cp:
                self.task_manager.create_task(
                    ...
                    assigned_to="Doctor", task_type="Follow-up",
                    description=f"Follow-up: {desc} on {date_str}",
                    department="Medical")
                self._refresh_tasks()

        btns.add_widget(Button(text="SAVE", font_size=16, bold=True,
                               on_press=save_fu,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        btns.add_widget(Button(text="CANCEL", font_size=16,
    ...
    def _refresh_timeline(self):
        ...
        if events:
            ...
            self.timeline_text.text = f"No events for {self.cp.get('name', '')}."
...
# ====================== END OF PART 3 ======================
    ...
    def _clinical_summary_tab(self):
        ...
        main.add_widget(Label(text="COMPREHENSIVE CLINICAL ASSESSMENT",
                              font_size=18, bold=True, size_hint_y=None, height=30,
                              color=get_color_from_hex('#e67e22')))
        scroll = ScrollView(size_hint_y=0.45, bar_width=10, scroll_type=['bars'])
        self.cl_summary = TextInput(multiline=True, readonly=True, font_size=13,
                                     background_color=get_color_from_hex('#fef9e7'),
                                     size_hint_y=None)
        self.cl_summary.bind(minimum_height=self.cl_summary.setter('height'))
        scroll.add_widget(self.cl_summary)
        main.add_widget(scroll)
        main.add_widget(Label(text="TREATMENT SUGGESTIONS", font_size=16, bold=True,
                              size_hint_y=None, height=24,
                              color=get_color_from_hex('#2980b9')))
        t_scroll = ScrollView(size_hint_y=0.2, bar_width=10, scroll_type=['bars'])
        self.treatment_suggestions = TextInput(
            multiline=True, readonly=True, font_size=13,
            background_color=get_color_from_hex('#eaf2f8'), size_hint_y=None)
        self.treatment_suggestions.bind(
            minimum_height=self.treatment_suggestions.setter('height'))
        t_scroll.add_widget(self.treatment_suggestions)
        main.add_widget(t_scroll)
        row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row.add_widget(Button(text="GENERATE TREATMENT SUGGESTIONS", font_size=14,
                              bold=True, on_press=self._generate_treatment_suggestions,
                              background_color=get_color_from_hex('#8e44ad'),
                              background_normal=''))
        row.add_widget(Button(text="EXPORT PDF", font_size=14, bold=True,
                              size_hint_x=0.2,
                              on_press=lambda x: self._export_to_pdf(
                                  self.cl_summary.text, "Clinical_Summary",
                                  "Clinical Assessment Report"),
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal=''))
        main.add_widget(row)
        main.add_widget(Label(text="PATIENT TIMELINE", font_size=16, bold=True,
                              size_hint_y=None, height=24,
                              color=get_color_from_hex('#2980b9')))
        tl_scroll = ScrollView(size_hint_y=0.25, bar_width=10, scroll_type=['bars'])
        self.timeline_text = TextInput(multiline=True, readonly=True, font_size=12,
                                        background_color=get_color_from_hex('#f0f8ff'),
                                        size_hint_y=None)
        self.timeline_text.bind(minimum_height=self.timeline_text.setter('height'))
        tl_scroll.add_widget(self.timeline_text)
        main.add_widget(tl_scroll)
        tab.add_widget(main)
        self.tp.add_widget(tab)
        self._refresh_clinical_summary()
    ...
    def _refresh_clinical_summary(self):
        ...
        emrs = [r for r in self.emr_records if r.get("patient_id") == pid]
        if not emrs:
            self.cl_summary.text = f"No records for {self.cp.get('name','')}."
            return
        latest = emrs[-1]
        summary = (f"CLINICAL SUMMARY - {self.cp.get('name','')}\n" +
                   "=" * 50 + "\n\n" +
                   f"Patient ID: {pid}\n" +
                   f"Age: {self.cp.get('age', 'N/A')} years\n" +
                   f"Gender: {self.cp.get('gender', 'N/A')}\n" +
                   f"Blood Type: {self.cp.get('blood', 'N/A')}\n" +
                   f"Last Visit: {latest.get('ts', 'N/A')}\n" +
                   f"Department: {latest.get('dept', 'N/A')}\n\n")
        summary += "VITAL SIGNS\n"
        for k, lb in [("bp", "BP"), ("pulse", "Pulse"), ("temp", "Temp"),
                      ("spo2", "SpO2"), ("rr", "RR"), ("avpu", "AVPU")]:
            summary += f"  {lb}: {latest.get(k, 'N/A')}\n"
        summary += "\nCLINICAL DETAILS\n"
        label_map = {"cc": "Chief Complaint", "hpi": "HPI", "pmh": "PMH",
                     "psh": "Past Surgical History", "dh": "Drug History",
                     "all": "Allergies", "fh": "Family History",
                     "sh": "Social History", "ros": "Review of Systems",
                     "gen": "General Exam", "cvs": "CVS", "resp": "Resp",
                     "abd": "Abd", "cns": "CNS", "dx": "Diagnosis",
                     "tx": "Treatment Plan", "fu": "Follow-up",
        ...
        for k, v in latest.items():
            if k in skip or not v or not str(v).strip():
                continue
            lb = label_map.get(k, k.upper().replace("_", " "))
            summary += f"  {lb}: {v}\n"
        structured = latest.get("_structured_drugs")
        if structured:
            summary += "\nPRESCRIBED DRUGS\n"
            for drug in structured:
                summary += (f"  {drug.get('name','')} — {drug.get('total',0)} "
                            f"units over {drug.get('days',0)} days\n")
        tasks = self.task_manager.get_tasks_for_patient(pid)
        active = [t for t in tasks if t.get("status") != "Completed"]
        if active:
            summary += "\nACTIVE TASKS\n"
            for t in active:
                summary += (f"  - [{t.get('status')}] {t.get('task_type')}: "
    ...
    def _generate_treatment_suggestions(self, instance):
        ...
        if emrs:
            latest = emrs[-1]
            context += f"Diagnosis: {latest.get('dx', 'N/A')}\n"
            context += f"Treatment: {latest.get('tx', 'N/A')}\n"
        if labs:
            context += "\nRecent Labs:\n"
            for lab in labs[-5:]:
                context += f"- {lab.get('test_name','')}: {lab.get('result','')}\n"
        if rx:
            context += "\nMedications:\n"
            for med in rx[-5:]:
                context += f"- {med.get('drug','')} x{med.get('qty',0)}\n"
        if not self.ai.enabled:
            self.treatment_suggestions.text = self._generate_fallback_treatment_suggestions(context)
            self._popup("Treatment Suggestions", "Generated using local knowledge.")
            return
        prompt = f"Provide comprehensive treatment suggestions:\n\n{context}"
        self.treatment_suggestions.text = "Generating..."
        ...
        def run():
            ...
            if ok:
                Clock.schedule_once(lambda dt: setattr(
                    self.treatment_suggestions, 'text', resp), 0)
            else:
                fallback = self._generate_fallback_treatment_suggestions(context)
                Clock.schedule_once(lambda dt: setattr(
                    self.treatment_suggestions, 'text', fallback), 0)
        threading.Thread(target=run, daemon=True).start()
...
- CBC, CMP, additional tests as indicated

4. LIFESTYLE MODIFICATIONS
- Regular exercise, healthy diet, hydration

5. FOLLOW-UP
- Review in 2 weeks, earlier if symptoms worsen

6. RED FLAGS
- Worsening symptoms, new onset symptoms

7. REFERRALS
- Specialist referral if indicated
"""

    # ====================== EMR (FIXED COLLAPSIBLE CLERKING) ======================
    def _emr(self):
        ...
        self.emr_dept = Spinner(text="General Medicine", values=self.depts,
                                size_hint_x=0.18, size_hint_y=None, height=40)
        self.emr_dept.bind(text=self._on_dept_change)
        bar.add_widget(self.emr_dept)
        bar.add_widget(Label(text="Admission:", font_size=13, bold=True,
                              size_hint_x=0.08))
        self.emr_admission_spinner = Spinner(text="None", values=["None"],
                                              size_hint_x=0.2,
                                              size_hint_y=None, height=40)
        bar.add_widget(self.emr_admission_spinner)
        bar.add_widget(Button(text="AI", font_size=13, size_hint_x=0.06,
                              on_press=lambda x: self._switch("Clinical AI"),
                              background_color=get_color_from_hex('#e67e22'),
                              background_normal=''))
        bar.add_widget(Button(text="VIEW HISTORY", font_size=13, size_hint_x=0.12,
                              on_press=self._view_emr_history,
                              background_color=get_color_from_hex('#16a085'),
                              background_normal=''))
        bar.add_widget(Button(text="AUTO BILL", font_size=13, size_hint_x=0.12,
                              on_press=self._auto_bill_from_emr,
                              background_color=get_color_from_hex('#27ae60'),
                              background_normal=''))
        bar.add_widget(Button(text="AUTO TASKS", font_size=13, size_hint_x=0.12,
                              on_press=self._auto_assign_tasks_from_current_emr,
                              background_color=get_color_from_hex('#8e44ad'),
                              background_normal=''))
        l.add_widget(bar)

        # ---- Collapsible clerking container ----
        self.dept_fields = BoxLayout(orientation='vertical',
        ...
        # ---- Vitals ----
        l.add_widget(Label(text="VITAL SIGNS", font_size=17, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2980b9')))
        ...
        # ---- Prescribed drugs with search ----
        l.add_widget(Label(text="PRESCRIBED DRUGS (search, then add)",
                           font_size=14, bold=True, size_hint_y=None, height=20,
                           color=get_color_from_hex('#2980b9')))
        search_row = BoxLayout(size_hint_y=None, height=36, spacing=4)
        search_row.add_widget(Label(text="🔍 Search:", font_size=13, bold=True,
                                     size_hint_x=0.12))
        self.emr_drug_search = TextInput(
            font_size=13, size_hint_x=0.78, size_hint_y=None, height=36,
            hint_text="Type to filter drugs (e.g. 'para', 'amox', 'insulin')",
            multiline=False)
        self.emr_drug_search.bind(text=self._on_drug_search_change)
        search_row.add_widget(self.emr_drug_search)
        search_row.add_widget(Button(text="Clear", font_size=12, size_hint_x=0.10,
                                      size_hint_y=None, height=36,
                                      on_press=lambda x: setattr(
                                          self.emr_drug_search, 'text', ''),
                                      background_color=get_color_from_hex('#95a5a6'),
                                      background_normal=''))
        l.add_widget(search_row)

        drug_add_box = BoxLayout(size_hint_y=None, height=40, spacing=4)
        drug_add_box.add_widget(Label(text="Drug:", font_size=13, bold=True,
                                       size_hint_x=0.06))
        self.emr_drug_spinner = Spinner(text="Select",
                                         values=[d['name'] for d in self.drugs],
                                         size_hint_x=0.34,
                                         size_hint_y=None, height=35)
        drug_add_box.add_widget(self.emr_drug_spinner)
        drug_add_box.add_widget(Label(text="Qty/day:", font_size=13, bold=True,
                                       size_hint_x=0.08))
        self.emr_drug_qty = TextInput(text="1", size_hint_x=0.08, font_size=13,
                                       size_hint_y=None, height=35)
        drug_add_box.add_widget(self.emr_drug_qty)
        drug_add_box.add_widget(Label(text="Days:", font_size=13, bold=True,
                                       size_hint_x=0.06))
        self.emr_drug_days = TextInput(text="1", size_hint_x=0.08, font_size=13,
                                        size_hint_y=None, height=35)
        drug_add_box.add_widget(self.emr_drug_days)
        drug_add_box.add_widget(Button(text="ADD", font_size=13, bold=True,
                                        size_hint_x=0.10,
        ...
        self.drug_display = BoxLayout(orientation='vertical',
        ...
        btns.add_widget(Button(text="SAVE EMR", font_size=17, bold=True,
                               on_press=self._save_emr,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        btns.add_widget(Button(text="CLEAR", font_size=15,
    ...
    def _build_dept_fields(self, department):
        """Rebuild collapsible clerking sections (FIXED version)."""
        # Purge old dept-specific widgets
        ...
        def make_section(title, start_open):
            sec = CollapsibleSection(
                title,
                start_open=start_open,
                on_height_change=self._recalc_dept_fields_height)
            self.dept_fields.add_widget(sec)
            return sec

        # ---- HISTORY ----
        sec_hist = make_section("HISTORY", start_open=True)
        self._add_fields_to_section(sec_hist, [
        ...
        self._dept_sections["history"] = sec_hist

        # ---- EXAMINATION ----
        sec_exam = make_section("EXAMINATION", start_open=False)
        self._add_fields_to_section(sec_exam, [
        ...
        self._dept_sections["examination"] = sec_exam

        # ---- ASSESSMENT & PLAN ----
        sec_plan = make_section("ASSESSMENT & PLAN", start_open=False)
        self._add_fields_to_section(sec_plan, [
            ("Diagnosis / Impression", "dx", 2),
            ("Treatment Plan", "tx", 2),
            ("Follow-up", "fu", 1),
            ("Nursing Orders", "nursing_orders", 2),
        ])
        self._dept_sections["plan"] = sec_plan
        ...
        extra_fields = extras.get(department, [])
        if extra_fields:
            sec_extra = make_section(f"{department.upper()} SPECIFIC",
                                     start_open=False)
            self._add_fields_to_section(sec_extra, extra_fields)
            self._dept_sections["extras"] = sec_extra

        # Force one recalculation after everything is built
        Clock.schedule_once(lambda dt: self._recalc_dept_fields_height(), 0)

    def _add_fields_to_section(self, section, fields):
        for label, key, lines in fields:
            section.body.add_widget(Label(text=label + ":", font_size=13,
                                           bold=True, size_hint_y=None, height=18))
            w = TextInput(multiline=True, font_size=13,
                          size_hint_y=None, height=30 * lines)
            self.ew[key] = w
            self._dept_field_keys.append(key)
            section.body.add_widget(w)

    def _recalc_dept_fields_height(self):
        """Manually recompute dept_fields height (Kivy's minimum_height
        binding only watches children list, not child heights)."""
        if not hasattr(self, 'dept_fields'):
            return
        h = 0
        n = 0
        for child in self.dept_fields.children:
            if child.size_hint_y is None:
                h += child.height
                n += 1
        if n > 1:
            h += self.dept_fields.spacing * (n - 1)
        self.dept_fields.height = h

    def _build_common_fields(self):
        self.common_fields.clear_widgets()
        self.common_fields.add_widget(Label(text="ADDITIONAL NOTES", font_size=14,
                                             bold=True, size_hint_y=None, height=20,
                                             color=get_color_from_hex('#2980b9')))
        common = [("Clinical Notes", "notes", 2),
        ...
        for label, key, lines in common:
            self.common_fields.add_widget(Label(text=label + ":", font_size=13,
                                                 bold=True,
    ...
    def _on_drug_search_change(self, instance, query):
        q = query.strip().lower()
        if not q:
            matches = [d['name'] for d in self.drugs]
        else:
            matches = [d['name'] for d in self.drugs
                       if q in d['name'].lower() or q in d.get('cat', '').lower()]
        self.emr_drug_spinner.values = matches
        if not matches:
            self.emr_drug_spinner.text = "No match"
        elif self.emr_drug_spinner.text not in matches:
            self.emr_drug_spinner.text = matches[0]

    def _on_dispense_drug_search_change(self, instance, query):
        q = query.strip().lower()
        if not q:
            matches = [d['name'] for d in self.drugs]
        else:
            matches = [d['name'] for d in self.drugs
                       if q in d['name'].lower() or q in d.get('cat', '').lower()]
        self.dispense_drug.values = matches
        if not matches:
            self.dispense_drug.text = "No match"
        elif self.dispense_drug.text not in matches:
            self.dispense_drug.text = matches[0]

    def _on_pharm_drug_search_change(self, instance, query):
        q = query.strip().lower()
        if not q:
            matches = [d['name'] for d in self.drugs]
        else:
            matches = [d['name'] for d in self.drugs
                       if q in d['name'].lower() or q in d.get('cat', '').lower()]
        self.pharm_drug.values = matches
        if not matches:
            self.pharm_drug.text = "No match"
        elif self.pharm_drug.text not in matches:
            self.pharm_drug.text = matches[0]

    def _add_drug_to_emr(self, instance):
        drug_name = self.emr_drug_spinner.text
        if drug_name in ("Select", "No match"):
        ...
        try:
            ...
            self._popup("Error", "Qty/day and days must be positive integers.")
            return
        ...
        if stock_available < total_qty:
            self._popup("Stock Warning",
                        f"Insufficient stock. Available: {stock_available}, "
                        f"Requested: {total_qty}\nYou can still prescribe.")
        self.emr_drug_list.append({
            "name": drug_name, "qty_per_day": qty_per_day,
            "days": days, "total": total_qty, "stock_checked": stock_available})
        self._refresh_drug_display()
        ...
        if hasattr(self, 'emr_drug_search'):
            self.emr_drug_search.text = ""

    def _refresh_drug_display(self):
        if not hasattr(self, 'drug_display'):
            return
        self.drug_display.clear_widgets()
        if not self.emr_drug_list:
            self.drug_display.add_widget(Label(text="No drugs added yet.",
                                                font_size=12,
                                                size_hint_y=None, height=25))
            return
        for i, drug in enumerate(self.emr_drug_list):
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            row.add_widget(Label(text=f"{drug['name']}", font_size=12,
                                  size_hint_x=0.3))
            row.add_widget(Label(text=f"Total: {drug['total']}", font_size=12,
                                  size_hint_x=0.15, bold=True,
                                  color=get_color_from_hex('#2980b9')))
            row.add_widget(Label(text=f"Days: {drug['days']}", font_size=12,
                                  size_hint_x=0.1))
            if drug.get('stock_checked', 0) < drug.get('total', 0):
                row.add_widget(Label(text="⚠️ Low Stock", font_size=11,
                                      size_hint_x=0.12,
                                      color=get_color_from_hex('#e74c3c')))
            btn_remove = Button(text="X", font_size=11, size_hint_x=0.08,
                                size_hint_y=None, height=25,
                                background_color=get_color_from_hex('#e74c3c'),
                                background_normal='')
            btn_remove.idx = i
    ...
    def _save_emr(self, instance):
        ...
        r = {"emr_id": f"EMR{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
             "patient_id": pid, "patient_name": name,
             "dept": self.emr_dept.text,
             "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
             "admission_id": adm_id}
        active_keys = (list(self._vitals_keys) +
        ...
        self.audit_log("SAVE_EMR",
                       f"Dept: {self.emr_dept.text} | EMR ID: {r['emr_id']}", pid)
        self._auto_assign_tasks_from_emr(r)
    ...
    def _view_emr_history(self, instance):
        ...
        emrs = [r for r in self.emr_records if r.get("patient_id") == pid]
        if not emrs:
            self._popup("EMR History", "No records.")
            return
        c = BoxLayout(orientation='vertical', spacing=5, padding=8)
        pl = BoxLayout(orientation='vertical', size_hint_y=None, spacing=1)
        pl.bind(minimum_height=pl.setter('height'))
    ...
    def _view_emr_detail(self, record):
        text = (f"EMR ID: {record.get('emr_id','')}\n"
                f"Date: {record.get('ts','')}\n"
                f"Department: {record.get('dept','')}\n"
                f"Admission ID: {record.get('admission_id', 'None')}\n\n"
                "Vital Signs:\n")
        for k in ["bp", "temp", "pulse", "spo2", "rr", "wt", "ht",
                  "bmi", "liver_span", "avpu"]:
            if record.get(k):
    ...
    def _admissions_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="PATIENT ADMISSIONS", font_size=20, bold=True,
        ...
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.adm_name = TextInput(size_hint_x=0.3, readonly=True, font_size=15,
                                   size_hint_y=None, height=40)
        bar.add_widget(self.adm_name)
        bar.add_widget(Button(text="REFRESH", font_size=13, size_hint_x=0.12,
                              on_press=lambda x: self._refresh_admissions_list(),
                              background_color=get_color_from_hex('#16a085'),
                              background_normal=''))
        l.add_widget(bar)
        l.add_widget(Label(text="NEW ADMISSION", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#e67e22')))
        ...
        self.adm_date = TextInput(text=datetime.now().strftime("%Y-%m-%d"),
        ...
        self.adm_discharge = TextInput(text="", font_size=14, size_hint_y=None,
                                        height=35, hint_text="(optional)")
        form.add_widget(self.adm_discharge)
        form.add_widget(Label(text="Department:", font_size=14, bold=True))
        self.adm_dept = Spinner(text="General Medicine", values=self.depts,
                                size_hint_y=None, height=35)
        ...
        l.add_widget(form)
        l.add_widget(Button(text="ADD ADMISSION", font_size=16, bold=True,
                            size_hint_y=None, height=40,
                            on_press=self._add_admission,
                            background_color=get_color_from_hex('#27ae60'),
                            background_normal=''))
        l.add_widget(Label(text="ADMISSION HISTORY", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.adm_list = BoxLayout(orientation='vertical',
                                   size_hint_y=None, spacing=1)
    ...
    def _refresh_admissions_list(self):
        ...
        if not self.adm_pid.text:
            self.adm_list.add_widget(Label(text="Select a patient.",
        ...
        adm.sort(key=lambda x: x.get("admission_date", ""), reverse=True)
        if not adm:
            self.adm_list.add_widget(Label(text="No admissions.",
                                            font_size=13, size_hint_y=None,
                                            height=30))
            return
        for a in adm:
            ...
            btn_del = Button(text="X", font_size=11, size_hint_x=0.08,
                             size_hint_y=None, height=25,
                             background_color=get_color_from_hex('#e74c3c'),
                             background_normal='')
            ...
            btn_set = Button(text="Set Current", font_size=11, size_hint_x=0.12,
                             size_hint_y=None, height=25,
                             background_color=get_color_from_hex('#2980b9'),
                             background_normal='')
            btn_set.adm = a
    ...
    def _add_admission(self, instance):
        ...
        admission = {
            ...
            "discharge_date": self.adm_discharge.text.strip(),
        ...
        self._popup("Added", f"Admission for {self.adm_name.text} on {adm_date}")
        self.audit_log("ADD_ADMISSION", diagnosis, self.adm_pid.text)
    ...
    def _delete_admission(self, adm_id):
        ...
        self.save_all()
        self._refresh_admissions_list()
        self._popup("Deleted", "Admission removed.")
        self.audit_log("DELETE_ADMISSION", adm_id)

    # ====================== TASKS ======================
    def _task_management_tab(self):
        tab = TabbedPanelItem(text="Tasks")
        main = BoxLayout(orientation='vertical', padding=6, spacing=4)
        main.add_widget(Label(text="TASK MANAGEMENT", font_size=20, bold=True,
                              size_hint_y=None, height=32,
                              color=get_color_from_hex('#8e44ad')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.task_patient_name = TextInput(size_hint_x=0.3, readonly=True,
                                            font_size=15,
                                            size_hint_y=None, height=40)
        bar.add_widget(self.task_patient_name)
        bar.add_widget(Button(text="VIEW ALL TASKS", font_size=13, size_hint_x=0.15,
                              on_press=self._view_all_tasks,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal=''))
        main.add_widget(bar)
        task_form = BoxLayout(size_hint_y=None, height=40, spacing=4)
        task_form.add_widget(Label(text="Assign To:", font_size=14, bold=True,
                                    size_hint_x=0.1))
        ...
        task_form.add_widget(Label(text="Type:", font_size=14, bold=True,
                                    size_hint_x=0.06))
        ...
        task_form.add_widget(Label(text="Priority:", font_size=14, bold=True,
                                    size_hint_x=0.08))
        self.task_priority = Spinner(text="Normal", values=["High", "Normal", "Low"],
                                      size_hint_x=0.1, size_hint_y=None, height=40)
        task_form.add_widget(self.task_priority)
        task_form.add_widget(Button(text="CREATE TASK", font_size=14, bold=True,
                                     size_hint_x=0.12,
        ...
        main.add_widget(Label(text="Task Description:", font_size=14, bold=True,
                               size_hint_y=None, height=20))
        self.task_desc = TextInput(font_size=13, size_hint_y=None, height=60,
                                    multiline=True,
                                    hint_text="Enter task description...")
        main.add_widget(self.task_desc)
        filter_box = BoxLayout(size_hint_y=None, height=35, spacing=4)
        filter_box.add_widget(Label(text="Filter:", font_size=13, bold=True,
                                     size_hint_x=0.06))
        self.task_filter = Spinner(text="All",
        ...
        filter_box.add_widget(Button(text="REFRESH", font_size=12, size_hint_x=0.1,
                                      size_hint_y=None, height=32,
                                      on_press=lambda x: self._refresh_tasks(),
                                      background_color=get_color_from_hex('#16a085'),
                                      background_normal=''))
        main.add_widget(filter_box)
        main.add_widget(Label(text="TASK LIST", font_size=16, bold=True,
                               size_hint_y=None, height=24,
                               color=get_color_from_hex('#2c3e50')))
    ...
    def _create_manual_task(self, instance):
        ...
        task = self.task_manager.create_task(
            patient_id=self.cp.get("id"), patient_name=self.cp.get("name"),
            assigned_to=self.task_role.text, task_type=self.task_type.text,
            description=desc, priority=self.task_priority.text)
        self._refresh_tasks()
        self.task_desc.text = ""
    ...
    def _auto_assign_tasks_from_emr(self, emr_record):
        ...
        if tasks:
            self._refresh_tasks()
            self._popup("Tasks Created", f"{len(tasks)} tasks assigned")
    ...
    def _auto_assign_tasks_from_current_emr(self, instance):
        ...
        if self.emr_drug_list:
            emr_record["prescribed_drugs"] = "\n".join(
                d["name"] for d in self.emr_drug_list)
            emr_record["drug_quantities"] = "\n".join(
                str(d["total"]) for d in self.emr_drug_list)
    ...
    def _refresh_tasks(self):
        ...
        if not self.cp:
            ...
            tasks = self.task_manager.get_tasks_for_patient(self.cp.get("id"))
        if filter_status != "All":
            tasks = [t for t in tasks if t.get("status") == filter_status]
        if not tasks:
            self.task_list.add_widget(Label(text="No tasks found.", font_size=13,
                                             size_hint_y=None, height=30))
            return
        for task in tasks:
            status_color = {"Pending": "#f39c12", "In Progress": "#2980b9",
                            "Completed": "#27ae60", "Waiting": "#e67e22",
                            "On Hold": "#e74c3c", "Cancelled": "#95a5a6"}.get(
                                task.get("status"), "#7f8c8d")
            row = BoxLayout(size_hint_y=None, height=35, spacing=3)
            row.add_widget(Label(text=task.get("id", "")[:12], font_size=11,
                                  size_hint_x=0.1))
            row.add_widget(Label(text=task.get("task_type", ""), font_size=12,
                                  size_hint_x=0.1))
            row.add_widget(Label(text=task.get("assigned_to", ""), font_size=12,
                                  size_hint_x=0.1))
            row.add_widget(Label(text=task.get("description", "")[:25],
                                  font_size=12, size_hint_x=0.25))
            row.add_widget(Label(text=task.get("status", ""), font_size=12,
                                  size_hint_x=0.1, bold=True,
                                  color=get_color_from_hex(status_color)))
            row.add_widget(Label(text=task.get("created_at", "")[:16],
                                  font_size=10, size_hint_x=0.1))
            status_spinner = Spinner(text=task.get("status", "Pending"),
                                      values=TaskManager.STATUSES,
                                      size_hint_x=0.1, size_hint_y=None, height=30)
            status_spinner.task_id = task.get("id")
            status_spinner.bind(text=lambda s, tid=task.get("id"):
                                self._update_task_status(tid, s.text))
            row.add_widget(status_spinner)
            btn_view = Button(text="View", font_size=11, size_hint_x=0.08,
    ...
    def _view_task_detail(self, task):
        text = (f"TASK DETAIL\n{'=' * 50}\n\n"
                f"ID: {task.get('id')}\n"
                f"Patient: {task.get('patient_name')} ({task.get('patient_id')})\n"
                f"Assigned To: {task.get('assigned_to')}\n"
                f"Type: {task.get('task_type')}\n"
                f"Status: {task.get('status')}\n"
                f"Priority: {task.get('priority')}\n"
                f"Department: {task.get('department', 'N/A')}\n"
    ...
    def _auto_bill_from_emr(self, instance):
        if not self.cp:
            self._popup("Error", "No patient selected.")
            return
        emrs = [r for r in self.emr_records
                if r.get("patient_id") == self.cp.get("id")]
        if not emrs:
            self._popup("Info", "No EMR records.")
            return
        self._auto_bill_from_emr_record(emrs[-1])
    ...
    def _find_price_in_db(self, name, *lists):
        ...
        for lst in lists:
            ...
            for item in lst:
                iname = str(item.get('name', '')).strip().lower()
                if iname and (target in iname or iname in target):
                    return item.get('price')
        for lst in lists:
            if not lst:
                continue
    ...
    def _auto_bill_from_emr_record(self, emr_record):
        ...
        for test in str(emr_record.get("lab_orders", "")).split('\n'):
            test = test.strip()
            if not test:
                continue
            price = self._find_price_in_db(test, self.investigations,
                                            self.charges, self.procedures)
            if price is None:
                missing_prices.append(f"Lab: {test}")
                price = 0
            self.bill_items.append({"name": f"Lab: {test}",
                                     "cat": "Investigations",
                                     "qty": 1, "price": price, "total": price})
            bill_items_added.append(f"Lab: {test} (₦{price:,.2f})")

        for study in str(emr_record.get("radiology_orders", "")).split('\n'):
            study = study.strip()
            if not study:
                continue
            price = self._find_price_in_db(study, self.investigations,
                                            self.charges, self.procedures)
            if price is None:
                missing_prices.append(f"Radiology: {study}")
                price = 0
            self.bill_items.append({"name": f"Radiology: {study}",
                                     "cat": "Investigations",
                                     "qty": 1, "price": price, "total": price})
            bill_items_added.append(f"Radiology: {study} (₦{price:,.2f})")

        structured = emr_record.get("_structured_drugs")
        if structured:
            for drug in structured:
                ...
                if not drug_name or total_qty <= 0:
                    continue
                price = self._find_price_in_db(drug_name, self.drugs,
                                                self.investigations,
                                                self.charges)
                if price is None:
                ...
                self.bill_items.append({
                    "name": f"Drug: {drug_name}", "cat": "Drugs",
                    "qty": total_qty, "price": price,
        ...
        if nursing and str(nursing).strip():
            price = self._find_price_in_db("Nursing Care", self.charges,
                                            self.procedures)
            if price is not None:
                self.bill_items.append({"name": "Nursing Care",
                                         "cat": "Charges", "qty": 1,
        ...
        self.bill_items.append({"name": "Consultation", "cat": "Charges",
                                 "qty": 1, "price": price, "total": price})
        bill_items_added.append(f"Consultation (₦{price:,.2f})")

        if bill_items_added:
            self._refresh_bill()
            msg = (f"Added {len(bill_items_added)} items:\n" +
                   "\n".join(bill_items_added))
            if missing_prices:
                msg += (f"\n\n⚠️ No price for {len(missing_prices)} item(s) — "
                        f"added at ₦0:\n")
                msg += "\n".join(f"  • {m}" for m in missing_prices[:10])
            self._popup("Auto-Bill (Real Prices)", msg)
            self.audit_log("AUTO_BILL",
                           f"{len(bill_items_added)} items, "
                           f"{len(missing_prices)} missing", pid)
            if missing_prices:
                self.task_manager.create_task(
                    patient_id=pid, patient_name=self.cp.get("name"),
                    assigned_to="Administrator", task_type="Administrative",
                    description=(f"Set prices for {len(missing_prices)} items: " +
    ...
    def _pharmacy_dispense_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="PHARMACY DISPENSING", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#d35400')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
                              size_hint_x=0.12))
        self.dispense_pid = TextInput(size_hint_x=0.2, readonly=True,
                                       font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.dispense_pid)
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.dispense_name = TextInput(size_hint_x=0.3, readonly=True,
                                        font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.dispense_name)
        bar.add_widget(Button(text="VIEW PRESCRIPTIONS", font_size=13,
                              size_hint_x=0.15,
        ...
        search_row = BoxLayout(size_hint_y=None, height=36, spacing=4)
        search_row.add_widget(Label(text="🔍 Search:", font_size=13,
                                     bold=True, size_hint_x=0.1))
        self.dispense_drug_search = TextInput(
            font_size=13, size_hint_x=0.75, size_hint_y=None, height=36,
            hint_text="Type to filter drugs (name or category)...",
            multiline=False)
        self.dispense_drug_search.bind(text=self._on_dispense_drug_search_change)
        search_row.add_widget(self.dispense_drug_search)
        search_row.add_widget(Button(text="Clear", font_size=12, size_hint_x=0.1,
                                      size_hint_y=None, height=36,
                                      on_press=lambda x: setattr(
        ...
        dispense_form.add_widget(Label(text="Drug:", font_size=15, bold=True,
                                        size_hint_x=0.06))
        self.dispense_drug = Spinner(text="Select",
                                      values=[d['name'] for d in self.drugs],
                                      size_hint_x=0.3, size_hint_y=None, height=40)
        dispense_form.add_widget(self.dispense_drug)
        dispense_form.add_widget(Label(text="Qty:", font_size=15, bold=True,
                                        size_hint_x=0.05))
        self.dispense_qty = TextInput(text="1", size_hint_x=0.08, font_size=15,
                                       size_hint_y=None, height=40)
        dispense_form.add_widget(self.dispense_qty)
        dispense_form.add_widget(Button(text="DISPENSE", font_size=15, bold=True,
                                         size_hint_x=0.12,
                                         on_press=self._dispense_from_pharmacy,
                                         background_color=get_color_from_hex('#27ae60'),
                                         background_normal=''))
        dispense_form.add_widget(Button(text="AUTO FROM EMR", font_size=15,
        ...
        l.add_widget(Label(text="TASK ASSIGNMENTS", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.dispense_task_list = BoxLayout(orientation='vertical',
                                              size_hint_y=None, spacing=1)
        self.dispense_task_list.bind(
            minimum_height=self.dispense_task_list.setter('height'))
    ...
    def _refresh_pharmacy_dispense(self):
        ...
        if not self.dispense_pid.text:
            self.dispense_task_list.add_widget(Label(
                text="Select a patient.", font_size=13,
                size_hint_y=None, height=30))
            return
        pid = self.dispense_pid.text
        tasks = [t for t in self.tasks if t.get("patient_id") == pid
                 and t.get("task_type") == "Pharmacy"
                 and t.get("status") != "Completed"]
        if not tasks:
            self.dispense_task_list.add_widget(Label(
                text="No pending pharmacy tasks.", font_size=13,
                size_hint_y=None, height=30))
            return
        for task in tasks:
            row = BoxLayout(size_hint_y=None, height=35, spacing=3)
            ...
            row.add_widget(Label(text=task.get("status", ""), font_size=12,
                                  size_hint_x=0.1))
            btn = Button(text="DISPENSE", font_size=12, size_hint_x=0.15,
                          size_hint_y=None, height=30,
                          background_color=get_color_from_hex('#27ae60'),
                          background_normal='')
            btn.task = task
            ...
            row.add_widget(btn)
            btn_c = Button(text="Complete", font_size=12, size_hint_x=0.15,
                            size_hint_y=None, height=30,
                            background_color=get_color_from_hex('#2980b9'),
                            background_normal='')
            btn_c.task = task
            btn_c.bind(on_press=lambda x: self._complete_pharmacy_task(x.task))
            row.add_widget(btn_c)
    ...
    def _dispense_from_pharmacy(self, instance):
        ...
        task = self.task_manager.create_task(
            ...
            order_items=[drug_name], department="Pharmacy")
    ...
    def _dispense_from_task(self, instance):
        ...
        m = re.search(r'Dispense:?\s*(.+?)(?:\s*$|\s+x)', desc, re.IGNORECASE)
        if not m:
            m = re.search(r'Drug:?\s*(.+?)(?:\s*$|\s+x)', desc, re.IGNORECASE)
        if m:
            ...
            for d in self.drugs:
                if d['name'].lower() in desc.lower():
                    ...
                    break
            if not drug_name:
                self._popup("Error", "Could not identify drug.")
                return
    ...
    def _complete_pharmacy_task(self, task):
        ...
        self._refresh_pharmacy_dispense()
        self._refresh_tasks()
        self._popup("Completed", f"Task {task.get('id')} completed.")
    ...
    def _auto_dispense_from_emr(self, instance):
        ...
        if structured:
            for drug in structured:
                if drug.get("name") and drug.get("total", 0) > 0:
                    drugs_to_dispense.append((drug["name"], drug["total"]))
        else:
            drugs_str = (latest.get("prescribed_drugs", "") or
                         latest.get("medications", ""))
            qtys_str = latest.get("drug_quantities", "")
            if not drugs_str:
                self._popup("Info", "No prescribed drugs in EMR.")
                return
        ...
        for drug_name, qty in drugs_to_dispense:
            ...
            if not found:
                self.prescriptions.append({
                    ...
                    "patient_id": pid,
                    "patient_name": self.cp.get("name"),
                    "drug": drug_name, "qty": qty, "price": 200,
                    "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "dispensed": True})
    ...
    def _view_patient_prescriptions(self, instance):
        ...
        if not rx:
            self._popup("Prescriptions", "None found.")
            return
        text = ""
        for r in rx:
            text += (f"Drug: {r.get('drug', '')}\n"
                     f"Quantity: {r.get('qty', 0)}\n"
                     f"Price: N{r.get('price', 0):,.2f}\n"
                     f"Date: {r.get('ts', '')}\n" + "-" * 30 + "\n")
    ...
    def _ecg_tab(self):
        ...
        left.add_widget(Label(text="COMPREHENSIVE ECG ANALYSIS", font_size=20,
                              bold=True, size_hint_y=None, height=32,
                              color=get_color_from_hex('#c0392b')))
        self.ecg_info = Label(text="No patient selected", font_size=13,
                              size_hint_y=None, height=22,
                              color=get_color_from_hex('#7f8c8d'))
        left.add_widget(self.ecg_info)
        left.add_widget(Label(text="ECG IMAGE:", font_size=15, bold=True,
                              size_hint_y=None, height=20))
        self.ecg_img_disp = TextInput(text="[ECG Image will appear here]",
        ...
        btn_box.add_widget(Button(text="CAPTURE ECG", font_size=16, bold=True,
                                  on_press=self._capture_ecg,
                                  background_color=get_color_from_hex('#e74c3c'),
                                  background_normal=''))
        btn_box.add_widget(Button(text="UPLOAD ECG", font_size=16, bold=True,
                                  on_press=self._upload_ecg,
                                  background_color=get_color_from_hex('#c0392b'),
                                  background_normal=''))
        left.add_widget(btn_box)
        left.add_widget(Label(text="OR ENTER ECG DATA:", font_size=15,
                              bold=True, size_hint_y=None, height=20))
        self.ecg_data_inp = TextInput(font_size=13, size_hint_y=None, height=80,
                                       multiline=True,
        ...
        left.add_widget(Button(text="ANALYZE ECG", font_size=18, bold=True,
                               size_hint_y=None, height=50,
                               on_press=self._analyze_ecg,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        left.add_widget(Button(text="COMBINED AI REPORT (ECG+Lab)", font_size=16,
        ...
        right.add_widget(Label(text="ECG REPORT", font_size=18, bold=True,
                               size_hint_y=None, height=28,
                               color=get_color_from_hex('#2c3e50')))
        ecg_scroll = ScrollView(size_hint_y=0.6, bar_width=10, scroll_type=['bars'])
        self.ecg_report_disp = TextInput(multiline=True, readonly=True, font_size=16,
                                          background_color=get_color_from_hex('#fafafa'),
                                          size_hint_y=None)
        self.ecg_report_disp.bind(minimum_height=self.ecg_report_disp.setter('height'))
        ecg_scroll.add_widget(self.ecg_report_disp)
        right.add_widget(ecg_scroll)
        right.add_widget(Label(text="KEY FINDINGS", font_size=15, bold=True,
                               size_hint_y=None, height=20))
        f_scroll = ScrollView(size_hint_y=None, height=100, bar_width=10,
                               scroll_type=['bars'])
        self.ecg_findings_disp = TextInput(multiline=True, readonly=True,
                                            font_size=16,
                                            background_color=get_color_from_hex('#fef9e7'),
                                            size_hint_y=None)
        self.ecg_findings_disp.bind(
            minimum_height=self.ecg_findings_disp.setter('height'))
        f_scroll.add_widget(self.ecg_findings_disp)
        right.add_widget(f_scroll)
        act = BoxLayout(size_hint_y=None, height=42, spacing=5)
        act.add_widget(Button(text="EXPORT PDF", font_size=15, bold=True,
                              on_press=self._print_ecg,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal=''))
        act.add_widget(Button(text="SAVE", font_size=15, bold=True,
                              on_press=self._save_ecg,
                              background_color=get_color_from_hex('#27ae60'),
                              background_normal=''))
        act.add_widget(Button(text="ADD TO BILL", font_size=15,
    ...
    def _upload_ecg(self, instance):
        ...
        def sel(inst):
            if fc.selection:
                ...
                pp.dismiss()
        btns.add_widget(Button(text="SELECT", font_size=16, bold=True,
                               on_press=sel,
                               background_color=get_color_from_hex('#c0392b'),
                               background_normal=''))
        btns.add_widget(Button(text="CANCEL", font_size=16,
    ...
    def _analyze_ecg(self, instance):
        ...
        pt = {"name": self.cp.get("name", ""), "id": self.cp.get("id", ""),
              "age": self.cp.get("age", 0), "gender": self.cp.get("gender", ""),
    ...
    def _run_ecg(self, pt):
        ...
        self.save_all()
        Clock.schedule_once(lambda dt: self._upd_ecg(
            result["structured"], result["ai_used"], full_report=report), 0)

    def _upd_ecg(self, sd, ai_used, full_report=None):
        self.ecg_report_disp.text = full_report if full_report else ""
        f = (f"Rate: {sd.get('rate','N/A')} | Rhythm: {sd.get('rhythm','N/A')}\n"
             f"PR: {sd.get('pr_interval','N/A')} | "
    ...
    def _print_ecg(self, instance):
        if not self.cur_ecg_report:
            ...
            return
        self._export_to_pdf(self.cur_ecg_report['report'],
    ...
    def _save_ecg(self, instance):
        ...
        self.audit_log("SAVE_ECG", self.cur_ecg_report.get('rid', ''))
    ...
    def _ecg_to_bill(self, instance):
        ...
        self.bill_pid.text = self.cp.get("id", "")
        self.bill_name.text = self.cp.get("name", "")
        price = self._find_price_in_db("ECG", self.investigations,
                                        self.charges) or 0
        self.bill_items.append({"name": "ECG Analysis", "cat": "Charges",
                                 "qty": 1, "price": price, "total": price})
        self._refresh_bill()
    ...
    def _combined_report(self, instance):
        if not self.cp:
            ...
            return
        if not self.cur_ecg_report:
            self._popup("Error", "Run ECG analysis first!")
            return
        if not self.ai.enabled:
        ...
        labs = [t for t in self.lab_tests if t.get("patient_id") == pid
                and t.get("status") == "Completed"]
        if not labs:
            self._popup("Info", "No completed lab results.")
            return
        ecg_summary = self.cur_ecg_report.get("analysis", "No ECG analysis.")
        lab_summary = "\n".join([f"- {lab.get('test_name','')}: "
                                  f"{lab.get('result','')}" for lab in labs[-10:]])
        prompt = (f"Patient: {self.cp.get('name','')} (ID: {pid}), "
                  f"Age: {self.cp.get('age','')}\n\n"
                  f"ECG Report:\n{ecg_summary}\n\n"
                  f"Lab Results:\n{lab_summary}\n\n"
                  f"Provide integrated clinical summary.")

        def run():
            ok, response = self.ai.call(
                "You are a senior clinician synthesizing ECG and lab data.",
                prompt, mt=3000)
            if ok:
                Clock.schedule_once(
                ...
                Clock.schedule_once(
                    lambda dt: self._popup("AI Error", response), 0)
    ...
    def _show_combined_report(self, response):
        c = BoxLayout(orientation='vertical', spacing=5, padding=8)
        sv = ScrollView()
        ti = TextInput(text=response, readonly=True, font_size=16,
                       size_hint_y=None,
                       background_color=get_color_from_hex('#fef9e7'))
        ...
        pp = Popup(title="Combined AI Report (ECG + Lab)", content=c,
                   size_hint=(0.9, 0.85))
        btn_box.add_widget(Button(text="CLOSE", font_size=16, bold=True,
                                  on_press=pp.dismiss, background_normal=''))
        btn_box.add_widget(Button(text="EXPORT PDF", font_size=16, bold=True,
                                  on_press=lambda x: self._export_to_pdf(
    ...
    def _liver_span_tab(self):
        ...
        left.add_widget(Label(text="BELEMA'S LAW", font_size=18, bold=True,
                              size_hint_y=None, height=30,
                              color=get_color_from_hex('#16a085')))
        f = GridLayout(cols=2, size_hint_y=None, height=120, spacing=6)
        for lb, attr, hint in [("Age:", "liver_age", ""),
                                ("Height(m):", "liver_height", "1.65"),
                                ("Measured(cm):", "liver_measured", "")]:
            f.add_widget(Label(text=lb, font_size=15, bold=True, size_hint_x=0.35))
            w = TextInput(font_size=15, size_hint_x=0.5, size_hint_y=None,
                          height=40, hint_text=hint)
            setattr(self, attr, w)
            f.add_widget(w)
        left.add_widget(f)
        left.add_widget(Button(text="CALCULATE", font_size=17, bold=True,
                               size_hint_y=None, height=48,
                               on_press=self._calc_liver,
                               background_color=get_color_from_hex('#16a085'),
                               background_normal=''))
        self.liver_predicted = Label(text="Predicted: -- cm", font_size=18,
                                      bold=True, size_hint_y=None, height=30,
                                      color=get_color_from_hex('#2980b9'))
        left.add_widget(self.liver_predicted)
        self.liver_interpretation = Label(text="Interpretation: --", font_size=18,
                                            bold=True, size_hint_y=None, height=30)
        left.add_widget(self.liver_interpretation)
        right = BoxLayout(orientation='vertical', size_hint_x=0.55,
                          spacing=4, padding=10)
        right.add_widget(Label(text="FORMULA", font_size=16, bold=True,
                               size_hint_y=None, height=25,
                               color=get_color_from_hex('#16a085')))
        right.add_widget(TextInput(
            text="h = 12.6x - 3.00 (<12)\nh = 12.6x - 5.04 (>=12)",
            readonly=True, font_size=13, size_hint_y=0.6,
            background_color=get_color_from_hex('#f8f9fa')))
        main.add_widget(left)
        main.add_widget(right)
        tab.add_widget(main)
    ...
    def _pathology_tab(self):
        ...
        left.add_widget(Label(text="AI PATHOLOGY", font_size=18, bold=True,
                              size_hint_y=None, height=30,
                              color=get_color_from_hex('#8e44ad')))
        left.add_widget(Button(text="EXTRACT PATHOLOGY", font_size=17, bold=True,
                               size_hint_y=None, height=48,
                               on_press=self._run_pathology,
                               background_color=get_color_from_hex('#8e44ad'),
                               background_normal=''))
        left.add_widget(Button(text="VIEW ML", font_size=15, size_hint_y=None,
                               height=40, on_press=self._view_ml,
                               background_color=get_color_from_hex('#2c3e50'),
                               background_normal=''))
        left.add_widget(Button(text="EXPORT ML", font_size=15, size_hint_y=None,
                               height=40, on_press=self._export_ml,
                               background_color=get_color_from_hex('#16a085'),
                               background_normal=''))
        right = BoxLayout(orientation='vertical', size_hint_x=0.55, spacing=3)
        scroll = ScrollView(size_hint_y=0.7, bar_width=10, scroll_type=['bars'])
        self.path_output = TextInput(multiline=True, readonly=True, font_size=16,
                                      background_color=get_color_from_hex('#fafafa'),
                                      size_hint_y=None)
        self.path_output.bind(minimum_height=self.path_output.setter('height'))
        scroll.add_widget(self.path_output)
    ...
    def _run_pathology(self, instance):
        ...
        labs = [t for t in self.lab_tests if t.get("patient_id") == pid]
        rx = [p for p in self.prescriptions if p.get("patient_id") == pid]
        pd = {"id": pid, "name": self.cp.get("name", ""),
              "age": self.cp.get("age", 0), "gender": self.cp.get("gender", "")}
        threading.Thread(target=self._run_path_thread,
                         args=(pd, emr, labs, rx), daemon=True).start()

    def _run_path_thread(self, pd, emr, labs, rx):
        result = self.pathology_ai.extract(pd, emr, labs, rx)
        self.pathology_data.append({"patient_id": pd.get("id", ""),
                                     "ts": datetime.now().isoformat(),
                                     "structured": result.get("structured", {})})
        self.save_all()
        Clock.schedule_once(lambda dt: setattr(
            self.path_output, 'text', result.get("analysis", "No analysis")), 0)

    def _view_ml(self, instance):
        if not self.cp:
            self._popup("ML Data", "Select a patient first.")
            return
        pid = self.cp.get("id")
        pp_data = [p for p in self.pathology_data if p.get("patient_id") == pid]
        if not pp_data:
            self._popup("ML Data", f"No ML data for {self.cp.get('name','')}.")
            return
        latest = pp_data[-1]
        s = latest.get("structured", {})
        lines = ["=" * 70,
                 "          ML / PATHOLOGY ANALYSIS",
                 f"          Patient: {self.cp.get('name','')} (ID: {pid})",
                 f"          Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                 "=" * 70, "",
                 "📊 RISK ASSESSMENT", "-" * 50,
                 f"  Risk Score: {s.get('risk_score', 0)}/10",
                 f"  Risk Level: {s.get('risk_level', 'Unknown')}", ""]
        mf = s.get("ml_features", {})
        if mf:
            lines += ["🤖 ML FEATURES", "-" * 50]
            for k, v in mf.items():
                lines.append(f"  {k.replace('_', ' ').title()}: {v}")
            lines.append("")
        completed = [t for t in self.lab_tests if t.get("patient_id") == pid
                     and t.get("status") == "Completed"]
        if completed:
            lines += ["🧪 LABORATORY RESULTS", "-" * 50]
            for lab in completed[-15:]:
                st = ("⚠️" if any(w in str(lab.get('result', '')).lower()
                                   for w in ['positive', 'abnormal', 'high', 'low'])
                      else "✅")
                lines.append(f"  {st} {lab.get('test_name', '')}: "
                             f"{lab.get('result', '')}")
            lines.append("")
        lines += ["=" * 70, "                    END OF ML ANALYSIS", "=" * 70]
    ...
    def _export_ml(self, instance):
        if not self.cp:
            ...
            return
        pid = self.cp.get("id")
        data = [p for p in self.pathology_data if p.get("patient_id") == pid]
        if not data:
        ...
        try:
            ...
            image_text = self.path_output.text or json.dumps(latest.get('structured', {}), indent=2, default=str)
            image_path = self._render_professional_report_image(
                image_text, 'Pathology_Report', 'Pathology / ML Report',
                patient=self._patient_identity_for_report(pid),
                report_id=f"HMG-PATH-{datetime.now().strftime('%Y%m%d%H%M%S')}")
            self._popup("Exported", f"Saved to {fn}" + (f"\nImage report: {image_path}" if image_path else ""))
            if image_path: self._open_file(image_path)
            self.audit_log("EXPORT_ML", fn, pid)
        except Exception as e:
    ...
    def _lis_tab(self):
        ...
        main = BoxLayout(orientation='vertical', padding=6, spacing=6)
        main.add_widget(Button(text="SCAN QR TO ADD INSTRUMENT", font_size=16,
                               bold=True, size_hint_y=None, height=44,
        ...
        for section, color, cat_name in [
            ...
            main.add_widget(Label(text=section, font_size=18, bold=True,
                                   size_hint_y=None, height=28,
    ...
    def refresh_lis_displays(self):
        for cat_name, cat in [("lab", "lab"), ("radiology", "radiology"),
            ...
            for inst_id in self.lis_manager.instrument_configs.get(cat, {}):
                ...
                info.add_widget(Label(text=status['name'], font_size=13,
                                       bold=True, size_hint_y=None, height=20))
        ...
        def capture(inst):
            if cam.texture:
                ...
                if qr_data:
                    ...
                    self._popup("QR Scan Result",
                                f"{'Connected' if success else 'Failed'}: {msg}")
                    self.refresh_lis_displays()
                else:
                    self._popup("QR Scan Result", "No QR code found.")
                pp.dismiss()
        btns.add_widget(Button(text="CAPTURE", font_size=16, bold=True,
                               on_press=capture,
                               background_color=get_color_from_hex('#27ae60'),
...
# ====================== END OF PART 4a ======================
    ...
    def _billing(self):
        ...
        self.bill_cat = Spinner(text="Procedures",
        ...
        it.add_widget(Button(text="SEARCH", font_size=15, bold=True, size_hint_x=0.1,
                              on_press=self._search_bill_item,
                              background_color=get_color_from_hex('#f39c12'),
                              background_normal=''))
        it.add_widget(Button(text="ADD", font_size=15, bold=True, size_hint_x=0.08,
                              on_press=self._add_bill,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal=''))
        it.add_widget(Button(text="ADD CUSTOM", font_size=15, bold=True,
                              size_hint_x=0.12, on_press=self._add_custom_item,
                              background_color=get_color_from_hex('#8e44ad'),
                              background_normal=''))
        it.add_widget(Button(text="VIEW SAVED", font_size=15, bold=True,
                              size_hint_x=0.12, on_press=self._view_saved_bills,
                              background_color=get_color_from_hex('#16a085'),
                              background_normal=''))
        l.add_widget(it)
        ...
        self.bill_tot = Label(text="TOTAL: N0.00", font_size=20, bold=True,
                               size_hint_x=0.5,
                               color=get_color_from_hex('#c0392b'))
        tot.add_widget(self.bill_tot)
        tot.add_widget(Button(text="SAVE", font_size=17, bold=True, size_hint_x=0.25,
                               on_press=self._save_bill,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        tot.add_widget(Button(text="PRINT PDF", font_size=17, bold=True,
                               size_hint_x=0.25,
                               on_press=lambda x: self._export_to_pdf(
    ...
    def _add_custom_item(self, instance):
        ...
        content.add_widget(Label(text="ADD CUSTOM ITEM", font_size=16, bold=True,
                                  size_hint_y=None, height=30,
                                  color=get_color_from_hex('#8e44ad')))
        ...
        def add(inst):
            ...
            if not name:
                self._popup("Error", "Enter an item name.")
                return
            try:
                price = float(price_inp.text.strip() or "0")
                qty = int(qty_inp.text.strip() or "1")
            except Exception:
            ...
            if price <= 0:
                ...
                return
            self.bill_items.append({"name": name, "cat": cat_inp.text,
                                     "qty": qty, "price": price,
                                     "total": price * qty})
            self._refresh_bill()
            pp.dismiss()
            self._popup("Added", f"{name} x{qty} = N{price * qty:,.2f}")
        btn_box.add_widget(Button(text="ADD", font_size=15, bold=True,
                                   on_press=add,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
    ...
    def _search_bill_item(self, instance):
        ...
        src = {"Procedures": self.procedures,
               "Investigations": self.investigations,
               "Drugs": self.drugs,
               "Charges": self.charges}.get(cat, [])
        c = BoxLayout(orientation='vertical', spacing=5, padding=8)
        sr = TextInput(size_hint_y=None, height=40, font_size=15,
        ...
        def refresh_list(t=""):
            ...
            for item in src:
                if not t or t.lower() in item['name'].lower():
                    ...
                    btn.bind(on_press=lambda x: (setattr(self.bill_item, 'text',
                                                          x.item_name),
                                                  pp.dismiss()))
                    pl.add_widget(btn)
    ...
    def _add_bill(self, instance):
        ...
        if nm == "Select":
            self._popup("Error", "Select an item.")
            return
        try:
        ...
        if not p:
            self._popup("Error", f"Price not found for '{nm}'.")
            return
        self.bill_items.append({"name": nm, "cat": ct, "qty": q,
                                 "price": p, "total": p * q})
        self._refresh_bill()
        self.bill_qty.text = "1"

    def _refresh_bill(self):
        if not hasattr(self, 'bill_box'):
            return
        ...
        for i, item in enumerate(self.bill_items):
            ...
            btn = Button(text="X", font_size=11, size_hint_x=0.05,
                          ...
                          background_normal='')
            btn.idx = i
            btn.bind(on_press=lambda x: (self.bill_items.pop(x.idx),
                                          self._refresh_bill()))
            row.add_widget(btn)
    ...
    def _save_bill(self, instance):
        ...
        self.bills.append({
            ...
            "patient": {"id": self.bill_pid.text, "name": self.bill_name.text},
    ...
    def _view_saved_bills(self, instance):
        ...
        bills = [b for b in self.bills if b.get("patient", {}).get("id") == pid]
        if not bills:
            self._popup("Saved Bills", "No bills.")
            return
        text = f"BILLS FOR {self.bill_name.text}\n" + "=" * 50 + "\n\n"
        for b in bills:
            text += (f"ID: {b.get('bill_id', '')}\n"
                     f"Date: {b.get('ts', '')}\n"
                     f"Total: N{b.get('total', 0):,.2f}\n"
    ...
    def _payment(self):
        ...
        form.add_widget(Label(text="Amount:", font_size=15, bold=True,
                               size_hint_x=0.08))
        self.pay_amt = TextInput(size_hint_x=0.15, font_size=15,
                                  size_hint_y=None, height=40)
        form.add_widget(self.pay_amt)
        form.add_widget(Label(text="Method:", font_size=15, bold=True,
                               size_hint_x=0.08))
        self.pay_method = Spinner(text="Cash",
                                   values=["Cash", "Bank Transfer", "Card", "Insurance"],
                                   size_hint_x=0.18, size_hint_y=None, height=40)
        form.add_widget(self.pay_method)
        form.add_widget(Button(text="RECORD", font_size=15, bold=True,
                                size_hint_x=0.18, on_press=self._record_pay,
                                background_color=get_color_from_hex('#27ae60'),
                                background_normal=''))
        l.add_widget(form)
        btn_row = BoxLayout(size_hint_y=None, height=40, spacing=4)
        btn_row.add_widget(Button(text="PRINT RECEIPT PDF", font_size=15,
                                   bold=True, size_hint_x=0.4,
                                   on_press=self._print_receipt,
                                   background_color=get_color_from_hex('#8e44ad'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="VIEW RECEIPTS", font_size=15, bold=True,
                                   size_hint_x=0.3,
    ...
    def _record_pay(self, instance):
        ...
        if bill_id:
            for b in self.bills:
                if b["bill_id"] == bill_id:
                    ...
                    b["status"] = "Paid" if paid >= b["total"] else "Partial"
                    break
        ...
        self.audit_log("RECORD_PAYMENT",
                       f"N{amt:,.2f} via {self.pay_method.text}", pid)

    def _refresh_pay(self):
        if not hasattr(self, 'pay_pid'):
            return
        pid = self.pay_pid.text
        if not pid:
            return
        ...
        bills = [b for b in self.bills if b.get("patient", {}).get("id") == pid]
        values = ["None"] + [f"{b['bill_id']} - N{b['total']:,.2f} "
                              f"({b.get('status', 'N/A')})" for b in bills]
        self.pay_bill_spinner.values = values
        self.pay_hist.clear_widgets()
        for p in reversed(self.pt_tracker.get_for(pid)):
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            row.add_widget(Label(text=f"{p.get('ts','')[:16]} | "
                                       f"N{p.get('amount', 0):,.2f}",
                                  font_size=12, size_hint_x=0.7))
            btn_v = Button(text="View", font_size=11, size_hint_x=0.15,
                            size_hint_y=None, height=28,
                            background_normal='')
            pay = p
            btn_v.bind(on_press=lambda x, pay=pay:
                       self._view_payment_detail_kivy(pay))
    ...
    def _print_receipt(self, instance):
        ...
        for b in bills:
            receipt += (f"- {b['bill_id']}: N{b['total']:,.2f} "
                        f"({b.get('status', 'N/A')})\n")
        receipt += (f"\nTotal Billed: N{total:,.2f}\n"
                    f"Total Paid: N{paid:,.2f}\nBalance: N{balance:,.2f}\n")
    ...
    def _view_saved_receipts(self, instance):
        ...
        if not payments:
            self._popup("Receipts", "No payments.")
            return
    ...
    def _lab(self):
        ...
        self.lab_test = Spinner(text="Select",
                                 values=[i['name'] for i in self.investigations],
                                 size_hint_x=0.4, size_hint_y=None, height=40)
        order.add_widget(self.lab_test)
        order.add_widget(Button(text="SEARCH", font_size=15, bold=True,
                                 size_hint_x=0.12, on_press=self._search_lab_test,
                                 background_color=get_color_from_hex('#f39c12'),
                                 background_normal=''))
        order.add_widget(Button(text="ORDER", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._order_lab,
                                 background_color=get_color_from_hex('#2980b9'),
                                 background_normal=''))
        order.add_widget(Button(text="VIEW PENDING", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._view_pending_lab,
                                 background_color=get_color_from_hex('#16a085'),
                                 background_normal=''))
        order.add_widget(Button(text="DELETE", font_size=15, bold=True,
                                 size_hint_x=0.1, on_press=self._delete_lab,
                                 background_color=get_color_from_hex('#e74c3c'),
                                 background_normal=''))
        l.add_widget(order)
        ...
        def refresh_list(t=""):
            ...
            for test in self.investigations:
                if not t or t.lower() in test['name'].lower():
                    ...
                    btn.bind(on_press=lambda x: (setattr(self.lab_test, 'text',
                                                          x.test_name),
                                                  pp.dismiss()))
                    pl.add_widget(btn)
        sr.bind(text=lambda i, v: refresh_list(v))
    ...
    def _order_lab(self, instance):
        ...
        self.lab_tests.append({
            ...
            "status": "Ordered", "result": "", "unit": "", "reference": "",
            "date_ordered": datetime.now().strftime("%Y-%m-%d %H:%M"),
            "date_completed": "", "billed": False})
        self.save_all()
        self._refresh_lab()
    ...
    def _refresh_lab(self):
        ...
        tests = [t for t in self.lab_tests if t.get("patient_id") == pid
                 and t.get("status") != "Completed"]
        if not tests:
            self.lab_pend.add_widget(Label(text="No pending tests.",
                                            font_size=13, size_hint_y=None,
                                            height=30))
            return
        for t in tests:
            row = BoxLayout(size_hint_y=None, height=35, spacing=3)
            row.add_widget(Label(text=f"{t.get('test_name','')} - "
                                       f"{t.get('status','')}",
                                  font_size=13, size_hint_x=0.6))
            if t.get("status") == "Ordered":
                btn = Button(text="ENTER RESULT", font_size=12,
                ...
                row.add_widget(btn)
            row.add_widget(Button(text="X", font_size=12, size_hint_x=0.1,
                                   size_hint_y=None, height=30,
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal='',
                                   on_press=lambda x, tid=t.get('test_id'):
                                   self._remove_lab_test(tid)))
            self.lab_pend.add_widget(row)

    def _enter_lab_result(self, instance):
        test = instance.test
        content = BoxLayout(orientation='vertical', spacing=5, padding=8)
        content.add_widget(Label(text=f"Result for {test.get('test_name')}",
                                  font_size=16, bold=True,
                                  size_hint_y=None, height=30))
        for lbl, attr in [("Result:", "result_inp"),
                           ("Unit:", "unit_inp"),
                           ("Reference Range:", "ref_inp")]:
            content.add_widget(Label(text=lbl, font_size=14, bold=True,
                                      size_hint_y=None, height=22))
            w = TextInput(font_size=14, size_hint_y=None, height=40)
            setattr(self, attr, w)
        ...
        def save(inst):
            test["result"] = self.result_inp.text.strip()
            test["unit"] = self.unit_inp.text.strip()
            test["reference"] = self.ref_inp.text.strip()
                ...
                if (task.get("patient_id") == test.get("patient_id") and
                    ...
                    if test.get("test_name") in task.get("description", ""):
                        self.task_manager.update_status(
                            ...
                            f"Result: {test.get('result')}")
                        break
            self._refresh_tasks()
            pp.dismiss()
            self._popup("Saved", f"{test.get('test_name')} result saved.")
            self.audit_log("ENTER_LAB_RESULT",
                           f"{test.get('test_name')} = {test.get('result')}",
                           test.get("patient_id"))
        btn_box.add_widget(Button(text="SAVE", font_size=15, bold=True,
                                   on_press=save,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_box.add_widget(Button(text="CANCEL", font_size=15, on_press=pp.dismiss,
    ...
    def _view_pending_lab(self, instance):
        ...
        tests = [t for t in self.lab_tests if t.get("patient_id") == pid]
        if not tests:
            self._popup("Lab Tests", "None found.")
            return
    ...
    def _delete_lab(self, instance):
        ...
        self.save_all()
        self._refresh_lab()
        self._popup("Deleted", "All lab tests removed.")
    ...
    def _enhanced_lab_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="ENHANCED LABORATORY MODULE", font_size=20,
                           bold=True, size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        bar.add_widget(Button(text="VIEW ALL", font_size=13, size_hint_x=0.15,
                               on_press=self._view_all_lab_results,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        l.add_widget(bar)
        tb = BoxLayout(size_hint_y=None, height=40, spacing=4)
        tb.add_widget(Label(text="Test:", font_size=15, bold=True, size_hint_x=0.06))
        self.elab_test = Spinner(text="Select",
                                  values=[i['name'] for i in self.investigations],
                                  size_hint_x=0.3, size_hint_y=None, height=40)
        tb.add_widget(self.elab_test)
        tb.add_widget(Label(text="Result:", font_size=15, bold=True,
                             size_hint_x=0.08))
        self.elab_result = TextInput(size_hint_x=0.15, font_size=15,
                                      size_hint_y=None, height=40)
        tb.add_widget(self.elab_result)
        tb.add_widget(Label(text="Unit:", font_size=15, bold=True,
                             size_hint_x=0.06))
        self.elab_unit = TextInput(size_hint_x=0.1, font_size=15,
                                    size_hint_y=None, height=40)
        tb.add_widget(self.elab_unit)
        tb.add_widget(Button(text="ADD", font_size=15, bold=True,
                              size_hint_x=0.1, on_press=self._add_enhanced_lab,
                              background_color=get_color_from_hex('#27ae60'),
                              background_normal=''))
        l.add_widget(tb)
        l.add_widget(Label(text="LAB RESULTS", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.elab_results = BoxLayout(orientation='vertical',
                                       size_hint_y=None, spacing=1)
        ...
        btn_row.add_widget(Button(text="EXPORT PDF", font_size=15, bold=True,
                                   on_press=self._export_enhanced_lab,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="GENERATE COMPREHENSIVE REPORT",
                                   font_size=15, bold=True,
                                   on_press=self._generate_comprehensive_lab_report,
                                   background_color=get_color_from_hex('#8e44ad'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="CLEAR RESULTS", font_size=15, bold=True,
                                   on_press=self._clear_enhanced_lab,
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal=''))
        l.add_widget(btn_row)
        l.add_widget(Label(text="SAVED REPORTS", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#1a5276')))
        self.saved_reports_list = BoxLayout(orientation='vertical',
                                              size_hint_y=None, spacing=1)
        self.saved_reports_list.bind(
            minimum_height=self.saved_reports_list.setter('height'))
    ...
    def _add_enhanced_lab(self, instance):
        ...
        if not result:
            ...
            return
        entry = {"id": f"ELAB{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                 "patient_id": self.elab_pid.text,
                 "patient_name": self.elab_name.text,
                 "test_name": self.elab_test.text,
                 "result": result, "unit": self.elab_unit.text.strip(),
                 "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                 "status": "Completed"}
        self.lab_results.append(entry)
        self.save_all()
        self._refresh_elab()
        self.elab_result.text = ""
        self.elab_unit.text = ""
        self._popup("Added", f"{entry['test_name']}: {result}")
        self.audit_log("ADD_ENHANCED_LAB",
                       f"{entry['test_name']} = {result}",
                       self.elab_pid.text)
    ...
    def _refresh_elab(self):
        ...
        for r in sorted(results, key=lambda x: x.get('date', ''), reverse=True):
            ...
            row.add_widget(Label(text=r.get('test_name', ''), font_size=13,
                                  size_hint_x=0.3))
            row.add_widget(Label(text=f"{r.get('result', '')} {r.get('unit', '')}",
                                  font_size=13, size_hint_x=0.2, bold=True,
                                  color=get_color_from_hex('#2980b9')))
            row.add_widget(Label(text=r.get('date', '')[:16], font_size=11,
                                  size_hint_x=0.25))
            btn_del = Button(text="X", font_size=11, size_hint_x=0.05,
                              size_hint_y=None, height=25,
                              background_color=get_color_from_hex('#e74c3c'),
                              background_normal='')
    ...
    def _view_all_lab_results(self, instance):
        ...
        results = [r for r in self.lab_results if r.get("patient_id") == pid]
        if not results:
            self._popup("Lab Results", "None found.")
            return
        text = f"LAB RESULTS FOR {self.elab_name.text}\n" + "=" * 50 + "\n\n"
        for r in results:
            text += (f"{r.get('test_name','')}: {r.get('result','')} "
                     f"{r.get('unit','')}\n"
                     f"Date: {r.get('date','')}\n" + "-" * 30 + "\n")
    ...
    def _export_enhanced_lab(self, instance):
        ...
        if not results:
            ...
            return
        text = (f"HMG HOSPITAL - LABORATORY RESULTS\n"
                f"Patient: {self.elab_name.text} (ID: {self.elab_pid.text})\n"
                f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n" +
                "=" * 60 + "\n\n")
        for r in results:
            text += (f"Test: {r.get('test_name','')}\n"
                     f"Result: {r.get('result','')} {r.get('unit','')}\n"
                     f"Date: {r.get('date','')}\n" + "-" * 40 + "\n")
    ...
    def _clear_enhanced_lab(self, instance):
        ...
        self.save_all()
        self._refresh_elab()
        self._popup("Deleted", "All lab results cleared.")
        self.audit_log("DELETE_ENHANCED_LAB", f"Patient {pid}", pid)

    def _generate_comprehensive_lab_report(self, instance):
        """Generate a professional, image-format comprehensive laboratory report.

        The existing Enhanced Lab workflow is preserved.  The report is rendered
        as a high-resolution PNG so it can be viewed/shared directly from Android,
        with hospital branding, patient identity, reporter identity, QR verification,
        electronic signature area and a pathology stamp/seal.
        """
        if not self.elab_pid.text:
            self._popup("Error", "Select a patient first.")
            return
        pid = self.elab_pid.text.strip()
        name = self.elab_name.text.strip() or "Unnamed Patient"
        results = self._get_patient_lab_results(pid)
        if not results:
            self._popup("Error", "No laboratory tests or results found for this patient.")
            return

        if not PIL_AVAILABLE:
            self._popup(
                "Image Report Error",
                "Pillow is required to generate the image report.\n\n"
                "Install it in Pydroid 3 with: pip install pillow"
            )
            return

        patient = next((p for p in self.patients if p.get("id") == pid), None)
        now = datetime.now()
        report_id = f"HMG-LAB-{now.strftime('%Y%m%d%H%M%S')}"
        generated = now.strftime("%d/%m/%Y %H:%M")

        try:
            from PIL import ImageDraw, ImageFont

            # A4-like portrait image at print-friendly resolution.
            W, H = 1654, 2339
            img = PILImage.new("RGB", (W, H), "white")
            draw = ImageDraw.Draw(img)
            ...
            f12 = font(30)
            f13 = font(33)
            f14b = font(36, bold=True)
            f16b = font(43, bold=True)
            f18b = font(48, bold=True)
            f22b = font(58, bold=True)
            f28b = font(72, bold=True)

            def rounded_box(xy, fill, outline=None, radius=16, width=2):
                draw.rounded_rectangle(xy, radius=radius, fill=fill,
                                       outline=outline, width=width if outline else 1)

            def text(x, y, value, f=f12, fill=dark, anchor=None):
                draw.text((x, y), str(value), font=f, fill=fill, anchor=anchor)

            # ---------------- HEADER / BRANDING ----------------
            # Simple vector HMG mark so the report is self-contained and does not
            # depend on an external logo file.
            cx, cy = 105, 105
            draw.rounded_rectangle((68, 68, 142, 142), radius=12, fill="#2FA84F")
            draw.rectangle((91, 38, 119, 172), fill="#2FA84F")
            draw.rectangle((38, 91, 172, 119), fill="#2FA84F")
            draw.rounded_rectangle((87, 87, 123, 123), radius=7, fill="white")
            text(190, 48, "HMG HOSPITAL LTD", f28b, navy)
            text(193, 126, "...Quality Care, Healthier Tomorrow", f12, muted)
            text(W - 70, 55, "HMG HOSPITAL LTD", f18b, navy, "ra")
            text(W - 70, 112, "5-Obazi Gate, Abonnema, Akulga, Rivers State", f11, dark, "ra")
            text(W - 70, 154, "Surgery  |  Maternity  |  IVF  |  Emergency", f11, blue, "ra")
            draw.line((38, 195, W - 38, 195), fill=blue, width=6)

            # Title ribbon
            rounded_box((38, 220, W - 38, 300), navy, radius=14)
            text(W // 2, 260, "COMPREHENSIVE LABORATORY REPORT", f22b, "white", "mm")

            # ---------------- PATIENT / REPORT IDENTITY ----------------
            info_top, info_bottom = 325, 505
            rounded_box((38, info_top, W - 38, info_bottom), pale_blue,
                        line, radius=14, width=3)
            mid = W // 2
            draw.line((mid, info_top + 18, mid, info_bottom - 18), fill=line, width=2)

            age = patient.get("age", "N/A") if patient else "N/A"
            sex = patient.get("gender", patient.get("sex", "N/A")) if patient else "N/A"
            dob = patient.get("dob", "N/A") if patient else "N/A"
            reporter = "Dr. Precious Belema Ibiabuo"
            ...
            def info_column(items, x, y):
                yy = y
                for label, value in items:
                    text(x, yy, f"{label}:", f12 if label != "Reporter" else f11,
                         navy if label != "Reporter" else navy)
                    text(x + 225, yy, value, f12 if label != "Reporter" else f11, dark)
                    yy += 31

            info_column(left, 62, 350)
            info_column(right, mid + 35, 350)

            # ---------------- RESULTS TABLE ----------------
            table_top = 535
            cols = [38, 105, 590, 850, 1015, 1295, W - 38]
            headers = ["S/N", "TEST", "RESULT", "UNIT", "REFERENCE RANGE", "STATUS"]
            draw.rounded_rectangle((cols[0], table_top, cols[-1], table_top + 55),
                                   radius=10, fill=navy)
            for i, h in enumerate(headers):
                text((cols[i] + cols[i+1]) // 2, table_top + 28, h, f11, "white", "mm")

            # Group by category when category is available; otherwise show all rows.
            row_y = table_top + 55
            row_h = 48
            categories = {}
            for r in sorted(results, key=lambda x: x.get('date', '')):
                cat = str(r.get("category", "Laboratory")).strip() or "Laboratory"
                categories.setdefault(cat, []).append(r)

            serial = 1
            for category, rows in categories.items():
                draw.rectangle((cols[0], row_y, cols[-1], row_y + 40), fill="#D9EFFC")
                text(cols[0] + 20, row_y + 20, category.upper(), f11, navy, "lm")
                row_y += 40
                for r in rows:
                    if row_y + row_h > 1930:
                        # Continue the report on a second canvas-like page below.
                        # This keeps the same image-format output while preventing
                        # result rows from overlapping the signature area.
                        break
                    if serial % 2 == 0:
                        draw.rectangle((cols[0], row_y, cols[-1], row_y + row_h), fill="#FAFCFE")
                    draw.rectangle((cols[0], row_y, cols[-1], row_y + row_h), outline=line, width=1)
                    for x in cols[1:-1]:
                        draw.line((x, row_y, x, row_y + row_h), fill=line, width=1)

                    result = str(r.get("result", ""))
                    unit = str(r.get("unit", ""))
                    reference = str(r.get("reference", r.get("reference_range", "")))
                    status = str(r.get("status", "Completed"))
                    status_lower = status.lower()
                    if status_lower in ("abnormal", "high", "low", "critical"):
                        status_color = red
                    elif "pending" in status_lower or "ordered" in status_lower:
                        status_color = "#B26A00"
                    else:
                        status_color = green
                    ...
                    aligns = ["mm", "lm", "lm", "mm", "mm", "mm"]
                    for i, val in enumerate(values):
                        text((cols[i] + cols[i+1]) // 2, row_y + row_h // 2,
                             val[:36], f10 if i != 1 else f11,
                             status_color if i == 5 else dark, aligns[i])
                    row_y += row_h
                    serial += 1

            # ---------------- INTERPRETATION ----------------
            interp_top = min(max(row_y + 24, 1740), 1930)
            interp_bottom = interp_top + 145
            rounded_box((38, interp_top, W - 38, interp_bottom), light_blue,
                        line, radius=12, width=2)
            text(62, interp_top + 22, "INTERPRETATION / COMMENTS", f14b, navy)
            comment = "Laboratory findings are documented as entered in the EMR. " \
                      "Clinical correlation and review by the responsible clinician are advised."
            # Wrap manually to fit the report.
            words = comment.split()
            lines, current = [], ""
            for word in words:
                trial = (current + " " + word).strip()
                bbox = draw.textbbox((0, 0), trial, font=f11)
                if bbox[2] > W - 150:
            ...
            for line_text in lines[:3]:
                text(62, yy, "• " + line_text, f11, dark)
                yy += 32

            # ---------------- SIGNATURE / QR / STAMP ----------------
            footer_top = 2015
            draw.line((38, footer_top, W - 38, footer_top), fill=blue, width=3)
            text(62, footer_top + 30, "REPORTED BY", f11, muted)
            text(62, footer_top + 63, reporter, f14b, navy)
            text(62, footer_top + 107, "B.Sc., MBBS, M.Sc., MLT, FWACS (Primary)", f11, dark)
            text(62, footer_top + 142, "Medical Director, HMG Hospital Ltd", f11, dark)
            text(62, footer_top + 174, f"Electronically signed: {generated}", f10, muted)

            # Signature mark
            draw.arc((62, footer_top + 5, 305, footer_top + 60), 195, 345, fill=navy, width=4)
            draw.line((78, footer_top + 58, 285, footer_top + 48), fill=navy, width=3)

            # QR code generated from the actual report identity.
            qr_drawn = False
            if qr_b64 := QRCodeGenerator.generate(f"HMG|LAB|{report_id}|{pid}|{name}"):
                try:
                    qr_img = PILImage.open(BytesIO(base64.b64decode(qr_b64))).convert("RGB")
                    qr_img = qr_img.resize((155, 155))
                    qx = (W // 2) - 78
                    qy = footer_top + 32
                    img.paste(qr_img, (qx, qy))
                    text(W // 2, qy + 165, "SCAN TO VERIFY REPORT", f10, navy, "ma")
                    text(W // 2, qy + 195, report_id, f10, muted, "ma")
                    qr_drawn = True
                except Exception:
                    qr_drawn = False

            # Pathology stamp / seal
            sx, sy, sr = W - 155, footer_top + 110, 105
            draw.ellipse((sx-sr, sy-sr, sx+sr, sy+sr), outline=navy, width=8)
            draw.ellipse((sx-sr+14, sy-sr+14, sx+sr-14, sy+sr-14), outline=blue, width=3)
            text(sx, sy - 52, "HMG HOSPITAL", f10, navy, "mm")
            text(sx, sy - 20, "PATHOLOGY", f10, navy, "mm")
            text(sx, sy + 15, "✓", f28b, navy, "mm")
            text(sx, sy + 55, "ABONNEMA", f10, navy, "mm")

            # Bottom contact strip
            strip_y = H - 72
            draw.rectangle((0, strip_y, W, H), fill=navy)
            text(40, strip_y + 36, "5-Obazi Gate, Abonnema, Akulga, Rivers State", f10, "white", "lm")
            text(W - 40, strip_y + 36, "...Quality Care, Healthier Tomorrow", f10, "white", "rm")

            # Save the report as a PNG image.
            dir_path = self._get_reports_dir()
            png_fn = os.path.join(dir_path, f"Comprehensive_Lab_Report_{report_id}.png")
            img.save(png_fn, "PNG", optimize=True)

            self.saved_lab_reports.append({
                "id": report_id,
                "patient_id": pid,
                "patient_name": name,
            ...
            self.save_all()
            self._refresh_saved_reports_list()
            self._open_file(png_fn)
            self._popup(
                "Report Generated",
                f"Professional image report saved:\n\n{os.path.basename(png_fn)}\n\n"
                f"{len(results)} laboratory item(s) included.\n"
                "Completed results are shown as entered; ordered tests without results are marked pending."
            )
            self.audit_log("GENERATE_LAB_IMAGE_REPORT", report_id, pid)

        except Exception as e:
            import traceback
            print("Comprehensive lab image report error:", traceback.format_exc())
            self._popup("Image Report Error", str(e))
    ...
    def _refresh_saved_reports_list(self):
        ...
        for r in sorted(reports, key=lambda x: x.get('date', ''), reverse=True):
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            row.add_widget(Label(text=f"{r.get('id', '')} | "
                                       f"{r.get('date', '')[:16]} | "
                                       f"{r.get('result_count', 0)} results",
                                  font_size=12, size_hint_x=0.6))
            btn_view = Button(text="OPEN", font_size=11, size_hint_x=0.2,
                              size_hint_y=None, height=25,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal='')
            ...
            btn_del.bind(on_press=lambda x:
                         self._delete_saved_report(x.report_id))
            row.add_widget(btn_del)
    ...
    def _delete_saved_report(self, report_id):
        ...
        self.save_all()
        self._refresh_saved_reports_list()
        self._popup("Deleted", "Report removed.")
    ...
    def _rad(self):
        ...
        l.add_widget(bar)
        order = BoxLayout(size_hint_y=None, height=40, spacing=4)
        order.add_widget(Label(text="Study:", font_size=15, bold=True,
                                size_hint_x=0.06))
        ...
        order.add_widget(Button(text="ORDER", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._order_rad,
                                 background_color=get_color_from_hex('#2980b9'),
                                 background_normal=''))
        order.add_widget(Button(text="VIEW", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._view_rad,
                                 background_color=get_color_from_hex('#16a085'),
                                 background_normal=''))
        order.add_widget(Button(text="DELETE", font_size=15, bold=True,
                                 size_hint_x=0.1, on_press=self._delete_rad,
                                 background_color=get_color_from_hex('#e74c3c'),
                                 background_normal=''))
        l.add_widget(order)
        self.rad_list = BoxLayout(orientation='vertical',
                                   size_hint_y=None, spacing=1)
    ...
    def _order_rad(self, instance):
        ...
        self.radiology.append({
            ...
            "study": self.rad_study.text, "status": "Ordered", "report": "",
    ...
    def _refresh_rad(self):
        ...
        if not studies:
            self.rad_list.add_widget(Label(text="No radiology studies.",
                                            font_size=13,
                                            size_hint_y=None, height=30))
            return
        for r in studies:
            row = BoxLayout(size_hint_y=None, height=35, spacing=3)
            row.add_widget(Label(text=f"{r.get('study','')} - "
                                       f"{r.get('status','')}",
                                  font_size=13, size_hint_x=0.5))
            if r.get("status") == "Ordered":
                btn = Button(text="ENTER REPORT", font_size=12,
                ...
                btn = Button(text="VIEW REPORT", font_size=12,
                              size_hint_x=0.25, size_hint_y=None, height=30,
                              background_color=get_color_from_hex('#2980b9'),
                              background_normal='')
                btn.rad = r
                ...
                row.add_widget(btn)
                img_btn = Button(text="IMAGE REPORT", font_size=12, size_hint_x=0.18,
                                 size_hint_y=None, height=30,
                                 background_color=get_color_from_hex('#8e44ad'),
                                 background_normal='')
                img_btn.rad = r
                img_btn.bind(on_press=self._export_radiology_image)
                row.add_widget(img_btn)
            row.add_widget(Button(text="X", font_size=12, size_hint_x=0.1,
                                   size_hint_y=None, height=30,
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal='',
                                   on_press=lambda x, rid=r.get('rad_id'):
                                   self._remove_rad(rid)))
            self.rad_list.add_widget(row)

    def _enter_rad_report(self, instance):
        rad = instance.rad
        content = BoxLayout(orientation='vertical', spacing=5, padding=8)
        content.add_widget(Label(text=f"Report for {rad.get('study')}",
                                  font_size=16, bold=True,
                                  size_hint_y=None, height=30))
        ...
        content.add_widget(Label(text="Impression:", font_size=14, bold=True,
                                  size_hint_y=None, height=22))
        imp_inp = TextInput(font_size=14, size_hint_y=None, height=80,
                             multiline=True)
        content.add_widget(imp_inp)
        btn_box = BoxLayout(size_hint_y=None, height=40, spacing=5)
        ...
        def save(inst):
            ...
            self._refresh_tasks()
            pp.dismiss()
            self._popup("Saved", "Radiology report completed.")
    ...
    def _export_radiology_image(self, instance):
        rad = getattr(instance, 'rad', None) or {}
        pid = str(rad.get('patient_id', '')).strip()
        if not pid:
            self._popup('Error', 'Patient identity is missing.')
            return
        patient = self._patient_identity_for_report(pid)
        report = (f"Study: {rad.get('study','')}\n"
                  f"Status: {rad.get('status','')}\n"
                  f"Date: {rad.get('date', rad.get('completed',''))}\n\n"
                  f"{rad.get('report','No radiology report entered.')}")
        path = self._render_professional_report_image(
            report, 'Radiology_Report', 'Radiology Report', patient=patient,
            report_id=rad.get('rad_id') or None)
        if path:
            self._open_file(path)
            self._popup('Report Generated', f'Professional image report saved:\n{path}')
            self.audit_log('EXPORT_RADIOLOGY_IMAGE', path, pid)

    def _remove_rad(self, rad_id):
        self.radiology = [r for r in self.radiology
                          if r.get("rad_id") != rad_id]
        self.save_all()
        self._refresh_rad()
    ...
    def _view_rad(self, instance):
        ...
        studies = [r for r in self.radiology if r.get("patient_id") == pid]
        if not studies:
            self._popup("Radiology", "None found.")
            return
    ...
    def _upload_tab(self):
        ...
        upload.add_widget(Button(text="UPLOAD FILE", font_size=15, bold=True,
                                  size_hint_x=0.25, on_press=self._upload_file,
                                  background_color=get_color_from_hex('#2980b9'),
                                  background_normal=''))
        upload.add_widget(Button(text="VIEW RESULTS", font_size=15, bold=True,
                                  size_hint_x=0.25,
                                  on_press=self._view_uploaded_results,
                                  background_color=get_color_from_hex('#16a085'),
                                  background_normal=''))
        upload.add_widget(Button(text="DELETE", font_size=15, bold=True,
                                  size_hint_x=0.15, on_press=self._delete_uploaded,
                                  background_color=get_color_from_hex('#e74c3c'),
                                  background_normal=''))
        l.add_widget(upload)
        self.up_list = BoxLayout(orientation='vertical',
                                  size_hint_y=None, spacing=1)
    ...
    def _upload_file(self, instance):
        ...
        def select(inst):
            if fc.selection:
                src = fc.selection[0]
                dest_dir = os.path.join(self._get_reports_dir(),
                                         self.up_pid.text)
                os.makedirs(dest_dir, exist_ok=True)
                dest = os.path.join(dest_dir, os.path.basename(src))
                shutil.copy2(src, dest)
                self._popup("Uploaded", f"File saved to {dest}")
    ...
    def _refresh_uploaded_results(self):
        ...
        if not os.path.exists(dir_path):
            self.up_list.add_widget(Label(text="No uploaded files.",
                                            font_size=13, size_hint_y=None,
                                            height=30))
            return
        files = os.listdir(dir_path)
        if not files:
            self.up_list.add_widget(Label(text="No files in patient directory.",
                                            font_size=13, size_hint_y=None,
                                            height=30))
            return
        for f in files:
            ...
            btn = Button(text="VIEW", font_size=13, size_hint_x=0.2,
                          size_hint_y=None, height=30,
                          background_color=get_color_from_hex('#2980b9'),
                          background_normal='')
    ...
    def _saved_items_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=5, spacing=3, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="SAVED ITEMS", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        for txt, cb, col in [("BILLS", self._show_saved_bills, '#2980b9'),
                              ("PAYMENTS", self._show_saved_payments, '#27ae60'),
                              ("ECG REPORTS", self._show_saved_ecg, '#e74c3c'),
                              ("LAB TESTS", self._show_saved_lab, '#8e44ad'),
                              ("RADIOLOGY", self._show_saved_rad, '#16a085'),
                              ("LAB REPORTS", self._show_saved_lab_reports,
                               '#8e44ad')]:
            row.add_widget(Button(text=txt, font_size=15, bold=True,
                                   on_press=cb, background_normal='',
                                   background_color=get_color_from_hex(col)))
        l.add_widget(row)
        self.saved_items_display = BoxLayout(orientation='vertical',
                                               size_hint_y=None, spacing=1)
        self.saved_items_display.bind(
            minimum_height=self.saved_items_display.setter('height'))
    ...
    def _show_saved_payments(self, instance):
        ...
        payments = self.pt_tracker.get_for(pid)
        if not payments:
            self._popup("Payments", "None found.")
            return
        text = ""
        for p in payments:
    ...
    def _show_saved_ecg(self, instance):
        ...
        if not reports:
            self._popup("ECG Reports", "None found.")
            return
        text = ""
        for e in reports:
            sd = e.get("structured", {})
            text += (f"Report ID: {e.get('rid', '')}\n"
                     f"Date: {e.get('ts', '')}\n"
                     f"Interpretation: {sd.get('interpretation', 'N/A')}\n"
                     f"Urgency: {sd.get('urgency', 'Routine')}\n" +
                     "-" * 30 + "\n")
    ...
    def _show_saved_lab_reports(self, instance):
        ...
        reports = [r for r in self.saved_lab_reports
                   if r.get("patient_id") == pid]
        if not reports:
            self._popup("Lab Reports", "None found.")
            return
        text = ""
        for r in sorted(reports, key=lambda x: x.get('date', ''), reverse=True):
            text += (f"Report ID: {r.get('id', '')}\n"
                     f"Date: {r.get('date', '')}\n"
                     f"Results: {r.get('result_count', 0)}\n"
                     f"File: {os.path.basename(r.get('file_path', ''))}\n" +
                     "-" * 30 + "\n")
    ...
    def _daily_notes_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="DAILY CLINICAL NOTES", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        l.add_widget(Label(text="Date:", font_size=15, bold=True,
                           size_hint_y=None, height=18))
        self.daily_date = TextInput(text=datetime.now().strftime("%Y-%m-%d"),
                                     font_size=15, size_hint_y=None, height=40)
        l.add_widget(self.daily_date)
        l.add_widget(Label(text="Clinical Notes:", font_size=15, bold=True,
                           size_hint_y=None, height=18))
        self.daily_notes_text = TextInput(font_size=14, size_hint_y=None,
                                           height=120, multiline=True,
                                           hint_text="Enter daily clinical notes...")
        l.add_widget(self.daily_notes_text)
        vg = GridLayout(cols=4, size_hint_y=None, height=100, spacing=3)
        for lbl, attr in [("BP:", "daily_bp"), ("Pulse:", "daily_pulse"),
                           ("Temp:", "daily_temp"), ("SpO2:", "daily_spo2"),
                           ("RR:", "daily_rr")]:
            vg.add_widget(Label(text=lbl, font_size=13))
            w = TextInput(font_size=13, size_hint_y=None, height=35)
            setattr(self, attr, w)
            vg.add_widget(w)
        vg.add_widget(Label(text="AVPU:", font_size=13))
        self.daily_avpu = Spinner(text="Alert",
                                    values=["Alert", "Voice", "Pain",
                                            "Unresponsive"],
                                    size_hint_y=None, height=35)
        vg.add_widget(self.daily_avpu)
        l.add_widget(vg)
        btn_row = BoxLayout(size_hint_y=None, height=42, spacing=5)
        btn_row.add_widget(Button(text="SAVE NOTE", font_size=16, bold=True,
                                   on_press=self._save_daily_note,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="VIEW HISTORY", font_size=16, bold=True,
                                   on_press=self._view_daily_notes,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="AI GENERATE", font_size=16, bold=True,
                                   on_press=self._ai_generate_daily_note,
                                   background_color=get_color_from_hex('#8e44ad'),
                                   background_normal=''))
        l.add_widget(btn_row)
        self.daily_notes_list = BoxLayout(orientation='vertical',
                                            size_hint_y=None, spacing=1)
        self.daily_notes_list.bind(
            minimum_height=self.daily_notes_list.setter('height'))
    ...
    def _save_daily_note(self, instance):
        ...
        for attr in ["daily_bp", "daily_pulse", "daily_temp",
                      "daily_spo2", "daily_rr"]:
    ...
    def _refresh_daily_notes(self):
        ...
        for n in sorted(notes, key=lambda x: x.get('date', ''),
                        reverse=True)[:10]:
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            row.add_widget(Label(text=f"{n.get('date', '')} | "
                                       f"BP: {n.get('vitals', {}).get('bp', 'N/A')}",
                                  font_size=12, size_hint_x=0.6))
            btn = Button(text="VIEW", font_size=12, size_hint_x=0.2,
                          size_hint_y=None, height=25,
                          background_color=get_color_from_hex('#2980b9'),
                          background_normal='')
            btn.note = n
    ...
    def _view_daily_notes(self, instance):
        ...
        notes = [n for n in self.daily_notes if n.get("patient_id") == pid]
        if not notes:
            self._popup("Daily Notes", "None found.")
            return
        text = ""
    ...
    def _ai_generate_daily_note(self, instance):
        ...
        labs = [t for t in self.lab_tests if t.get("patient_id") == pid
                and t.get("status") == "Completed"]
        prompt = (f"Patient: {self.cp.get('name', '')}, "
                  f"Age: {self.cp.get('age', 0)}\n"
                  f"Recent EMRs: {len(emrs)}, Recent Labs: {len(labs)}\n"
                  f"Generate a daily clinical note in SOAP format.")
        self.daily_notes_text.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a clinician writing daily notes.",
                                     prompt, mt=800)
            Clock.schedule_once(lambda dt: setattr(
                self.daily_notes_text, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _pharmacy(self):
        ...
        pharm_search = BoxLayout(size_hint_y=None, height=36, spacing=4)
        pharm_search.add_widget(Label(text="🔍 Search:", font_size=13, bold=True,
                                       size_hint_x=0.1))
        self.pharm_drug_search = TextInput(
            font_size=13, size_hint_x=0.75, size_hint_y=None, height=36,
            hint_text="Type to filter drugs (name or category)...",
            multiline=False)
        self.pharm_drug_search.bind(text=self._on_pharm_drug_search_change)
        pharm_search.add_widget(self.pharm_drug_search)
        pharm_search.add_widget(Button(text="Clear", font_size=12, size_hint_x=0.1,
                                        size_hint_y=None, height=36,
                                        on_press=lambda x: setattr(
                                            self.pharm_drug_search, 'text', ''),
                                        background_color=get_color_from_hex('#95a5a6'),
                                        background_normal=''))
        l.add_widget(pharm_search)

        order = BoxLayout(size_hint_y=None, height=40, spacing=4)
        order.add_widget(Label(text="Drug:", font_size=15, bold=True,
                                size_hint_x=0.06))
        self.pharm_drug = Spinner(text="Select",
                                   values=[d['name'] for d in self.drugs],
                                   size_hint_x=0.3, size_hint_y=None, height=40)
        order.add_widget(self.pharm_drug)
        order.add_widget(Label(text="Qty:", font_size=15, bold=True,
                                size_hint_x=0.05))
        self.pharm_qty = TextInput(text="1", size_hint_x=0.08, font_size=15,
                                    size_hint_y=None, height=40)
        order.add_widget(self.pharm_qty)
        order.add_widget(Button(text="DISPENSE", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._dispense_drug,
                                 background_color=get_color_from_hex('#27ae60'),
                                 background_normal=''))
        order.add_widget(Button(text="VIEW RX", font_size=15, bold=True,
                                 size_hint_x=0.15, on_press=self._view_rx,
                                 background_color=get_color_from_hex('#2980b9'),
                                 background_normal=''))
        l.add_widget(order)
        self.pharm_list = BoxLayout(orientation='vertical',
                                     size_hint_y=None, spacing=1)
    ...
    def _dispense_drug(self, instance):
        ...
        for d in self.drugs:
            if d['name'] == drug_name:
                if d['qty'] < qty:
                    self._popup("Error",
                                f"Insufficient stock. Available: {d['qty']}")
                    return
                d['qty'] -= qty
                break
        self.prescriptions.append({
    ...
    def _refresh_pharm(self):
        ...
        rx = [r for r in self.prescriptions if r.get("patient_id") == pid]
        if not rx:
            self.pharm_list.add_widget(Label(text="No prescriptions.",
                                              font_size=13,
                                              size_hint_y=None, height=30))
            return
        for r in rx:
    ...
    def _drug_mgmt(self):
        ...
        l = BoxLayout(orientation='vertical', padding=5, spacing=3, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="DRUG INVENTORY MANAGEMENT", font_size=20,
                           bold=True, size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        ...
        self.dm_cat = Spinner(text="General",
                               values=["Analgesic", "Antibiotic",
        ...
        l.add_widget(form)
        btn_row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        btn_row.add_widget(Button(text="ADD DRUG", font_size=15, bold=True,
                                   on_press=self._add_drug,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="UPDATE STOCK", font_size=15, bold=True,
                                   on_press=self._update_drug_stock,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="DELETE DRUG", font_size=15, bold=True,
                                   on_press=self._delete_drug,
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="REFRESH", font_size=15, bold=True,
                                   on_press=self._refresh_drug_list,
                                   background_color=get_color_from_hex('#16a085'),
                                   background_normal=''))
        l.add_widget(btn_row)
        self.drug_list = BoxLayout(orientation='vertical',
                                    size_hint_y=None, spacing=1)
    ...
    def _add_drug(self, instance):
        ...
        try:
            ...
            self._popup("Error", "Invalid values.")
            return
        if price <= 0 or qty < 0:
            self._popup("Error", "Price must be > 0, qty >= 0.")
            return
        self.drugs.append({"name": name, "price": price, "qty": qty,
                            "min": min_stock, "cat": self.dm_cat.text})
        self.save_all()
        self._refresh_drug_list()
        self._popup("Added", f"Added {name}")
        self.audit_log("ADD_DRUG", f"{name} @ N{price}")
        for attr in ['emr_drug_spinner', 'dispense_drug', 'pharm_drug']:
            if hasattr(self, attr):
                getattr(self, attr).values = [d['name'] for d in self.drugs]
    ...
    def _update_drug_stock(self, instance):
        ...
        content.add_widget(Label(
            text=f"Update stock for: "
                 f"{self.drugs[self.sel_drug_idx]['name']}",
            font_size=16, bold=True, size_hint_y=None, height=30))
        content.add_widget(Label(text="New Quantity:", font_size=14,
        ...
        def do_update(inst):
            try:
                ...
                self.audit_log("UPDATE_DRUG_STOCK",
                               f"{self.drugs[self.sel_drug_idx]['name']} "
                               f"→ {new_qty}")
            except Exception:
                self._popup("Error", "Invalid quantity.")
        btn_box.add_widget(Button(text="UPDATE", font_size=15, bold=True,
                                   on_press=do_update,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_box.add_widget(Button(text="CANCEL", font_size=15,
                                   on_press=pp.dismiss, background_normal=''))
        content.add_widget(btn_box)
        pp.open()
    ...
    def _delete_drug(self, instance):
        ...
        content = BoxLayout(orientation='vertical', spacing=10, padding=10)
        content.add_widget(Label(text=f"Delete {drug['name']}?"))
        btn_box = BoxLayout(size_hint_y=None, height=40, spacing=5)
        ...
        def do_delete(inst):
            ...
            for attr in ['emr_drug_spinner', 'dispense_drug', 'pharm_drug']:
                if hasattr(self, attr):
                    getattr(self, attr).values = [d['name'] for d in self.drugs]
        btn_box.add_widget(Button(text="DELETE", font_size=15, bold=True,
                                   on_press=do_delete,
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal=''))
        btn_box.add_widget(Button(text="CANCEL", font_size=15,
    ...
    def _refresh_drug_list(self, instance=None):
        ...
        if not self.drugs:
            self.drug_list.add_widget(Label(text="No drugs in inventory.",
                                              font_size=13,
                                              size_hint_y=None, height=30))
            return
        for i, d in enumerate(self.drugs):
            ...
            row.add_widget(Label(text=f"{d['name']}", font_size=13,
                                  size_hint_x=0.3))
            ...
            row.add_widget(Label(text=f"{d.get('cat', '')}", font_size=12,
                                  size_hint_x=0.15))
            btn = Button(text="SELECT", font_size=11, size_hint_x=0.1,
                          size_hint_y=None, height=25,
                          background_color=get_color_from_hex('#2980b9'),
                          background_normal='')
            btn.idx = i
    ...
    def _inv_mgmt(self):
        ...
        l = BoxLayout(orientation='vertical', padding=5, spacing=3, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="INVESTIGATION MANAGEMENT", font_size=20,
                           bold=True, size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        form = GridLayout(cols=4, size_hint_y=None, height=120, spacing=4)
        ...
        form.add_widget(Label(text="Department:", font_size=14, bold=True))
        self.im_dept = Spinner(text="Laboratory",
                                values=["Laboratory", "Radiology", "Cardiology"],
                                size_hint_y=None, height=35)
        form.add_widget(self.im_dept)
        form.add_widget(Label(text="Category:", font_size=14, bold=True))
        self.im_cat = Spinner(text="General",
        ...
        l.add_widget(form)
        btn_row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        btn_row.add_widget(Button(text="ADD", font_size=15, bold=True,
                                   on_press=self._add_inv,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="DELETE", font_size=15, bold=True,
        ...
        btn_row.add_widget(Button(text="REFRESH", font_size=15, bold=True,
                                   ...
                                   background_normal=''))
        l.add_widget(btn_row)
        self.inv_list = BoxLayout(orientation='vertical',
                                   size_hint_y=None, spacing=1)
    ...
    def _add_inv(self, instance):
        ...
        if price <= 0:
            self._popup("Error", "Price must be > 0.")
            return
        self.investigations.append({"name": name, "price": price,
        ...
        for attr in ['lab_test', 'elab_test']:
            if hasattr(self, attr):
                getattr(self, attr).values = [i['name'] for i in self.investigations]
    ...
    def _delete_inv(self, instance):
        ...
        content = BoxLayout(orientation='vertical', spacing=10, padding=10)
        content.add_widget(Label(text=f"Delete {inv['name']}?"))
        btn_box = BoxLayout(size_hint_y=None, height=40, spacing=5)
        pp = Popup(title="Confirm Delete", content=content, size_hint=(0.6, 0.3))
    ...
    def _refresh_inv_list(self, instance=None):
        ...
        if not self.investigations:
            self.inv_list.add_widget(Label(text="No investigations configured.",
                                            font_size=13, size_hint_y=None,
                                            height=30))
            return
    ...
    def _settings(self):
        ...
        row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row.add_widget(Button(text="SAVE AI SETTINGS", font_size=16, bold=True,
                               on_press=self._save_ai_settings,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        row.add_widget(Button(text="TEST AI", font_size=16, bold=True,
                               on_press=self._test_ai_connection,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        l.add_widget(row)
        ...
        l.add_widget(Label(text="SECURITY", font_size=18, bold=True,
                           size_hint_y=None, height=28,
                           color=get_color_from_hex('#c0392b')))
        sec_row = BoxLayout(size_hint_y=None, height=42, spacing=5)
        sec_row.add_widget(Button(text="🔒 CHANGE DELETE PASSCODE", font_size=15,
                                   bold=True, on_press=self._change_delete_passcode,
                                   background_color=get_color_from_hex('#8e44ad'),
                                   background_normal=''))
        sec_row.add_widget(Button(text="🔐 CHANGE MY PASSWORD", font_size=15,
                                   bold=True, on_press=self._change_my_password,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        l.add_widget(sec_row)
        l.add_widget(Label(
            text="Delete passcode stored hashed in config.json. Default: 1234",
            font_size=12, size_hint_y=None, height=20,
            color=get_color_from_hex('#7f8c8d')))

        l.add_widget(Label(text="", size_hint_y=None, height=10))
        l.add_widget(Label(text="DATA MANAGEMENT", font_size=18, bold=True,
                           size_hint_y=None, height=28,
                           color=get_color_from_hex('#c0392b')))
        data_row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        data_row.add_widget(Button(text="BACKUP DATA", font_size=15, bold=True,
                                    on_press=self._backup_data,
                                    background_color=get_color_from_hex('#16a085'),
                                    background_normal=''))
        data_row.add_widget(Button(text="EXPORT PATIENTS CSV", font_size=15,
                                    bold=True, on_press=self._export_csv,
                                    background_color=get_color_from_hex('#2980b9'),
                                    background_normal=''))
        data_row.add_widget(Button(text="REFRESH ALL", font_size=15, bold=True,
        ...
        l.add_widget(Label(text="", size_hint_y=None, height=10))
        l.add_widget(Label(text="STATUS", font_size=18, bold=True,
                           size_hint_y=None, height=28,
                           color=get_color_from_hex('#2c3e50')))
        status = (f"User: {self._current_username()} ({self._current_role()})\n"
                  f"AI: {'Enabled' if self.ai.enabled else 'Disabled'}\n"
                  f"Provider: {self.ai.provider}\n"
                  f"Model: {self.ai.model}\n"
                  f"Patients: {len(self.patients)}\n"
                  f"EMR Records: {len(self.emr_records)}\n"
                  f"Drugs: {len(self.drugs)}\n"
                  f"Investigations: {len(self.investigations)}\n"
                  f"Lab Tests: {len(self.lab_tests)}\n"
                  f"Bills: {len(self.bills)}\n"
                  f"Payments: {len(self.pt_tracker.payments)}\n"
                  f"ECG Reports: {len(self.ecg_reports)}\n"
                  f"Radiology: {len(self.radiology)}\n"
                  f"Tasks: {len(self.tasks)}\n"
                  f"Admissions: {len(self.admissions)}\n"
                  f"Lab Reports: {len(self.saved_lab_reports)}\n"
                  f"Appointments: {len(self.appointments)}\n"
                  f"Nursing Notes: {len(self.nursing_notes)}\n"
                  f"Users: {len(self.user_manager.users)}\n"
                  f"Audit Entries: {len(self.audit.entries)}\n")
        l.add_widget(TextInput(text=status, readonly=True, font_size=13,
    ...
    def _save_ai_settings(self, instance):
        self.ai.save(self.ai_provider.text, self.ai_key.text.strip(),
                     self.ai_model.text)
        self.ai.enabled = bool(self.ai_key.text.strip() and
                                len(self.ai_key.text.strip()) > 10)
        if self.ai.enabled:
            self.ai_lbl.text = f"AI: {self.ai_provider.text}"
            self.ai_lbl.color = get_color_from_hex('#27ae60')
        else:
            self.ai_lbl.text = "AI: Offline"
            self.ai_lbl.color = get_color_from_hex('#e74c3c')
        self._popup("AI Settings", "Saved.")
        self.audit_log("UPDATE_AI_SETTINGS",
                       f"{self.ai_provider.text} / {self.ai_model.text}")
    ...
    def _backup_data(self, instance):
        try:
            ...
            backup_file = os.path.join(backup_dir, f"hmg_backup_{ts}.json")
            data = {k: getattr(self, k) for k in [
                "patients", "emr_records", "lab_tests", "radiology",
                "prescriptions", "bills", "drugs", "investigations",
                "ecg_reports", "followups", "daily_notes", "consent_forms",
                "referral_notes", "patient_snapshots", "lab_results", "tasks",
                "admissions", "saved_lab_reports", "appointments",
                "nursing_notes"]}
            data["payments"] = self.pt_tracker.payments
            data["users"] = self.user_manager.users
            self._save_json(backup_file, data)
            self._popup("Backup", f"Saved to {backup_file}")
    ...
    def _export_csv(self, instance):
        if not self.patients:
            self._popup("Export", "No patients.")
            return
        try:
    ...
    def _change_delete_passcode(self, instance):
        ...
        content.add_widget(Label(text="Change Delete Patient Passcode",
                                  font_size=16, bold=True,
                                  size_hint_y=None, height=30))
        for lbl, attr in [("Current passcode:", "cur_inp"),
                           ("New passcode (min 4 chars):", "new_inp"),
                           ("Confirm new passcode:", "conf_inp")]:
            content.add_widget(Label(text=lbl, font_size=13, bold=True,
                                      size_hint_y=None, height=20))
            w = TextInput(password=True, multiline=False,
                           font_size=15, size_hint_y=None, height=40)
            setattr(self, attr, w)
            content.add_widget(w)
        btns = BoxLayout(size_hint_y=None, height=42, spacing=5)
        pp = Popup(title="Security", content=content, size_hint=(0.6, 0.55),
                   auto_dismiss=False)

        def do_change(inst):
            if not self._verify_delete_passcode(self.cur_inp.text):
                self._popup("Error", "Current passcode is incorrect.")
                return
            if self.new_inp.text.strip() != self.conf_inp.text.strip():
                self._popup("Error", "New passcodes do not match.")
                return
            ok, msg = self._set_delete_passcode(self.new_inp.text)
            pp.dismiss()
            self._popup("Security", msg)
            if ok:
                self.audit_log("CHANGE_DELETE_PASSCODE")
        btns.add_widget(Button(text="SAVE", font_size=15, bold=True,
                               on_press=do_change,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        btns.add_widget(Button(text="CANCEL", font_size=15, bold=True,
                               on_press=lambda x: pp.dismiss(),
                               background_color=get_color_from_hex('#e74c3c'),
                               background_normal=''))
        content.add_widget(btns)
    ...
    def _change_my_password(self, instance):
        ...
        content.add_widget(Label(
            ...
            font_size=16, bold=True, size_hint_y=None, height=30))
        for lbl, attr in [("Current password:", "cp_cur"),
                           ("New password (min 4 chars):", "cp_new"),
                           ("Confirm new password:", "cp_conf")]:
            content.add_widget(Label(text=lbl, font_size=13, bold=True,
                                      size_hint_y=None, height=20))
            w = TextInput(password=True, multiline=False,
                           font_size=15, size_hint_y=None, height=40)
        ...
        def do_change(inst):
            if self.cp_new.text != self.cp_conf.text:
                self._popup("Error", "New passwords do not match.")
                return
            ok, msg = self.user_manager.change_password(
                self._current_username(), self.cp_cur.text, self.cp_new.text)
            pp.dismiss()
    ...
    def _chat_tab(self):
        ...
        main = BoxLayout(orientation='vertical', padding=6, spacing=4)
        main.add_widget(Label(text="HOSPITAL CHAT", font_size=20, bold=True,
                              size_hint_y=None, height=32,
                              color=get_color_from_hex('#1a5276')))
        top = BoxLayout(size_hint_y=None, height=40, spacing=4)
        top.add_widget(Label(text="Room:", font_size=15, bold=True,
                              size_hint_x=0.08))
        ...
        top.add_widget(self.chat_room)
        top.add_widget(Button(text="NEW ROOM", font_size=15, bold=True,
                               size_hint_x=0.15, on_press=self._new_chat_room,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        top.add_widget(Button(text="DELETE ROOM", font_size=15, bold=True,
                               size_hint_x=0.15, on_press=self._delete_chat_room,
                               background_color=get_color_from_hex('#e74c3c'),
                               background_normal=''))
        main.add_widget(top)
        scroll = ScrollView(size_hint_y=0.65, bar_width=10, scroll_type=['bars'])
        self.chat_display = TextInput(multiline=True, readonly=True, font_size=14,
                                       background_color=get_color_from_hex('#f8f9fa'),
                                       size_hint_y=None)
        ...
        self.chat_input = TextInput(size_hint_x=0.7, font_size=15,
                                     size_hint_y=None, height=45,
                                     hint_text="Type message...",
                                     on_text_validate=self._send_chat)
        bottom.add_widget(self.chat_input)
        bottom.add_widget(Button(text="SEND", font_size=16, bold=True,
                                  size_hint_x=0.2, on_press=self._send_chat,
                                  background_color=get_color_from_hex('#27ae60'),
                                  background_normal=''))
        bottom.add_widget(Button(text="📷", font_size=16, bold=True,
                                  size_hint_x=0.1, on_press=self._chat_camera,
                                  background_color=get_color_from_hex('#2980b9'),
                                  background_normal=''))
    ...
    def _new_chat_room(self, instance):
        ...
        def create(inst):
            ...
            if name and name not in self.chat_sys.rooms:
                ...
                self._popup("Error", "Invalid or duplicate name.")
        btn_box.add_widget(Button(text="CREATE", font_size=15, bold=True,
                                   on_press=create,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_box.add_widget(Button(text="CANCEL", font_size=15,
    ...
    def _chat_camera(self, instance):
        ...
        def capture(inst):
            if cam.texture:
                ...
                cam.texture.save(fn)
                user = (self.cp.get("name", "Guest") if self.cp
                        else self._current_username())
                self.chat_sys.send(user, self.chat_room.text, "[Photo]", image=fn)
                pp.dismiss()
                self._refresh_chat()
        btns.add_widget(Button(text="📸 CAPTURE", font_size=16, bold=True,
                               on_press=capture,
    ...
    def _ai_analysis_tab(self):
        ...
        main = BoxLayout(orientation='vertical', padding=6, spacing=4)
        main.add_widget(Label(text="AI CLINICAL ANALYSIS", font_size=20,
                              bold=True, size_hint_y=None, height=32,
                              color=get_color_from_hex('#8e44ad')))
        stats = GridLayout(cols=3, size_hint_y=None, height=60, spacing=5)
        ...
        self.ai_stats_rx = Label(text="Rx: 0", font_size=16, bold=True,
                                  color=get_color_from_hex('#e67e22'))
        stats.add_widget(self.ai_stats_emr)
        stats.add_widget(self.ai_stats_lab)
        stats.add_widget(self.ai_stats_rx)
        main.add_widget(stats)
        row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row.add_widget(Button(text="FULL PATIENT ANALYSIS", font_size=16,
                               bold=True, on_press=self._run_full_analysis,
                               background_color=get_color_from_hex('#8e44ad'),
                               background_normal=''))
        row.add_widget(Button(text="CLINICAL SUMMARY AI", font_size=16,
                               bold=True, on_press=self._run_clinical_summary_ai,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        row.add_widget(Button(text="PROGNOSIS AI", font_size=16, bold=True,
                               on_press=self._run_prognosis_ai,
                               background_color=get_color_from_hex('#e67e22'),
                               background_normal=''))
        row.add_widget(Button(text="TREATMENT AI", font_size=16, bold=True,
                               on_press=self._run_treatment_ai,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        main.add_widget(row)
        main.add_widget(Label(text="ANALYSIS OUTPUT", font_size=18, bold=True,
                               size_hint_y=None, height=28,
                               color=get_color_from_hex('#2c3e50')))
        scroll = ScrollView(size_hint_y=1, bar_width=10, scroll_type=['bars'])
        self.ai_analysis_output = TextInput(multiline=True, readonly=True,
                                             font_size=14,
                                             background_color=get_color_from_hex('#fafafa'),
                                             size_hint_y=None)
        self.ai_analysis_output.bind(
            minimum_height=self.ai_analysis_output.setter('height'))
        scroll.add_widget(self.ai_analysis_output)
        main.add_widget(scroll)
        row2 = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row2.add_widget(Button(text="EXPORT PDF", font_size=15, bold=True,
                               on_press=lambda x: self._export_to_pdf(
                                   self.ai_analysis_output.text,
                                   "AI_Analysis", "AI Clinical Analysis"),
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        row2.add_widget(Button(text="CLEAR", font_size=15, bold=True,
                               on_press=lambda x: setattr(self.ai_analysis_output,
                                                          'text', ''),
                               background_color=get_color_from_hex('#e74c3c'),
                               background_normal=''))
    ...
    def _run_full_analysis(self, instance):
        ...
        prompt = (f"Patient: {self.cp.get('name', '')} (ID: {pid})\n"
                  f"Age: {self.cp.get('age', 0)}\n"
                  f"EMR: {len(emrs)}, Lab: {len(labs)}, Rx: {len(rx)}\n"
                  f"Provide comprehensive clinical analysis.")
        self.ai_analysis_output.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a senior clinician.",
                                     prompt, mt=2000)
            Clock.schedule_once(lambda dt: setattr(
                self.ai_analysis_output, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _run_clinical_summary_ai(self, instance):
        ...
        prompt = (f"Patient: {self.cp.get('name', '')}, "
                  f"Age: {self.cp.get('age', 0)}\n"
                  f"Generate a concise clinical summary.")
        self.ai_analysis_output.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a clinician.", prompt, mt=1000)
            Clock.schedule_once(lambda dt: setattr(
                self.ai_analysis_output, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _run_prognosis_ai(self, instance):
        ...
        prompt = (f"Patient: {self.cp.get('name', '')}, "
                  f"Age: {self.cp.get('age', 0)}\n"
                  f"Provide prognosis assessment.")
        self.ai_analysis_output.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a clinician.", prompt, mt=1200)
            Clock.schedule_once(lambda dt: setattr(
                self.ai_analysis_output, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _run_treatment_ai(self, instance):
        ...
        prompt = (f"Patient: {self.cp.get('name', '')}, "
                  f"Age: {self.cp.get('age', 0)}\n"
                  f"Generate evidence-based treatment recommendations.")
        self.ai_analysis_output.text = "Generating..."
    ...
    def _ai_bos_tab(self):
        ...
        main = BoxLayout(orientation='vertical', padding=6, spacing=4)
        main.add_widget(Label(text="AI BUSINESS OPERATING SYSTEM", font_size=20,
                              bold=True, size_hint_y=None, height=32,
                              color=get_color_from_hex('#1a5276')))
        pb = BoxLayout(orientation='vertical', size_hint_y=0.3, spacing=3)
        pb.add_widget(Label(text="BUSINESS PROFILE", font_size=16, bold=True,
                            size_hint_y=None, height=24,
                            color=get_color_from_hex('#e67e22')))
        self.bos_profile_display = TextInput(multiline=True, readonly=True,
                                              font_size=13,
                                              background_color=get_color_from_hex('#fef9e7'),
                                              size_hint_y=None)
        self.bos_profile_display.bind(
            minimum_height=self.bos_profile_display.setter('height'))
        pb.add_widget(self.bos_profile_display)
        main.add_widget(pb)
        for buttons in [
            ...
            row = BoxLayout(size_hint_y=None, height=40, spacing=5)
            for txt, cb, col in buttons:
                row.add_widget(Button(text=txt, font_size=14, bold=True,
                                       on_press=cb, background_normal='',
                                       background_color=get_color_from_hex(col)))
            main.add_widget(row)
        main.add_widget(Label(text="AI-BOS OUTPUT", font_size=16, bold=True,
                               size_hint_y=None, height=24,
                               color=get_color_from_hex('#2c3e50')))
        scroll = ScrollView(size_hint_y=1, bar_width=10, scroll_type=['bars'])
        self.bos_output = TextInput(multiline=True, readonly=True, font_size=13,
                                     background_color=get_color_from_hex('#fafafa'),
                                     size_hint_y=None)
        self.bos_output.bind(minimum_height=self.bos_output.setter('height'))
        scroll.add_widget(self.bos_output)
        main.add_widget(scroll)
        row4 = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row4.add_widget(Button(text="EXPORT PDF", font_size=15, bold=True,
                               on_press=lambda x: self._export_to_pdf(
                                   self.bos_output.text, "AI_BOS",
                                   "AI-BOS Report"),
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        row4.add_widget(Button(text="CLEAR", font_size=15, bold=True,
                               on_press=lambda x: setattr(self.bos_output,
                                                          'text', ''),
                               background_color=get_color_from_hex('#e74c3c'),
                               background_normal=''))
    ...
    def _bos_discover(self, instance):
        q = self.bos.get_business_profile()
        text = "BUSINESS DISCOVERY QUESTIONS\n" + "=" * 50 + "\n\n"
        for k, v in q.items():
            text += f"{k}: {v}\n"
        self.bos_profile_display.text = text
        self.bos_output.text = "Answer these questions to build your business profile."

    def _bos_health(self, instance):
        scores = self.bos.calculate_health_score()
        text = "BUSINESS HEALTH SCORE\n" + "=" * 50 + "\n\n"
        for area, score in scores.items():
            st = ("🔴 Critical" if score < 40 else "🟡 Needs Work"
                  if score < 60 else "🟢 Good")
            text += f"{area}: {score}/100 - {st}\n"
    ...
    def _bos_revenue(self, instance):
        ...
        for opp, d in self.bos.revenue_engine.get("ranked", [])[:10]:
            text += (f"{opp.upper()}\n"
                     f"  Revenue Potential: {d['revenue_potential']}\n"
                     f"  Profit Potential: {d['profit_potential']}\n"
                     f"  Cost: {d['cost']}\n  ROI: {d['roi']}\n" +
                     "-" * 40 + "\n")
        self.bos_output.text = text
    ...
    def _bos_financial(self, instance):
        ...
        f = self.bos.financial_engine
        pl = f.get('profit_loss', {})
        text = (f"FINANCIAL ENGINE\n{'=' * 50}\n\nPROFIT & LOSS\n"
                f"Revenue: N{pl.get('revenue', 0):,.2f}\n"
                f"COGS: N{pl.get('cost_of_goods', 0):,.2f}\n"
                f"Gross Profit: N{pl.get('gross_profit', 0):,.2f}\n"
                f"Net Profit: N{pl.get('net_profit', 0):,.2f}\n")
        self.bos_output.text = text
    ...
    def _bos_roadmap(self, instance):
        r = self.bos.generate_implementation_roadmap()
        text = "IMPLEMENTATION ROADMAP\n" + "=" * 50 + "\n\n"
        for period, d in r.items():
            text += f"{period.upper().replace('_', ' ')}\n"
            for m in d.get('milestones', []):
                text += f"  - {m}\n"
            text += f"  Cost: {d.get('costs', 'N/A')}\n\n"
        self.bos_output.text = text

    def _bos_decision(self, instance):
        a = self.bos.decision_support(
            "Strategic growth planning",
            ["Expand services", "Improve technology", "Enhance customer experience"])
        self.bos_output.text = (f"DECISION SUPPORT\n{'=' * 50}\n\n"
                                 f"Context: {a['context']}\n"
                                 f"Options: {', '.join(a['options'])}\n"
    ...
    def _bos_documents(self, instance):
        ...
        c.add_widget(Label(text="Select Document Type:", font_size=16, bold=True,
                           size_hint_y=None, height=30))
        doc = Spinner(text="business_plan",
                       values=["business_plan", "sales_plan", "marketing_plan",
                               "referral_program", "sop", "employee_handbook",
                               "financial_report", "risk_assessment",
                               "pitch_deck", "grant_proposal",
                               "partnership_proposal", "project_plan",
                               "kpi_dashboard"],
                       size_hint_y=None, height=40)
        c.add_widget(doc)
        btn_box = BoxLayout(size_hint_y=None, height=40, spacing=5)
        ...
        def generate(inst):
            self.bos_output.text = self.bos.generate_document(doc.text)
            pp.dismiss()
        btn_box.add_widget(Button(text="GENERATE", font_size=15, bold=True,
                                   on_press=generate,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_box.add_widget(Button(text="CANCEL", font_size=15,
    ...
    def _snapshot_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="PATIENT SNAPSHOT", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="ID:", font_size=15, bold=True, size_hint_x=0.05))
        self.snapshot_pid = TextInput(size_hint_x=0.15, readonly=True,
                                       font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.snapshot_pid)
        bar.add_widget(Label(text="Name:", font_size=15, bold=True, size_hint_x=0.07))
        self.snapshot_name = TextInput(size_hint_x=0.35, readonly=True,
                                        font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.snapshot_name)
        l.add_widget(bar)
        row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        row.add_widget(Button(text="📸 CAPTURE SNAPSHOT", font_size=16,
                               bold=True, on_press=self._capture_snapshot,
                               background_color=get_color_from_hex('#27ae60'),
                               background_normal=''))
        row.add_widget(Button(text="VIEW SNAPSHOTS", font_size=16, bold=True,
                               on_press=self._view_snapshots,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        l.add_widget(row)
        l.add_widget(Label(text="SNAPSHOT HISTORY", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.snapshot_history = BoxLayout(orientation='vertical',
                                            size_hint_y=None, spacing=1)
        self.snapshot_history.bind(
            minimum_height=self.snapshot_history.setter('height'))
    ...
    def _capture_snapshot(self, instance):
        ...
        tasks = self.task_manager.get_tasks_for_patient(pid)
        latest = emrs[-1] if emrs else {}
        self.patient_snapshots.append({
            "patient_id": pid, "patient_name": self.cp.get("name", ""),
            "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "vitals": {"bp": latest.get("bp", ""),
            ...
            "lab_count": len(labs), "rx_count": len(rx),
            "emr_count": len(emrs),
        ...
        self.save_all()
        self._refresh_snapshots()
        self._popup("Snapshot Captured", f"Saved for {self.cp.get('name', '')}")
    ...
    def _refresh_snapshots(self):
        ...
        if not snaps:
            self.snapshot_history.add_widget(Label(
                ...
                font_size=13, size_hint_y=None, height=30))
            return
        for s in sorted(snaps, key=lambda x: x.get('ts', ''),
                        reverse=True)[:15]:
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            v = s.get('vitals', {})
            row.add_widget(Label(text=f"{s.get('ts','')[:16]} | "
                                       f"BP: {v.get('bp','N/A')} | "
                                       f"Tasks: {s.get('task_pending',0)} pending",
                                  font_size=12, size_hint_x=0.5))
            row.add_widget(Label(text=f"DX: {s.get('diagnosis','N/A')[:20]}",
                                  font_size=11, size_hint_x=0.25))
            btn = Button(text="VIEW", font_size=11, size_hint_x=0.15,
                          size_hint_y=None, height=25,
                          background_color=get_color_from_hex('#2980b9'),
                          background_normal='')
            btn.snap = s
    ...
    def _view_snapshot_detail(self, snapshot):
        v = snapshot.get('vitals', {})
        text = (f"SNAPSHOT - {snapshot.get('patient_name', '')}\n"
                f"{'=' * 50}\n\nDate: {snapshot.get('ts', '')}\n\n"
                f"VITAL SIGNS\n"
                f"  BP: {v.get('bp', 'N/A')}\n  Pulse: {v.get('pulse', 'N/A')}\n"
                f"  Temp: {v.get('temp', 'N/A')}\n  SpO2: {v.get('spo2', 'N/A')}\n"
                f"  RR: {v.get('rr', 'N/A')}\n  AVPU: {v.get('avpu', 'Alert')}\n\n"
                f"Diagnosis: {snapshot.get('diagnosis', 'N/A')}\n"
                f"Treatment: {snapshot.get('treatment', 'N/A')}\n"
                f"Captured by: {snapshot.get('captured_by', 'N/A')}\n")
    ...
    def _procedures_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="PROCEDURE MANAGEMENT", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        l.add_widget(bar)
        pf = BoxLayout(size_hint_y=None, height=40, spacing=4)
        pf.add_widget(Label(text="Procedure:", font_size=15, bold=True,
                             size_hint_x=0.12))
        self.proc_select = Spinner(text="Select",
                                    values=[p['name'] for p in self.procedures],
                                    size_hint_x=0.3, size_hint_y=None, height=40)
        pf.add_widget(self.proc_select)
        pf.add_widget(Button(text="ADD", font_size=15, bold=True,
                              size_hint_x=0.12, on_press=self._add_procedure,
                              background_color=get_color_from_hex('#27ae60'),
                              background_normal=''))
        pf.add_widget(Button(text="CUSTOM", font_size=15, bold=True,
                              size_hint_x=0.12,
                              on_press=self._add_custom_procedure,
                              background_color=get_color_from_hex('#8e44ad'),
                              background_normal=''))
        l.add_widget(pf)
        l.add_widget(Label(text="PROCEDURE LIST", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.proc_list = BoxLayout(orientation='vertical',
                                    size_hint_y=None, spacing=1)
    ...
    def _add_procedure(self, instance):
        ...
        self.procedures_db.append({
            ...
            "procedure": proc_name, "price": price,
            "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "status": "Scheduled"})
        self.save_all()
        self._refresh_procedures()
    ...
    def _add_custom_procedure(self, instance):
        if not self.proc_pid.text:
            ...
            return
        content = BoxLayout(orientation='vertical', spacing=5, padding=8)
        content.add_widget(Label(text="Custom Procedure", font_size=16,
                                  bold=True, size_hint_y=None, height=30))
        content.add_widget(Label(text="Procedure Name:", font_size=14,
                                  bold=True, size_hint_y=None, height=22))
        name_inp = TextInput(font_size=14, size_hint_y=None, height=40)
        content.add_widget(name_inp)
        content.add_widget(Label(text="Price (N):", font_size=14, bold=True,
    ...
    def _refresh_procedures(self):
        ...
        procs = [p for p in self.procedures_db
        ...
        for p in procs:
            row = BoxLayout(size_hint_y=None, height=30, spacing=3)
            row.add_widget(Label(text=p.get('procedure', ''), font_size=13,
                                  size_hint_x=0.35))
    ...
    def _vitals_timeline_tab(self):
        ...
        main.add_widget(Label(text="VITALS HISTORY", font_size=16, bold=True,
                               size_hint_y=None, height=25,
                               color=get_color_from_hex('#2980b9')))
        scroll = ScrollView(size_hint_y=1, bar_width=10, scroll_type=['bars'])
        ...
        scroll.add_widget(self.vitals_list)
        main.add_widget(scroll)
        tab.add_widget(main)
        self.tp.add_widget(tab)
        self._refresh_vitals_timeline()
    ...
    def _refresh_vitals_timeline(self):
        ...
        if not self.cp:
            self.vitals_list.add_widget(Label(
                text="Select a patient.", font_size=14,
                size_hint_y=None, height=30))
        ...
        if not records:
            self.vitals_list.add_widget(Label(text="No records found.",
                                                font_size=14,
                                                size_hint_y=None, height=30))
            return
        latest = records[-1]
        ...
        news_color = ("#27ae60" if news <= 4
                      else "#f39c12" if news <= 6 else "#e74c3c")
        ...
        for r in records:
            ...
            row.add_widget(Label(text=r.get("ts", "N/A")[:16], font_size=12,
                                  size_hint_x=0.15))
            row.add_widget(Label(text=r.get("bp", "-"), font_size=12,
                                  size_hint_x=0.1))
            row.add_widget(Label(text=r.get("pulse", "-"), font_size=12,
                                  size_hint_x=0.08))
            row.add_widget(Label(text=r.get("spo2", "-"), font_size=12,
                                  size_hint_x=0.08))
            row.add_widget(Label(text=r.get("rr", "-"), font_size=12,
                                  size_hint_x=0.08))
            row.add_widget(Label(text=r.get("temp", "-"), font_size=12,
                                  size_hint_x=0.08))
            row.add_widget(Label(text=r.get("avpu", "Alert"), font_size=12,
                                  size_hint_x=0.1))
    ...
    def _consent_form_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="INFORMED CONSENT", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
                              size_hint_x=0.12))
        self.consent_pid = TextInput(size_hint_x=0.2, readonly=True,
                                      font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.consent_pid)
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.consent_name = TextInput(size_hint_x=0.3, readonly=True,
                                       font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.consent_name)
        l.add_widget(bar)
        pb = BoxLayout(size_hint_y=None, height=40, spacing=4)
        pb.add_widget(Label(text="Procedure:", font_size=15, bold=True,
                             size_hint_x=0.15))
        self.consent_procedure = TextInput(font_size=15, size_hint_x=0.55,
                                            size_hint_y=None, height=40,
                                            hint_text="e.g., Appendectomy")
        pb.add_widget(self.consent_procedure)
        pb.add_widget(Button(text="AI GENERATE", font_size=15, bold=True,
                              size_hint_x=0.3,
        ...
        l.add_widget(Label(text="Consent Statement:", font_size=15, bold=True,
                           size_hint_y=None, height=18))
        self.consent_statement = TextInput(font_size=14, size_hint_y=None,
                                            height=150, multiline=True,
                                            hint_text="Consent statement...")
        ...
        btns.add_widget(Button(text="SAVE", font_size=16, bold=True,
                                ...
                                background_color=get_color_from_hex('#27ae60'),
                                background_normal=''))
        btns.add_widget(Button(text="EXPORT PDF", font_size=16, bold=True,
    ...
    def _ai_generate_consent(self, instance):
        ...
        if not procedure:
            ...
            return
        prompt = (f"Generate a professional informed consent statement for: "
                  f"{procedure}. Include procedure explanation, benefits, "
                  f"risks, alternatives, voluntary consent confirmation.")
        self.consent_statement.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a medical legal expert.",
                                     prompt, mt=500)
            Clock.schedule_once(lambda dt: setattr(
                self.consent_statement, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _save_consent(self, instance):
        ...
        self.consent_forms.append({
            ...
            "created_by": self._current_username()})
        self.save_all()
        self._popup("Saved", "Consent saved.")
    ...
    def _referral_note_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="REFERRAL NOTE", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#8e44ad')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
                              size_hint_x=0.12))
        self.referral_pid = TextInput(size_hint_x=0.2, readonly=True,
                                       font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.referral_pid)
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.referral_name = TextInput(size_hint_x=0.3, readonly=True,
                                        font_size=15, size_hint_y=None, height=40)
        bar.add_widget(self.referral_name)
        l.add_widget(bar)
        l.add_widget(Label(text="Referring Physician:", font_size=15, bold=True,
                           size_hint_y=None, height=18))
        self.referral_doctor = TextInput(font_size=15, size_hint_y=None,
                                          height=40, hint_text="Dr. ...")
        l.add_widget(self.referral_doctor)
        rb = BoxLayout(size_hint_y=None, height=40, spacing=4)
        rb.add_widget(Label(text="Reason:", font_size=15, bold=True,
                             size_hint_x=0.1))
        self.referral_reason = TextInput(font_size=15, size_hint_x=0.5,
                                          size_hint_y=None, height=40)
        rb.add_widget(self.referral_reason)
        rb.add_widget(Button(text="AI WRITE", font_size=15, bold=True,
                              size_hint_x=0.3,
        ...
        l.add_widget(Label(text="Clinical Summary:", font_size=15, bold=True,
                           size_hint_y=None, height=18))
        self.referral_summary = TextInput(font_size=15, size_hint_y=None,
                                           height=120, multiline=True)
    ...
    def _ai_generate_referral(self, instance):
        ...
        if not reason:
            self._popup("Error", "Enter reason first.")
            return
        prompt = (f"Write a professional referral letter. Reason: {reason}. "
                  f"Include patient summary, findings, urgency.")
        self.referral_summary.text = "Generating..."

        def run():
            ok, resp = self.ai.call("You are a medical referral specialist.",
                                     prompt, mt=800)
            Clock.schedule_once(lambda dt: setattr(
                self.referral_summary, 'text', resp), 0)
        threading.Thread(target=run, daemon=True).start()
    ...
    def _save_referral(self, instance):
        ...
        self.referral_notes.append({
            ...
            "created_by": self._current_username()})
        self.save_all()
        self._popup("Saved", "Referral saved.")
    ...
    def _appointments_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="APPOINTMENT SCHEDULING", font_size=20,
                           bold=True, size_hint_y=None, height=32,
                           color=get_color_from_hex('#2980b9')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        bar.add_widget(Button(text="REFRESH", font_size=13, size_hint_x=0.12,
                               ...
                               background_normal=''))
        l.add_widget(bar)
        l.add_widget(Label(text="NEW APPOINTMENT", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#e67e22')))
        form = GridLayout(cols=4, size_hint_y=None, height=150, spacing=4)
        for lbl, attr, w in [
            ("Date:", "appt_date",
             TextInput(text=datetime.now().strftime("%Y-%m-%d"),
                       font_size=14, size_hint_y=None, height=35)),
            ("Time:", "appt_time",
             TextInput(text=datetime.now().strftime("%H:%M"),
                       font_size=14, size_hint_y=None, height=35)),
            ("Type:", "appt_type",
             Spinner(text="Consultation",
                     values=["Consultation", "Follow-up", "Procedure",
                             "Lab Visit", "Radiology", "Review"],
                     size_hint_y=None, height=35)),
            ("With:", "appt_with",
             Spinner(text="General",
                     values=["General", "Dr. Smith", "Dr. Jones",
                             "Dr. Williams", "Nurse"],
                     size_hint_y=None, height=35)),
            ("Notes:", "appt_notes",
             TextInput(font_size=14, size_hint_y=None, height=35))]:
            form.add_widget(Label(text=lbl, font_size=14, bold=True))
            setattr(self, attr, w)
            form.add_widget(w)
        l.add_widget(form)
        btn_row = BoxLayout(size_hint_y=None, height=40, spacing=5)
        btn_row.add_widget(Button(text="SCHEDULE", font_size=16, bold=True,
                                   on_press=self._add_appointment,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="VIEW ALL", font_size=16, bold=True,
                                   on_press=self._view_all_appointments,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        l.add_widget(btn_row)
        l.add_widget(Label(text="APPOINTMENT LIST", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.appt_list = BoxLayout(orientation='vertical',
                                    size_hint_y=None, spacing=1)
    ...
    def _refresh_appointments(self):
        ...
        appts.sort(key=lambda x: x.get("date", ""), reverse=True)
        if not appts:
            self.appt_list.add_widget(Label(text="No appointments.",
                                              font_size=13,
                                              size_hint_y=None, height=30))
            return
        for a in appts:
            status_color = {"Scheduled": "#f39c12", "Completed": "#27ae60",
                            "Cancelled": "#e74c3c", "No-show": "#95a5a6"}.get(
                                a.get("status", "Scheduled"), "#7f8c8d")
            row = BoxLayout(size_hint_y=None, height=35, spacing=3)
            row.add_widget(Label(text=f"{a.get('date','')} {a.get('time','')}",
            ...
            row.add_widget(Label(text=a.get('with', ''), font_size=12,
                                  size_hint_x=0.15))
            row.add_widget(Label(text=a.get('status', 'Scheduled'), font_size=12,
                                  size_hint_x=0.12, bold=True,
                                  color=get_color_from_hex(status_color)))
            status_spinner = Spinner(text=a.get("status", "Scheduled"),
                                      values=["Scheduled", "Completed",
                                              "Cancelled", "No-show"],
                                      size_hint_x=0.1,
                                      size_hint_y=None, height=30)
            status_spinner.appt_id = a.get("id")
            status_spinner.bind(text=lambda s, aid=a.get("id"):
                                self._update_appointment_status(aid, s.text))
            row.add_widget(status_spinner)
            btn_del = Button(text="X", font_size=11, size_hint_x=0.08,
                              size_hint_y=None, height=25,
                              background_color=get_color_from_hex('#e74c3c'),
    ...
    def _add_appointment(self, instance):
        ...
        try:
            ...
            self._popup("Error", "Invalid date (YYYY-MM-DD).")
            return
        self.appointments.append({
            "id": f"APT{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
            "patient_id": self.appt_pid.text,
            "patient_name": self.appt_name.text,
            "date": date, "time": self.appt_time.text.strip(),
            "type": self.appt_type.text, "with": self.appt_with.text,
        ...
        self.save_all()
        self._refresh_appointments()
        self._popup("Scheduled", f"Appointment on {date}")
        self.audit_log("SCHEDULE_APPOINTMENT",
                       f"{date} {self.appt_time.text}", self.appt_pid.text)
    ...
    def _view_all_appointments(self, instance):
        ...
        appts = [a for a in self.appointments if a.get("patient_id") == pid]
        if not appts:
            self._popup("Appointments", "None found.")
            return
        text = ""
    ...
    def _nursing_notes_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="NURSING NOTES", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#1a5276')))
        bar = BoxLayout(size_hint_y=None, height=40, spacing=4)
        bar.add_widget(Label(text="Patient ID:", font_size=15, bold=True,
        ...
        bar.add_widget(Label(text="Name:", font_size=15, bold=True,
                              size_hint_x=0.08))
        self.nurse_name = TextInput(size_hint_x=0.3, readonly=True,
                                     font_size=15, size_hint_y=None, height=40)
        ...
        bar.add_widget(Button(text="REFRESH", font_size=13, size_hint_x=0.12,
                               ...
                               background_normal=''))
        l.add_widget(bar)
        l.add_widget(Label(text="NEW NURSING NOTE", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#e67e22')))
        ...
        for lbl, attr, w in [
            ("Date:", "nurse_date",
             TextInput(text=datetime.now().strftime("%Y-%m-%d"),
                       font_size=14, size_hint_y=None, height=35)),
            ("Shift:", "nurse_shift",
             Spinner(text="Morning",
                     values=["Morning", "Afternoon", "Night"],
                     size_hint_y=None, height=35)),
            ("Vitals:", "nurse_vitals",
             TextInput(font_size=14, size_hint_y=None, height=35,
                       hint_text="BP: 120/80, Pulse: 72")),
            ("Observations:", "nurse_obs",
             TextInput(font_size=14, size_hint_y=None, height=35)),
            ("Care Provided:", "nurse_care",
             TextInput(font_size=14, size_hint_y=None, height=35)),
            ("Status:", "nurse_status",
             Spinner(text="Stable",
                     values=["Stable", "Improving", "Deteriorating",
                             "Critical"],
                     size_hint_y=None, height=35))]:
            form.add_widget(Label(text=lbl, font_size=14, bold=True))
            setattr(self, attr, w)
            form.add_widget(w)
        l.add_widget(form)
        l.add_widget(Button(text="SAVE NURSING NOTE", font_size=16, bold=True,
                            size_hint_y=None, height=40,
                            on_press=self._add_nursing_note,
                            background_color=get_color_from_hex('#27ae60'),
                            background_normal=''))
        l.add_widget(Label(text="NURSING NOTES HISTORY", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.nurse_list = BoxLayout(orientation='vertical',
                                     size_hint_y=None, spacing=1)
    ...
    def _refresh_nursing_notes(self):
        ...
        if not self.nurse_pid.text:
            self.nurse_list.add_widget(Label(text="Select a patient.",
                                              font_size=13,
                                              size_hint_y=None, height=30))
            return
        ...
        if not notes:
            self.nurse_list.add_widget(Label(text="No nursing notes.",
                                              font_size=13,
                                              size_hint_y=None, height=30))
            return
        for n in notes:
            ...
            row.add_widget(Label(text=n.get('care', '')[:20], font_size=11,
                                  size_hint_x=0.25))
            btn_view = Button(text="VIEW", font_size=11, size_hint_x=0.15,
                               size_hint_y=None, height=25,
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal='')
            btn_view.note = n
            btn_view.bind(on_press=lambda x:
                          self._view_nursing_note_detail(x.note))
            row.add_widget(btn_view)
            btn_del = Button(text="X", font_size=11, size_hint_x=0.08,
                              size_hint_y=None, height=25,
                              background_color=get_color_from_hex('#e74c3c'),
            ...
            btn_del.bind(on_press=lambda x:
                         self._delete_nursing_note(x.note_id))
            row.add_widget(btn_del)
            self.nurse_list.add_widget(row)

    def _view_nursing_note_detail(self, note):
        text = (f"NURSING NOTE\n{'=' * 50}\n\n"
                f"Patient: {note.get('patient_name', '')} "
                f"(ID: {note.get('patient_id', '')})\n"
                f"Date: {note.get('date', '')}\n"
                f"Shift: {note.get('shift', '')}\n"
    ...
    def _add_nursing_note(self, instance):
        ...
        self._popup("Saved", f"Nursing note for {self.nurse_name.text} on {date}")
        self.audit_log("ADD_NURSING_NOTE",
                       f"{date} {self.nurse_shift.text}", self.nurse_pid.text)
    ...
    # ====================== MEDICAL REPORT DRAFTER ======================
    def _medical_report_tab(self):
        tab = TabbedPanelItem(text="Report Drafter")
        s = ScrollView()
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="MEDICAL REPORT DRAFTER", font_size=20,
                           bold=True, size_hint_y=None, height=32,
                           color=get_color_from_hex('#16a085')))
        l.add_widget(Label(text="All fields are blank by design — fill them in yourself.",
                           font_size=12, size_hint_y=None, height=20,
                           color=get_color_from_hex('#7f8c8d')))
        l.add_widget(Label(text="ISSUING FACILITY", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#1a5276')))
        fg = GridLayout(cols=2, size_hint_y=None, height=250, spacing=4)
        self.rpt_facility_name = TextInput(font_size=14, size_hint_y=None,
                                            height=38,
                                            hint_text="Hospital / facility name")
        self.rpt_facility_address = TextInput(font_size=14, size_hint_y=None,
                                               height=70, multiline=True,
                                               hint_text="Address")
        self.rpt_facility_contact = TextInput(font_size=14, size_hint_y=None,
                                               height=38,
                                               hint_text="Phone / email")
        self.rpt_ref_number = TextInput(font_size=14, size_hint_y=None,
                                         height=38,
                                         hint_text="Reference number")
        self.rpt_report_date = TextInput(
            text=datetime.now().strftime("%d %B %Y"),
            font_size=14, size_hint_y=None, height=38)
        for lb, w in [("Name:", self.rpt_facility_name),
                      ("Address:", self.rpt_facility_address),
                      ("Contact:", self.rpt_facility_contact),
                      ("Ref No.:", self.rpt_ref_number),
                      ("Date:", self.rpt_report_date)]:
            fg.add_widget(Label(text=lb, font_size=13, bold=True,
                                 size_hint_x=0.25))
            fg.add_widget(w)
        l.add_widget(fg)

        logo_row = BoxLayout(size_hint_y=None, height=42, spacing=5)
        logo_row.add_widget(Label(text="Facility Logo:", font_size=13,
        ...
        logo_row.add_widget(Button(text="ATTACH LOGO", font_size=13, bold=True,
                                    size_hint_x=0.25,
                                    on_press=lambda x:
                                    self._pick_image_for_report("logo"),
                                    background_color=get_color_from_hex('#2980b9'),
                                    background_normal=''))
        l.add_widget(logo_row)
        self.rpt_logo_path = None

        l.add_widget(Label(text="PATIENT DETAILS (auto-filled from selected patient)",
                           font_size=16, bold=True, size_hint_y=None, height=24,
                           color=get_color_from_hex('#1a5276')))
        pg = GridLayout(cols=4, size_hint_y=None, height=90, spacing=4)
        ...
        for lb, w in [("Name:", self.rpt_patient_name),
                      ("ID:", self.rpt_patient_id),
                      ("Age:", self.rpt_patient_age),
                      ("Sex:", self.rpt_patient_sex)]:
            pg.add_widget(Label(text=lb, font_size=13, bold=True))
            pg.add_widget(w)
        l.add_widget(pg)

        l.add_widget(Label(text="CLINICAL CONTENT", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#1a5276')))
        l.add_widget(Label(text="Diagnosis / Conditions:", font_size=13,
                           bold=True, size_hint_y=None, height=18))
        self.rpt_conditions = TextInput(font_size=13, size_hint_y=None,
                                         height=70, multiline=True)
        l.add_widget(self.rpt_conditions)
        l.add_widget(Label(text="Narrative Summary:", font_size=13, bold=True,
                           size_hint_y=None, height=18))
        self.rpt_narrative = TextInput(font_size=13, size_hint_y=None,
                                        height=140, multiline=True)
        l.add_widget(self.rpt_narrative)
        l.add_widget(Label(text="Recommendations (one per line):",
                           font_size=13, bold=True, size_hint_y=None, height=18))
        self.rpt_recommendations = TextInput(font_size=13, size_hint_y=None,
                                              height=140, multiline=True)
        l.add_widget(self.rpt_recommendations)

        l.add_widget(Label(text="SIGNING CLINICIAN", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#1a5276')))
        sg = GridLayout(cols=4, size_hint_y=None, height=90, spacing=4)
        self.rpt_doctor_name = TextInput(font_size=14, size_hint_y=None, height=38)
        self.rpt_doctor_qual = TextInput(font_size=14, size_hint_y=None, height=38)
        self.rpt_doctor_role = TextInput(font_size=14, size_hint_y=None, height=38)
        for lb, w in [("Name:", self.rpt_doctor_name),
                      ("Quals:", self.rpt_doctor_qual),
                      ("Role:", self.rpt_doctor_role)]:
            sg.add_widget(Label(text=lb, font_size=13, bold=True))
            sg.add_widget(w)
        l.add_widget(sg)

        sig_row = BoxLayout(size_hint_y=None, height=42, spacing=5)
        sig_row.add_widget(Label(text="Signature Image:", font_size=13,
                                  bold=True, size_hint_x=0.2))
        self.rpt_sig_label = Label(text="(none attached)", font_size=12,
                                    size_hint_x=0.5,
                                    color=get_color_from_hex('#7f8c8d'))
        sig_row.add_widget(self.rpt_sig_label)
        sig_row.add_widget(Button(text="ATTACH SIGNATURE", font_size=13,
                                   bold=True, size_hint_x=0.25,
                                   on_press=lambda x:
                                   self._pick_image_for_report("signature"),
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        ...
        btn_row.add_widget(Button(text="GENERATE PDF REPORT", font_size=16,
                                   bold=True,
                                   on_press=self._generate_medical_report_pdf,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="CLEAR", font_size=15, bold=True,
                                   on_press=lambda x:
                                   self._clear_medical_report_form(),
                                   background_color=get_color_from_hex('#e74c3c'),
                                   background_normal=''))
        l.add_widget(btn_row)
        s.add_widget(l)
    ...
    def _pick_image_for_report(self, target):
        ...
        cam_avail = CameraHelper.available()
        pb = Button(
            text="📷  Take Photo" if cam_avail
            else "📷  Take Photo (unavailable)",
            font_size=15, bold=True, size_hint_y=None, height=48,
            background_color=get_color_from_hex('#2980b9'),
            background_normal='', disabled=not cam_avail)
        pb.bind(on_press=take_photo)
        menu.add_widget(pb)
        fb = Button(text="📁  Choose File", font_size=15, bold=True,
                     size_hint_y=None, height=48,
                     background_color=get_color_from_hex('#27ae60'),
                     background_normal='')
        fb.bind(on_press=choose_file)
        menu.add_widget(fb)
        cb = Button(text="Cancel", font_size=14, size_hint_y=None, height=40,
                     background_color=get_color_from_hex('#e74c3c'),
                     background_normal='')
        cb.bind(on_press=lambda *_: pp.dismiss())
        menu.add_widget(cb)
        pp.open()
    ...
    def _pick_image_from_file(self, target):
        ...
        btns = BoxLayout(size_hint_y=None, height=42, spacing=5)
        pp = Popup(
            title=f"Choose {'Logo' if target == 'logo' else 'Signature'}",
        ...
        def select(inst):
            ...
            pp.dismiss()
        btns.add_widget(Button(text="SELECT", font_size=15, bold=True,
                                on_press=select,
                                background_color=get_color_from_hex('#27ae60'),
                                background_normal=''))
    ...
    def _take_photo_for_report(self, target):
        ...
        try:
            ...
            self._popup("Camera Error", str(e))
            return
        content = BoxLayout(orientation='vertical', spacing=4, padding=4)
        content.add_widget(cam)
        btns = BoxLayout(size_hint_y=None, height=44, spacing=5)
        ...
        def capture(inst):
            ...
            try:
                ...
                self._popup("Photo Saved",
                            f"Saved as {os.path.basename(fn)}")
            except Exception as e:
        ...
        btns.add_widget(Button(text="📸 CAPTURE", font_size=16, bold=True,
                                ...
                                background_color=get_color_from_hex('#27ae60'),
                                background_normal=''))
        btns.add_widget(Button(text="CANCEL", font_size=16, on_press=cancel,
                                background_color=get_color_from_hex('#e74c3c'),
                                background_normal=''))
        content.add_widget(btns)
        pp.open()
    ...
    def _set_report_image(self, target, path):
        if not path or not os.path.exists(path):
            self._popup("Error", f"File not found: {path}")
            return
        if target == "logo":
    ...
    def _generate_medical_report_pdf(self, instance):
        if not REPORTLAB_AVAILABLE:
            self._popup("Error", "ReportLab is not installed.")
            return
        if (not self.rpt_patient_name.text.strip() or
                not self.rpt_doctor_name.text.strip()):
            self._popup("Error", "Fill at least patient and doctor name.")
            return
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        ...
        try:
            ...
            def ensure(n):
                if state["y"] - n < bottom_limit:
                    new_page()

            def heading(text, size=11.5):
                ensure(4 * mm + size + 3 * mm)
                state["y"] -= 4 * mm
                c.setFont(FB, size)
            ...
            def ln(text, size=None, bold=False, italic=False,
                   gap=None, color=None):
                size = size or BS
                gap = gap or BL
                ensure(gap)
            ...
            def para(text, size=None, gap=None, indent=0, prefix=""):
                size = size or BS
                gap = gap or BL
                maxw = content_width - indent
                pw = c.stringWidth(prefix, FR, size) if prefix else 0
                wrapped = wrap(text, size,
                                maxw - pw if prefix else maxw)
                for i, wl in enumerate(wrapped):
                    ensure(gap)
                    c.setFont(FR, size)
                    if i == 0 and prefix:
                        c.setFont(FB, size)
                        c.drawString(margin, state["y"], prefix)
                        c.setFont(FR, size)
                        c.drawString(margin + pw, state["y"], wl)
                    else:
                        c.drawString(margin + indent, state["y"], wl)
                    state["y"] -= gap
            ...
            if self.rpt_logo_path and os.path.exists(self.rpt_logo_path):
                try:
                    ...
                    c.drawImage(img, margin, ht - 20 * mm,
            ...
            c.setFont(FB, 19)
            c.setFillColorRGB(*ACCENT)
            c.drawString(hx, ht - 7 * mm,
                         self.rpt_facility_name.text.strip() or "[Facility name]")
            c.setFillColorRGB(0, 0, 0)
            c.setFont(FR, 8.5)
            c.setFillColorRGB(*MUTED)
            addr = (self.rpt_facility_address.text or "").strip().split("\n")
            ay = ht - 12.5 * mm
            for al in addr[:2]:
                if al.strip():
                    c.drawString(hx, ay, al.strip())
                    ay -= 4 * mm
            if self.rpt_facility_contact.text.strip():
                c.drawString(hx, ay, self.rpt_facility_contact.text.strip())
            c.setFillColorRGB(0, 0, 0)
            state["y"] = ht - 24 * mm
            c.setStrokeColorRGB(*ACCENT)
            c.setLineWidth(1.4)
            ...
            state["y"] -= 2.5 * mm
            c.setLineWidth(0.8)
            tw = c.stringWidth("MEDICAL REPORT", FB, 14.5)
            c.line(width / 2 - tw / 2, state["y"],
                   width / 2 + tw / 2, state["y"])
            state["y"] -= 9 * mm
            ln(f"RE:  {self.rpt_patient_name.text.strip()}",
            ...
            if details:
                ln("   |   ".join(details), size=9.5, color=MUTED, gap=13)
            state["y"] -= 3 * mm
            ln("To Whom It May Concern,", gap=15)
            state["y"] -= 2 * mm
            if self.rpt_conditions.text.strip():
                heading("Diagnosis / Conditions")
                para(self.rpt_conditions.text.strip())
                state["y"] -= 2 * mm
            if self.rpt_narrative.text.strip():
                heading("Clinical Summary")
                for p in self.rpt_narrative.text.strip().split("\n"):
                    if p.strip():
                        para(p.strip())
                        state["y"] -= 1.5 * mm
                state["y"] -= 1 * mm
            if self.rpt_recommendations.text.strip():
                ...
                items = [r.strip() for r in
                         self.rpt_recommendations.text.strip().split("\n")
                         if r.strip()]
                for i, item in enumerate(items, start=1):
                    para(item, indent=6 * mm, prefix=f"{i}.  ")
                    state["y"] -= 1.5 * mm
                state["y"] -= 2 * mm
            ensure(38 * mm)
            state["y"] -= 4 * mm
            ln("Yours faithfully,", gap=15)
            if self.rpt_signature_path and os.path.exists(self.rpt_signature_path):
                try:
                    img = ImageReader(self.rpt_signature_path)
                    sh = 14 * mm
                    c.drawImage(img, margin, state["y"] - sh,
                                width=42 * mm, height=sh,
                                preserveAspectRatio=True, mask="auto",
                                anchor="sw")
                except Exception:
                    pass
            ...
            state["y"] -= 5.5 * mm
            ln(self.rpt_doctor_name.text.strip() or "[Doctor name]",
               size=11, bold=True, gap=14)
            if self.rpt_doctor_qual.text.strip():
                ln(self.rpt_doctor_qual.text.strip(), size=9.5,
                   color=MUTED, gap=12.5)
            if self.rpt_doctor_role.text.strip():
                ln(self.rpt_doctor_role.text.strip(), size=9.5,
                   color=MUTED, gap=12.5)
            if self.rpt_facility_name.text.strip():
                ln(self.rpt_facility_name.text.strip(), size=9.5,
                   color=MUTED, gap=12.5)
            draw_footer()
            c.save()
            # Create the same professional image-format report from the EMR report fields.
            medical_text = (
                f"Chief Complaint: {self.rpt_complaint.text.strip()}\n"
                f"History: {self.rpt_history.text.strip()}\n"
                f"Examination: {self.rpt_examination.text.strip()}\n"
                f"Investigations: {self.rpt_investigations.text.strip()}\n"
                f"Diagnosis: {self.rpt_diagnosis.text.strip()}\n"
                f"Treatment / Plan: {self.rpt_treatment.text.strip()}\n"
                f"Recommendations: {self.rpt_recommendations.text.strip()}")
            patient = self._patient_identity_for_report()
            image_path = self._render_professional_report_image(
                medical_text, 'Medical_Report', 'Medical Report', patient=patient,
                report_id=f"HMG-MED-{datetime.now().strftime('%Y%m%d%H%M%S')}")
            self._popup("Report Generated", f"Saved to {out_path}" +
                        (f"\nImage report: {image_path}" if image_path else ""))
            ...
            self._popup("Report Error", str(e))
    ...
    def _user_management_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="USER MANAGEMENT", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#8e44ad')))
        l.add_widget(Label(text="Administrator access only", font_size=12,
                           size_hint_y=None, height=20,
                           color=get_color_from_hex('#7f8c8d')))
        ...
        l.add_widget(form)
        btn_row = BoxLayout(size_hint_y=None, height=42, spacing=5)
        btn_row.add_widget(Button(text="CREATE USER", font_size=15, bold=True,
                                   on_press=self._create_new_user,
                                   background_color=get_color_from_hex('#27ae60'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="RESET PASSWORD", font_size=15, bold=True,
                                   on_press=self._admin_reset_password,
                                   background_color=get_color_from_hex('#2980b9'),
                                   background_normal=''))
        btn_row.add_widget(Button(text="DELETE USER", font_size=15, bold=True,
        ...
        btn_row.add_widget(Button(text="REFRESH", font_size=15, bold=True,
                                   ...
                                   background_normal=''))
        l.add_widget(btn_row)
        l.add_widget(Label(text="USER LIST", font_size=16, bold=True,
                           size_hint_y=None, height=24,
                           color=get_color_from_hex('#2c3e50')))
        self.user_list = BoxLayout(orientation='vertical',
                                    size_hint_y=None, spacing=1)
    ...
    def _create_new_user(self, instance):
        ...
        ok, msg = self.user_manager.create_user(
            self.um_username.text.strip(), self.um_fullname.text.strip(),
            self.um_password.text, self.um_role.text)
    ...
    def _admin_reset_password(self, instance):
        ...
        if not username:
            self._popup("Error", "Enter username to reset.")
            return
        if len(self.um_password.text) < 4:
            self._popup("Error", "Enter new password (min 4 chars).")
            return
        ok, msg = self.user_manager.admin_reset_password(
            username, self.um_password.text)
    ...
    def _delete_user(self, instance):
        ...
        username = self.um_username.text.strip()
        if not username:
            self._popup("Error", "Enter username to delete.")
            return
        if username == self._current_username():
            self._popup("Error", "Cannot delete your own account.")
            return
    ...
    def _audit_log_tab(self):
        ...
        l = BoxLayout(orientation='vertical', padding=6, spacing=4, size_hint_y=None)
        l.bind(minimum_height=l.setter('height'))
        l.add_widget(Label(text="AUDIT LOG", font_size=20, bold=True,
                           size_hint_y=None, height=32,
                           color=get_color_from_hex('#c0392b')))
        l.add_widget(Label(text="Administrator access only", font_size=12,
                           size_hint_y=None, height=20,
        ...
        row.add_widget(Label(text="Filter user:", font_size=13, bold=True,
                              size_hint_x=0.12))
        self.audit_user_filter = TextInput(font_size=13, size_hint_x=0.3,
                                            size_hint_y=None, height=36)
        row.add_widget(self.audit_user_filter)
        row.add_widget(Button(text="APPLY", font_size=13, bold=True,
                               size_hint_x=0.15,
                               on_press=lambda x: self._refresh_audit_log(
                                   self.audit_user_filter.text.strip()),
                               background_color=get_color_from_hex('#2980b9'),
                               background_normal=''))
        row.add_widget(Button(text="SHOW ALL", font_size=13, bold=True,
                               size_hint_x=0.15,
                               on_press=lambda x: (setattr(
                                   self.audit_user_filter, 'text', ''),
                                   self._refresh_audit_log()),
                               background_color=get_color_from_hex('#16a085'),
                               background_normal=''))
        row.add_widget(Button(text="EXPORT", font_size=13, bold=True,
                               size_hint_x=0.15,
                               on_press=self._export_audit_log,
                               background_color=get_color_from_hex('#8e44ad'),
                               background_normal=''))
        l.add_widget(row)
        self.audit_display = TextInput(multiline=True, readonly=True,
                                        font_size=12,
                                        background_color=get_color_from_hex('#f8f9fa'),
    ...
    def _refresh_audit_log(self, username_filter=""):
        ...
        entries = (self.audit.filter_by_user(username_filter)
                   if username_filter else self.audit.get_recent(300))
        lines = [f"AUDIT LOG — {len(entries)} "
